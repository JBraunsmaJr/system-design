...
assert(
  h2026.some((h) => h.name === "New Year's Day" && h.date === '2026-01-01'),
  "New Year's Day 2026 is Jan 1",
);
assert(
  h2026.some((h) => h.name === 'Independence Day' && h.date === '2026-07-03'),
  'July 4 on Saturday is observed Friday July 3',
);
assert(
  h2026.some((h) => h.name === 'Thanksgiving Day' && h.date === '2026-11-26'),
  'Thanksgiving 2026 is 4th Thursday (Nov 26)',
);

console.log('\n2. Testing Date and Weekend Helpers:');
assert(
  parseISODate('2026-09-02') === parseISODate(formatISODate(parseISODate('2026-09-02'))),
  'Roundtrip ISO date parse/format',
);
...
assert(
  summary.sprintBusinessDays === 8,
  `Sprint business days is 8 (actual: ${summary.sprintBusinessDays})`,
);
...
assert(
  aliceCap.workingDays === 5.5,
  `Alice working days in sprint is 5.5 (actual: ${aliceCap.workingDays})`,
);
assert(
  aliceCap.capacityPoints === 5.5,
  `Alice capacity is 5.5 pts (actual: ${aliceCap.capacityPoints})`,
);
assert(
  aliceCap.assignedPoints === 5,
  `Alice assigned points is 5 pts (actual: ${aliceCap.assignedPoints})`,
);
assert(
  aliceCap.remainingPoints === 0.5,
  `Alice remaining points is 0.5 pts (actual: ${aliceCap.remainingPoints})`,
);
...
assert(
  bobCap.assignedPoints === 10,
  `Bob assigned points is 10 (actual: ${bobCap.assignedPoints})`,
);
assert(
  bobCap.remainingPoints === 6,
  `Bob remaining points is 6 (actual: ${bobCap.remainingPoints})`,
);
...
assert(
  summary.totalCapacityPoints === 21.5,
  `Team total capacity is 21.5 pts (actual: ${summary.totalCapacityPoints})`,
);
assert(
  summary.totalAssignedPoints === 17,
  `Team total assigned points is 17 pts (actual: ${summary.totalAssignedPoints})`,
);
assert(
  summary.unassignedPoints === 2,
  `Team unassigned points is 2 pts (actual: ${summary.unassignedPoints})`,
);
assert(
  summary.remainingCapacityPoints === 4.5,
  `Team remaining capacity is 4.5 pts (actual: ${summary.remainingCapacityPoints})`,
);

console.log('\n4. Testing Total PTO Days Calculation:');
assert(
  calculateTotalPtoDays(team.members[0]) === 2.5,
  `Total PTO days for Alice is 2.5 (actual: ${calculateTotalPtoDays(team.members[0])})`,
);
...
const reservation20Percent = [{ id: 'r1', name: 'Risk Reserve', unit: 'percentage', value: 20 }];

const capSummary20Pct = computeSprintCapacity(
  twoWeekSprint,
  twoWeekRange,
  teamTwoMembers,
  [],
  reservation20Percent,
);
assert(
  capSummary20Pct.grossCapacityPoints === 20,
  `Gross team capacity is 20 pts (actual: ${capSummary20Pct.grossCapacityPoints})`,
);
assert(
  capSummary20Pct.totalReservedPoints === 4,
  `Total reserved capacity is 4 pts (actual: ${capSummary20Pct.totalReservedPoints})`,
);
assert(
  capSummary20Pct.totalCapacityPoints === 16,
  `Total available net capacity is 16 pts (actual: ${capSummary20Pct.totalCapacityPoints})`,
);
...
assert(
  u1Cap.grossCapacityPoints === 10,
  `Dev 1 gross is 10 pts (actual: ${u1Cap.grossCapacityPoints})`,
);
assert(u1Cap.reservedPoints === 2, `Dev 1 reserved is 2 pts (actual: ${u1Cap.reservedPoints})`);
assert(
  u1Cap.capacityPoints === 8,
  `Dev 1 available capacity is 8 pts (actual: ${u1Cap.capacityPoints})`,
);
assert(
  u2Cap.capacityPoints === 8,
  `Dev 2 available capacity is 8 pts (actual: ${u2Cap.capacityPoints})`,
);

console.log('\n7. Testing Capacity Reservation - Whole Number / Fixed Points Deduction (Ceiled):');
const reservationFixedPoints = [{ id: 'r2', name: 'Bug Backlog', unit: 'points', value: 5 }];
...
const capSummaryFixed = computeSprintCapacity(
  twoWeekSprint,
  twoWeekRange,
  teamTwoMembers,
  [],
  reservationFixedPoints,
);
assert(
  capSummaryFixed.grossCapacityPoints === 20,
  `Gross team capacity is 20 pts (actual: ${capSummaryFixed.grossCapacityPoints})`,
);
assert(
  capSummaryFixed.totalReservedPoints === 6,
  `Total reserved capacity is 6 pts (actual: ${capSummaryFixed.totalReservedPoints})`,
);
assert(
  capSummaryFixed.totalCapacityPoints === 14,
  `Total available net capacity is 14 pts (actual: ${capSummaryFixed.totalCapacityPoints})`,
);
assert(
  capSummaryFixed.memberBreakdown[0].reservedPoints === 3,
  `Member 1 reserved points is 3 pts (actual: ${capSummaryFixed.memberBreakdown[0].reservedPoints})`,
);
assert(
  capSummaryFixed.memberBreakdown[0].capacityPoints === 7,
  `Member 1 available points is 7 pts (actual: ${capSummaryFixed.memberBreakdown[0].capacityPoints})`,
);
...
const capSummary15Pct = computeSprintCapacity(
  twoWeekSprint,
  twoWeekRange,
  teamTwoMembers,
  [],
...
assert(
  capSummary15Pct.memberBreakdown[0].reservedPoints === 2,
  `15% on 10 pts gives 2 pts reserved (ceil) (actual: ${capSummary15Pct.memberBreakdown[0].reservedPoints})`,
);
assert(
  capSummary15Pct.memberBreakdown[0].capacityPoints === 8,
  `15% on 10 pts gives 8 pts available (actual: ${capSummary15Pct.memberBreakdown[0].capacityPoints})`,
);
assert(
  capSummary15Pct.totalReservedPoints === 4,
  `Total reserved is 4 pts (actual: ${capSummary15Pct.totalReservedPoints})`,
);
assert(
  capSummary15Pct.totalCapacityPoints === 16,
  `Total net is 16 pts (actual: ${capSummary15Pct.totalCapacityPoints})`,
);
...
const capSummaryMulti = computeSprintCapacity(
  twoWeekSprint,
  twoWeekRange,
  teamTwoMembers,
  [],
...
assert(
  capSummaryMulti.grossCapacityPoints === 20,
  `Multi: Gross is 20 pts (actual: ${capSummaryMulti.grossCapacityPoints})`,
);
assert(
  capSummaryMulti.totalReservedPoints === 6,
  `Multi: Total reserved is 6 pts (actual: ${capSummaryMulti.totalReservedPoints})`,
);
assert(
  capSummaryMulti.totalCapacityPoints === 14,
  `Multi: Net available is 14 pts (actual: ${capSummaryMulti.totalCapacityPoints})`,
);
assert(
  capSummaryMulti.memberBreakdown[0].reservedPoints === 3,
  `Multi: Member 1 reserved is 3 pts (actual: ${capSummaryMulti.memberBreakdown[0].reservedPoints})`,
);
assert(
  capSummaryMulti.memberBreakdown[0].capacityPoints === 7,
  `Multi: Member 1 available is 7 pts (actual: ${capSummaryMulti.memberBreakdown[0].capacityPoints})`,
);
...
assert(
  sp1ActiveRes.length === 1 && sp1ActiveRes[0].id === 'r-pi',
  'Sprint 1 gets only the PI-level reservation',
);
assert(
  sp2ActiveRes.length === 2,
  'Sprint 2 gets both the PI-level and sprint-specific reservations',
);
...
assert(
  piSummaries[0].grossCapacityPoints === 20 &&
    piSummaries[0].totalReservedPoints === 2 &&
    piSummaries[0].totalCapacityPoints === 18,
  `Sprint 1 capacity is 18 pts net (actual: ${piSummaries[0].totalCapacityPoints})`,
);
// Sprint 2: 20 gross, 10% (2 pts) + 4 pts = 6 pts reserved -> 14 net
assert(
  piSummaries[1].grossCapacityPoints === 20 &&
    piSummaries[1].totalReservedPoints === 6 &&
    piSummaries[1].totalCapacityPoints === 14,
  `Sprint 2 capacity is 14 pts net (actual: ${piSummaries[1].totalCapacityPoints})`,
);

console.log('\n10. Testing Serialization of ProgramIncrement.reservations:');
const diagramWithReservations = toDiagramFile(
  'Test PI Reserves',
  [],
  [],
  [],
...
assert(
  parsedDiagram.programIncrements[0].reservations.length === 2,
  'Parsed diagram retains 2 capacity reservations',
);
assert(
  parsedDiagram.programIncrements[0].reservations[0].name === 'General 10% Reserve',
  'Parsed reservation retains name',
);
assert(
  parsedDiagram.programIncrements[0].reservations[1].sprintId === 'sp-2',
  'Parsed reservation retains sprintId',
);
...
assert(
  piTotalCapacity === 32,
  `PI total net available capacity is 32 pts (actual: ${piTotalCapacity})`,
);
