...
export type DirectoryErrorReason =
  'not-found' | 'conflict' | 'revoked' | 'not-approved' | 'bad-request';
...
export interface UserDirectory {
  ...
  registerDevice(
    userId: string,
    device: { publicKey: string; label?: string },
...
export function createMemoryUserDirectory(now: () => Date = () => new Date()): UserDirectory {
  ...
  function requireDevice(userId: string, deviceId: string): DeviceRecord {
    ...
    if (!device || device.userId !== userId)
      throw new DirectoryError(`No device ${deviceId}.`, 'not-found');
    return device;
  }
  ...
  return {
    ...
    async registerDevice(userId, device) {
      ...
      const isFirst = ![...devices.values()].some(
    ...
    async getWorkspaceKeys(userId) {
      return [...(workspaceKeys.get(userId) ?? new Map<number, string>())].map(
        ([generation, wrappedKey]) => ({ generation, wrappedKey }),
      );
    },
  };
}
