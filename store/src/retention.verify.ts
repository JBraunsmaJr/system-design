...
console.log('=== Parsing ===');
assert(
  parseRetentionPeriod(undefined).kind === 'duration',
  `unset means the default (${DEFAULT_RETENTION})`,
);
...
console.log('\n=== When purge becomes due ===');
{
  ...
  assert(
    purgeDueAt(parseRetentionPeriod('immediate'), deletedAt)?.getTime() === deletedAt.getTime(),
    'immediately, under immediate',
  );
  assert(
    purgeDueAt(parseRetentionPeriod('indefinite'), deletedAt) === null,
    'never on its own, under indefinite',
  );
  const due = purgeDueAt(parseRetentionPeriod('30d'), deletedAt);
  assert(
    due?.toISOString() === '2026-10-17T12:00:00.000Z',
    `thirty days later, under 30d (${due?.toISOString()})`,
  );
}
