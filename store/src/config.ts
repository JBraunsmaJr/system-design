...
export function loadStoreConfig(env: Env = process.env): StoreConfig {
  ...
  const publicUrl = url(
    ...
    required(
      env,
      'PUBLIC_URL',
      'the address people reach this store at, so sign-in can return to it.',
    ),
  );
  ...
  for (const id of list(env.AUTH_PROVIDERS)) {
    if (id === 'oidc') {
      providers.push({
        ...
        issuer: url(
          ...
          required(
            env,
            'OIDC_ISSUER',
            "the identity provider's issuer URL, for example a Keycloak realm.",
          ),
        ),
        // Where this server reaches the provider, when that differs from
        // where the browser does - a container network, usually.
        ...(env.OIDC_INTERNAL_URL?.trim()
          ? { internalUrl: url('OIDC_INTERNAL_URL', env.OIDC_INTERNAL_URL.trim()) }
          : {}),
      ...
      throw new ConfigError(
        `AUTH_PROVIDERS names an unknown provider "${id}". Supported: oidc, github.`,
      );
    }
  }
  ...
  const recoveryPublicKeyPem = (() => {
    ...
    if (inline && path)
      throw new ConfigError('Set RECOVERY_PUBLIC_KEY or RECOVERY_PUBLIC_KEY_FILE, not both.');
  ...
  const afterLoginUrl = configuredAfterLogin
    ? url('AFTER_LOGIN_URL', configuredAfterLogin)
    : (allowedOrigins[0] ?? publicUrl);
...
export function describeConfig(config: StoreConfig): string[] {
  return [
    ...
    config.databaseUrl
      ? 'Storage: PostgreSQL'
      : 'Storage: in memory (nothing is kept when this process stops)',
    config.providers.length
      ? `Sign-in: ${config.providers
          .map(
            (p) =>
              `${p.id}${p.issuer ? ` at ${p.issuer}` : ''}${p.internalUrl ? ` (reached here as ${p.internalUrl})` : ''}`,
          )
          .join(', ')}`
      : 'Sign-in: NONE - this store is open to anyone who can reach it',
    config.allowedOrigins.length
      ? `Editor origins: ${config.allowedOrigins.join(', ')}`
      : 'Editor origins: same origin only',
    `After signing in, people are sent to ${config.afterLoginUrl}`,
    config.admins.length
      ? `Administrators: ${config.admins.length}`
      : 'Administrators: none configured - holds and purges are refused to everyone',
