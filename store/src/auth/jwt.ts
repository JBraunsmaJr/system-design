...
export function verifyIdToken(token: string, options: VerifyOptions): IdTokenClaims {
  ...
  if (header.alg !== "RS256") {
    ...
    throw new TokenError(`Unsupported token algorithm ${header.alg ?? "(absent)"}; RS256 is required.`, "algorithm");
  }

  const candidates = options.keys.filter((key) => key.kty === "RSA" && (!header.kid || !key.kid || key.kid === header.kid));
  if (candidates.length === 0) throw new TokenError("No key in the provider's JWKS matches this token.", "unknown-key");

  const signed = Buffer.from(`${headerSegment}.${payloadSegment}`);
  const signature = Buffer.from(signatureSegment, "base64url");
  const signedByProvider = candidates.some((key) => {
    try {
      return verifySignature("RSA-SHA256", signed, createPublicKey({ key: key as never, format: "jwk" }), signature);
    } catch {
      return false;
    }
  });
  if (!signedByProvider) throw new TokenError("The token's signature does not match the provider's keys.", "signature");
  ...
  if (typeof claims.exp !== "number" || claims.exp + skew < now) throw new TokenError("The token has expired.", "expired");
  if (typeof claims.iat === "number" && claims.iat - skew > now) throw new TokenError("The token is not valid yet.", "issued-in-future");
  ...
  if (typeof claims.sub !== "string" || claims.sub.length === 0) throw new TokenError("The token carries no subject.", "subject");
  return claims;
}
