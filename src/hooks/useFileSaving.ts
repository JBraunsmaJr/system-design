...
function testHandleStore(): FileHandleStore | null {
  ...
  const supplied = (globalThis as unknown as { __TEST_FILE_HANDLE_STORE__?: FileHandleStore })
    .__TEST_FILE_HANDLE_STORE__;
...
export function useFileSaving(docId: string, deps: FileSavingDeps = {}): FileSaving {
  ...
  const [handles] = useState(
    () => deps.handles ?? testHandleStore() ?? createIndexedDbHandleStore(),
  );
  ...
  const adopt = useCallback(
    (state: ReturnType<FileBackedAutosave['state']>) => {
  ...
  const applyWrite = useCallback(
    (result: Awaited<ReturnType<FileBackedAutosave['write']>>) => {
      if (result.ok) {
        setStatus('written');
        setMessage(undefined);
