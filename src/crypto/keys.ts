...
export async function deriveStorageKey(documentKey: string): Promise<CryptoKey> {
  const material = await subtle().importKey(
    'raw',
    buffer(secretBytes(documentKey)),
    'HKDF',
    false,
    ['deriveKey'],
  );
  return subtle().deriveKey(
    {
      ...
      info: encoder.encode(STORAGE_KEY_INFO),
    },
    material,
...
export async function generateWrappingKeyPair(
  purpose: KeyPairPurpose = 'device',
...
export async function importRecoveryPrivateKey(pkcs8: Uint8Array): Promise<CryptoKey> {
  return subtle().importKey('pkcs8', buffer(pkcs8), { name: 'RSA-OAEP', hash: 'SHA-256' }, false, [
    'unwrapKey',
  ]);
}
...
export async function importPublicKey(spki: Uint8Array): Promise<CryptoKey> {
  return subtle().importKey('spki', buffer(spki), { name: 'RSA-OAEP', hash: 'SHA-256' }, true, [
    'wrapKey',
  ]);
}
...
export async function importDocumentKey(hex: string): Promise<CryptoKey> {
  return subtle().importKey('raw', buffer(secretBytes(hex)), { name: 'AES-GCM' }, true, [
    'encrypt',
    'decrypt',
  ]);
}
...
export async function wrapPrivateKeyForPublicKey(
  ...
  const transport = await subtle().generateKey({ name: 'AES-GCM', length: 256 }, true, [
    'encrypt',
    'decrypt',
  ]);
  return {
...
export function parseRecoveryCode(
  input: string,
): { ok: true; secret: Uint8Array } | { ok: false; problem: RecoveryCodeProblem } {
  const cleaned = input
    .toUpperCase()
    .replace(/[\s-]/g, '')
    .split('')
