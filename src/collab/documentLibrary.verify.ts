...
console.log("\n=== WS4-R7: compaction keeps identity, and offline clients merge ===");
{
  ...
  const offlineSees = (offline.getMap("nodes").get("outer") as Y.Map<unknown>).get("position") as { x: number };
  assert(merged === "Edited offline" && offlineSees.x === 42, "an offline client reconciles with no loss in either direction");
...
console.log("\n=== WS4-R6: rebase shrinks a churned document, same content, new id ===");
{
  ...
  if (result.ok) {
    assert(result.value.bytesBefore >= 24_000 && result.value.bytesAfter < 7_000, `${churnedBytes} bytes -> ${result.value.bytesAfter} bytes (under 7KB)`);
    assert(result.value.entry.docId !== "churned", "under a new document id");
    assert(!result.value.entry.sessionRoom && !result.value.entry.sessionKey, "with no session identity");
    ...
    assert(
      JSON.stringify({ ...after, meta: undefined, root: undefined }) === JSON.stringify({ ...before, meta: undefined, root: undefined }),
...
console.log("\n=== WS4-R8: rebase is blocked inside the reconciliation window ===");
{
  ...
  const shared = await store.writeDocument("shared-doc", makeFile("Shared"), { origin: "session", sessionRoom: "r9", sessionKey: "k" });
  ...
  assert(later.ok && later.value.lastSessionAt === shared.value.lastSessionAt, "and a later local save keeps that time");
  const sharedAt = Date.parse(shared.value.lastSessionAt ?? "");
  const blocked = library.rebaseEligibility(shared.value, sharedAt + 5 * DAY);
  assert(!blocked.allowed && /30 days/.test(blocked.reason ?? ""), `blocked five days later, naming the window: "${blocked.reason}"`);
  assert(library.rebaseEligibility(shared.value, sharedAt + 31 * DAY).allowed, "allowed once the window has passed");
  const legacy = { ...shared.value, lastSessionAt: undefined };
  assert(!library.rebaseEligibility(legacy, Date.parse(legacy.updatedAt) + DAY).allowed, "a shared document without the time falls back to its last save");
  ...
  assert(resolveReconciliationWindowDays("90") === 90 && resolveReconciliationWindowDays("soon") === 30, "configurable, with nonsense ignored");
}
