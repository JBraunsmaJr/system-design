...
export function createYjsMilestonesStore(doc: Y.Doc): MilestonesStore {
  ...
  function milestoneMapToPlain(id: string, m: Y.Map<unknown>): Milestone {
    const relArr = (m.get('relatedItemIds') ?? m.get('relatedWorkableItemIds')) as
  ...
  const store: MilestonesStore = {
    ...
    addMilestone: (milestone) => {
      ...
      const sanitizedIds = sanitizeRelatedItemIds(
      ...
      doc.transact(() => {
        ...
        if (patch.relatedItemIds !== undefined || patch.relatedWorkableItemIds !== undefined) {
          const sanitized = sanitizeRelatedItemIds(
            patch.relatedItemIds ?? patch.relatedWorkableItemIds,
          );
          let relArr = (mM.get('relatedItemIds') ?? mM.get('relatedWorkableItemIds')) as
            Y.Array<string> | undefined;
          if (!relArr) {
    ...
    removeRelatedItem: (milestoneId, itemId) => {
      ...
      doc.transact(() => {
        const relArr = (mM.get('relatedItemIds') ?? mM.get('relatedWorkableItemIds')) as
          Y.Array<string> | undefined;
        if (!relArr) return;
