...
import { attachPersistence, createNullPersistence, persistenceKeyForRoom } from './persistence.ts';
...
const root = {
  ...
  edges: [{ id: 'e1', source: 'n1', target: 'n2', type: 'typed', data: {} }],
...
console.log('=== A document survives closing and reopening ===');
{
  ...
  assert(snapshot.nodes.length === 2, 'reopening restores the document from disk, not from a peer');
  assert(
...
console.log('=== Edits made in one session are there in the next ===');
{
  ...
  assert(labels.includes('Added later'), 'an edit made after seeding survives the round trip');
  await second.persistence.destroy();
}
...
console.log('=== Leaving is not forgetting (WS2-R6) ===');
{
  ...
  assert(isYjsDocEmpty(third.doc), 'forget() erases it - a separate, deliberate action');
...
console.log('=== Reading the seed decision too early is an error ===');
{
  ...
  assert(threw, 'wasEmptyOnLoad() before whenSynced throws rather than returning a guess');
  await persistence.whenSynced;
