      ...
      nodes: snap.nodes
        .map((n) => {
          ...
          return {
            type: n.type,
            position: n.position,
            data: { ...n.data, parentPath: parentPath?.length ?? 0 },
          };
        })
...
// === Part 2: the actual point - flattening correctly reconstructs each tree level ===
{
  ...
  const grandchild = store.addNode(
    [rootA, childOfA1],
    'typed',
    { x: 0, y: 0 },
    mkNodeData('Deeply Nested Job'),
  );
  ...
  assert(
    atRoot.length === 2 && atRoot.every((n) => [rootA, rootB].includes(n.id)),
    'getNodesAtPath([]) returns exactly the two root-level nodes, none of the nested ones',
  );

  const atRootA = getNodesAtPath(nodes, [rootA]);
  assert(
    atRootA.length === 2 && atRootA.every((n) => [childOfA1, childOfA2].includes(n.id)),
    "getNodesAtPath([rootA]) returns exactly the two nodes one level inside rootA's sub-diagram, not the root-level nodes or the grandchild two levels down",
  );

  const atChildOfA1 = getNodesAtPath(nodes, [rootA, childOfA1]);
  assert(
    atChildOfA1.length === 1 && atChildOfA1[0].id === grandchild,
    'getNodesAtPath([rootA, childOfA1]) correctly reconstructs the THIRD level of nesting - just the one grandchild node, proving the flattened model handles more than one level of depth, not just root+1',
  );

  const rootEdges = getEdgesAtPath(edges, []);
  assert(
    rootEdges.length === 1 && rootEdges[0].source === rootA && rootEdges[0].target === rootB,
    'getEdgesAtPath([]) returns exactly the one root-level edge',
  );

  const nestedEdges = getEdgesAtPath(edges, [rootA]);
  assert(
    nestedEdges.length === 1 && nestedEdges[0].source === childOfA1,
    'getEdgesAtPath([rootA]) returns exactly the one edge that belongs one level inside rootA',
  );
}
...
// === Part 3a: concurrent field edits on the SAME node - position vs label. ===
{
  ...
  assert(
    nodeA.position.x === 500 && nodeA.data.label === 'Renamed by B',
    "peer A's merged view has BOTH concurrent edits - A's position change and B's label change both survived",
  );
  assert(
    nodeB.position.x === 500 && nodeB.data.label === 'Renamed by B',
...
// === Part 3b: concurrent creation at DIFFERENT tree levels by different peers, both correctly filtered afterward. ===
{
  ...
  const nestedFromB = peerB.store.addNode(
    [rootId],
    'typed',
    { x: 2, y: 2 },
    mkNodeData('Nested By B'),
  );
  ...
  assert(
    getNodesAtPath(allNodesA, []).length === 2,
    'peer A sees both root-level nodes (original + the one A itself added) after sync',
  );
  assert(
    getNodesAtPath(allNodesA, [rootId]).length === 1 &&
      getNodesAtPath(allNodesA, [rootId])[0].id === nestedFromB,
    'peer A ALSO sees the node B concurrently created one level deeper, correctly filtered to that level and not mixed in with the root level',
  );
  const allNodesB = peerB.store.getSnapshot().nodes;
  assert(
    JSON.stringify(allNodesA.map((n) => n.id).sort()) ===
      JSON.stringify(allNodesB.map((n) => n.id).sort()),
    'both peers converge to the identical set of nodes across both levels',
  );
}
...
// === Part 3c: delete cascade across MULTIPLE levels of nesting - the core new behavior the flattened model needs that the old nested-object model got for free. ===
{
  ...
  const level3 = store.addNode(
    [rootId, level1, level2],
    'typed',
    { x: 0, y: 0 },
    mkNodeData('Level 3'),
  );
  ...
  const { nodes, edges } = store.getSnapshot();
  assert(
    nodes.length === 1 && nodes[0].id === otherRootId,
    'deleting the root of a three-level-deep sub-diagram tree removes the root AND every descendant at every depth (level1, level2, level3) - only the genuinely unrelated sibling node survives',
  );
  assert(
    edges.length === 0,
    'the deeply-nested edge (between level2 and level3, three levels down) is also removed as part of the cascade, since both endpoints were deleted',
  );
}
...
// === Part 3d: an edge untouched by a deletion elsewhere correctly survives ===
{
  ...
  const { nodes, edges } = store.getSnapshot();
  assert(
    nodes.length === 2 && nodes.every((n) => [siblingA, siblingB].includes(n.id)),
    'deleting root removes root and its nested level1 child, leaving the two genuinely unrelated siblings',
  );
  assert(
    edges.length === 1 && edges[0].source === siblingA && edges[0].target === siblingB,
    "an edge that doesn't touch the deleted node or any of its descendants correctly survives the cascade",
  );
}
...
// === Part 3e: delete-vs-edit race on a node. ===
{
  ...
  assert(
    storeA.getSnapshot().nodes.find((n) => n.id === nodeId) === undefined,
    "the delete wins on peer A - not resurrected by B's concurrent edit",
  );
  assert(
    peerB.store.getSnapshot().nodes.find((n) => n.id === nodeId) === undefined,
...
// === Part 4: hasSubDiagram - correctly matches the real app's own definition (at least one child), not a separate "opened" flag ===
{
  ...
  assert(
    !hasSubDiagram(nodes, [], emptyNode),
    'a node with no children at all correctly reports no sub-diagram',
  );
  assert(
...
// === Part 5: sourceHandle/targetHandle are preserved - real, meaningful data for nodes with multiple named handles (discovered during actual UI wiring, not something addEdge can afford to silently drop) ===
{
  ...
  assert(
    localEdge.sourceHandle === 'source-right' && localEdge.targetHandle === 'target-left',
    'the local store correctly preserves which specific named handle a connection was made from/to',
  );
  assert(
    yjsEdge.sourceHandle === 'source-right' && yjsEdge.targetHandle === 'target-left',
    'the Yjs store correctly preserves the same, round-tripping through real yjs',
  );
  ...
  assert(
    plainEdge.sourceHandle === undefined && plainEdge.targetHandle === undefined,
    'an edge created without explicit handles has no sourceHandle/targetHandle forced onto it',
  );
}
...
// === Part 6: flattenSubDiagramTree / unflattenToSubDiagram round-trip - what leaveSession relies on to convert a session's final flat state back into the app's own recursive tree without losing anything ===
{
            ...
            nodes: [
              {
                ...
                type: 'typed',
                position: { x: 0, y: 0 },
                data: mkNodeData('Child One'),
              },
              {
                id: 'child-2',
                type: 'typed',
                position: { x: 1, y: 1 },
                data: mkNodeData('Child Two'),
              },
            ],
            edges: [
              {
  ...
  assert(
    JSON.stringify((flatChild.data as ArchNodeData & { parentPath?: string[] }).parentPath) ===
...
// directly instead.
{
  ...
  assert(
    !('color' in (yjsSnap.edges[0].data ?? {})) &&
      !('labelAnchorT' in (yjsSnap.edges[0].data ?? {})),
