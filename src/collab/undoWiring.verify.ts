...
import { seedYjsDocumentMeta, createYjsDocumentMetaStore, DEFAULT_DOCUMENT_TITLE } from "./yjsDocumentMetaStore.ts";
...
import { BUILT_IN_ITEM_TYPES, BUILT_IN_RELATIONSHIP_TYPES } from "../domain/requirementsRegistry.ts";
...
function makeFile(title: string): DiagramFile {
  return {
    ...
    nodes: [
      {
        ...
        data: {
          ...
          label: "Gateway",
          subDiagram: {
            nodes: [{ id: "n1-child", type: "typed", position: { x: 5, y: 5 }, data: { nodeType: "service", label: "Inner" } }],
            edges: [],
          },
        },
      },
    ...
    edges: [],
    scenarios: [{ id: "sc-file", title: "From file", steps: [{ id: "st", title: "s", nodeIds: [], edgeIds: [] }] }],
    requirements: {
...
console.log("=== Every mutating method of every store is wrapped ===");
{
  ...
  for (const [name, store] of Object.entries(raw)) {
    ...
    const entries = Object.entries(store as Record<string, unknown>).filter(([, v]) => typeof v === "function");
...
{
  const opened = await openDocument({ docId: "undo-domains", initial: makeFile("Start"), persist: false });
  ...
  const cases: { name: string; read: () => unknown; edit: () => void }[] = [
    { name: "diagram", read: () => s.diagram.getSnapshot().nodes.length, edit: () => void s.diagram.addNode([], "typed", { x: 9, y: 9 }, { nodeType: "service", label: "New" } as never) },
    { name: "requirements", read: () => s.requirements.getSnapshot().items.length, edit: () => void s.requirements.addItem(BUILT_IN_ITEM_TYPES[0].id) },
    { name: "programIncrements", read: () => s.programIncrements.getSnapshot().length, edit: () => void s.programIncrements.addPI() },
    { name: "team", read: () => s.team.getSnapshot().members.length, edit: () => s.team.addMember({ id: "m1", name: "Ada", ptoSpans: [] }) },
    { name: "milestones", read: () => s.milestones.getSnapshot().length, edit: () => void s.milestones.addMilestone({ type: "release", name: "GA", scheduledAt: "2026-10-01" } as never) },
    { name: "meta title", read: () => s.meta.getSnapshot().title, edit: () => s.meta.setTitle("Renamed") },
    { name: "meta scenarios", read: () => s.meta.getSnapshot().scenarios.map((sc) => sc.id), edit: () => s.meta.setScenarios([SCENARIO]) },
  ];
  for (const c of cases) {
    ...
    undo.undo();
    assert(before !== after && undone === before && redone === after, `${c.name}: ${before} -> ${after}, undo restores, redo reapplies`);
  }
  await opened.close();
}
...
console.log("\n=== Edits outside the wrapper are not undoable (pinned) ===");
{
  ...
  assert(!undo.canUndo(), "raw store edits leave history empty - which is how seeding stays out of it");
  destroyDocumentStores(raw);
}
...
console.log("\n=== Document boundaries stay out of history (WS3-R4) ===");
{
  ...
  assert(reopened.stores.meta.getSnapshot().title === "Edited before close", "title persists with the document");
  ...
  assert(
    JSON.stringify(reopened.stores.meta.getSnapshot().scenarios.map((sc) => sc.id)) === JSON.stringify(["sc-file"]),
...
console.log("\n=== A drag collapses into one undo entry at the default timeout (WS3-R3) ===");
{
  ...
  const id = s.diagram.addNode([], "typed", { x: 0, y: 0 }, { nodeType: "service", label: "Drag me" } as never);
  undo.clear();
...
console.log("\n=== Two peers: undo reverts only the local user's edits (WS3-R1, WS3-R2) ===");
{
  ...
  const aNode = A.diagram.addNode([], "typed", { x: 0, y: 0 }, { nodeType: "service", label: "A" } as never);
  const bNode = B.diagram.addNode([], "typed", { x: 1, y: 1 }, { nodeType: "service", label: "B" } as never);
  B.meta.setScenarios([SCENARIO]);
  assert(rawA.meta.getSnapshot().scenarios.length === 1 && rawB.meta.getSnapshot().title === "A's title", "title and scenarios sync between peers (WS1-R7)");

  undoA.undo();
  undoA.undo();
  const ids = (s: OpenDocumentStores) => s.diagram.getSnapshot().nodes.map((n) => n.id);
  assert(!ids(rawA).includes(aNode) && !ids(rawB).includes(aNode), "A's node is gone for both peers");
  assert(rawB.meta.getSnapshot().title === DEFAULT_DOCUMENT_TITLE, "A's rename is reverted for both peers");
  assert(ids(rawA).includes(bNode) && rawA.meta.getSnapshot().scenarios.length === 1, "B's node and scenarios are untouched");
...
console.log("\n=== Meta seeding ===");
{
  ...
  assert(back.stores.meta.getSnapshot().title === "From autosave", "but it recovers its title from the autosave");
  await back.close();
}
...
console.log("\n=== Controller cache and rebase ===");
{
  ...
  assert(undoControllerFor(doc) === c1, "one controller per document, however often it is requested");
