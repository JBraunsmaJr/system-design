              ...
              nodes: [
                {
                  ...
                  type: 'typed',
                  position: { x: 0, y: 0 },
                  data: { nodeType: 'service', label: 'Child' },
                },
              ],
              edges: [],
            },
...
step('move one node', () => store.updatePosition('n5', { x: 999, y: 0 }), {
  nodes: ['n5'],
  edges: [],
});
step("edit one node's data", () => store.updateNode('n7', { label: 'Renamed' }), {
  nodes: ['n7'],
  edges: [],
});
step('resize one node', () => store.updateDimensions('n8', 120, 40), { nodes: ['n8'], edges: [] });
step('move one waypoint', () => store.moveEdgeWaypoint('e3', 'wp', { x: 5, y: 5 }), {
  nodes: [],
  edges: ['e3'],
});
step('add a waypoint', () => store.addEdgeWaypoint('e4', 0, { id: 'wp2', x: 2, y: 2 }), {
  nodes: [],
  edges: ['e4'],
});
step('edit one edge', () => store.updateEdge('e9', { label: 'calls' }), {
  nodes: [],
  edges: ['e9'],
});
step('reconnect one edge', () => store.reconnectEdge('e10', { source: 'n10', target: 'n20' }), {
  nodes: [],
  edges: ['e10'],
});
...
{
  const before = store.getSnapshot();
  const added = store.addNode([], 'typed', { x: 0, y: 0 }, {
    nodeType: 'service',
    label: 'New',
  } as never);
  const got = changedIds(before, store.getSnapshot());
  assert(
    JSON.stringify(got.nodes) === JSON.stringify([added]) && got.edges.length === 0,
    'adding a node leaves every existing object untouched',
  );
}
{
  const before = store.getSnapshot();
  ...
  assert(
    !ids.includes('n0') && !ids.includes('child') && !after.edges.some((e) => e.id === 'e0'),
    'a cascading delete removes the node, its descendants and its edges',
  );
  const got = changedIds(before, after);
...
console.log('\n=== Remote updates rebuild only what they touch ===');
{
  ...
  const got = changedIds(before, store.getSnapshot());
  assert(
    JSON.stringify(got.nodes) === JSON.stringify(['n12']) &&
      JSON.stringify(got.edges) === JSON.stringify(['e15']),
    `a remote update rebuilds only n12 and e15 (got ${JSON.stringify(got)})`,
  );
  assert(
    notifications === 1,
    `one notification for the whole remote update (got ${notifications})`,
  );
  assert(
    store.getSnapshot().nodes.find((n) => n.id === 'n12')?.position.x === 1,
    'and carries the remote value',
  );
  peerStore.destroy();
}
...
console.log('\n=== Whole-document writes and teardown ===');
{
  ...
  const after = store.getSnapshot();
  assert(
    after.nodes.length === 31 && after.edges.length === 20,
    'replaceAll installs the full diagram again',
  );
  assert(
    after.nodes.every((n) => !before.nodes.includes(n)),
    'and every entry is rebuilt, since every shared type is new',
  );
  store.destroy();
  notifications = 0;
  const frozen = store.getSnapshot();
  doc.transact(() =>
    doc.getMap<Y.Map<unknown>>('nodes').get('n1')?.set('position', { x: -1, y: -1 }),
  );
  assert(
    notifications === 0 && store.getSnapshot() === frozen,
    'a destroyed store neither rebuilds nor notifies',
  );
}
