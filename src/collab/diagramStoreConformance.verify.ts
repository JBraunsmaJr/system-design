...
// === Part 2: flattening correctly reconstructs each tree level (same guarantee as the Yjs store, now for the adapter) ===
{
  ...
  assert(
    ...
    'getNodesAtPath([]) returns exactly the two root-level nodes',
  );

  const atRootA = getNodesAtPath(nodes, [rootA]);
  assert(
    atRootA.length === 2 && atRootA.every((n) => [childOfA1, childOfA2].includes(n.id)),
    'getNodesAtPath([rootA]) returns exactly the two nodes one level inside rootA',
  );

  const atChildOfA1 = getNodesAtPath(nodes, [rootA, childOfA1]);
  assert(
    atChildOfA1.length === 1 && atChildOfA1[0].id === grandchild,
    'getNodesAtPath([rootA, childOfA1]) correctly reconstructs the THIRD level of nesting through the real recursive tree, not just root+1',
  );
...
// === Part 3: updating a deeply-nested node finds and patches the right level of the real tree ===
{
  ...
  assert(
    updated.data.description === 'Updated at depth 2',
    'updateNode correctly finds and patches a node two levels deep in the real tree',
  );
  assert(
    updated.position.x === 999,
    'updatePosition correctly finds and patches a node two levels deep',
  );
  ...
  assert(
    actualLevel2.data.description === 'Updated at depth 2',
    "the update is reflected in the ACTUAL nested subDiagram structure, confirming this isn't just patching a flat snapshot that gets discarded",
  );
}
...
// === Part 4: delete cascade across multiple levels - the tree gets this for free by construction, verified it actually does ===
{
  ...
  assert(
    ...
    'deleting the root of a two-level-deep sub-diagram removes the root and every descendant (level1, level2), leaving only the genuinely unrelated sibling',
  );
  assert(
    edges.length === 0,
    'the edge nested inside the deleted subtree is gone too, since removing the node from its own level takes its whole nested subDiagram (edges included) along with it automatically',
  );
  void level2;
}
...
// === Part 5: an edge untouched by a deletion elsewhere correctly survives ===
{
  ...
  assert(
    ...
    'deleting root removes root and its nested child, leaving the two genuinely unrelated siblings',
  );
  assert(
    edges.length === 1 && edges[0].source === siblingA && edges[0].target === siblingB,
    "an edge that doesn't touch the deleted node or any of its descendants correctly survives",
  );
}
...
// === Part 6: hasSubDiagram against the tree-backed adapter - correctly matches the real app's own definition (at least one child) ===
{
  ...
  assert(
    ...
    'a node with no children at all correctly reports no sub-diagram, even via the tree-backed adapter',
  );
  assert(
    hasSubDiagram(nodes, [], populatedNode),
    "a node with at least one child correctly reports having a sub-diagram, matching the real app's own findLinkedNodes definition - not a separate flag that could drift out of sync with the actual tree",
  );
}
...
// === Part 7: group containment (parentId) and dimensions are independent of tree-level nesting ===
{
  ...
  assert(
    child.parentId === groupId && child.position.x === 5,
    'updateParentId correctly sets both parentId and the new (relative) position',
  );
  assert(
    group.width === 400 && group.height === 300,
    'updateDimensions correctly sets width/height',
  );
  assert(
...
// === Part 8: getSnapshot satisfies useSyncExternalStore's own contract - the exact bug a real user hit (infinite render loop, nothing rendering at all) when this didn't hold ===
{
  ...
  assert(
    snapshotC !== snapshotA,
    'after a genuine mutation (addNode), getSnapshot() correctly returns a NEW reference - the cache must invalidate on real changes, not just always return the same frozen object',
  );

  const snapshotD = store.getSnapshot();
  assert(
    snapshotC === snapshotD,
    'immediately after that mutation, calling getSnapshot() again (with nothing further changed) returns the SAME reference as the previous call - the cache re-stabilizes correctly after each change, not just once at the very start',
  );
  ...
  assert(
    allInvalidatedCorrectly,
    'every mutation type (updateNode, updatePosition, updateDimensions, updateParentId, deleteNode) correctly invalidates the cache exactly once - a new reference right after the change, then stable again until the next one',
  );
}
