...
const entry = (docId: string, updatedAt: string, wrappedDocKey = 'AA=='): IndexEntry => ({
  docId,
  wrappedDocKey,
  title: docId,
  updatedAt,
...
console.log('=== Document keys ===');
{
  ...
  assert(
    /^[0-9a-f]{32}$/.test(documentKey),
    'a new document key is 128 bits of hex, as a share link carries',
  );
  ...
  assert(
    recovered === documentKey,
    'unwrapping recovers the document key itself, so the document stays shareable by link',
  );

  const other = await generateWorkspaceKey();
  await rejects(
    () => documentKeyFor({ wrappedDocKey }, other),
    "another workspace's key cannot recover it",
  );
  ...
  const sealed = await crypto.seal(
    context,
    new TextEncoder().encode('a diagram'),
    await deriveStorageKey(documentKey),
  );
  const opened = await crypto.open(context, sealed, await deriveStorageKey(recovered));
  assert(
    new TextDecoder().decode(opened) === 'a diagram',
    'and a document sealed with it opens with the recovered key',
  );
}
...
console.log('\n=== The index key ===');
{
  ...
  assert(
    (await exportSymmetricKeyHex(first)) === (await exportSymmetricKeyHex(second)),
    'the same workspace key always derives the same index key',
  );
  assert(
    (await exportSymmetricKeyHex(first)) !==
      (await exportSymmetricKeyHex(await indexKeyFor(await generateWorkspaceKey()))),
    'and a different workspace key derives a different one',
  );
  assert(
    (await exportSymmetricKeyHex(first)) !== (await exportSymmetricKeyHex(workspaceKey)),
    'the index key is not the workspace key itself',
  );
}
...
console.log('\n=== The list ===');
{
  ...
  assert(
    updated.length === 2 && updated[0].docId === 'a',
    're-saving a document moves it to the top rather than duplicating it',
  );
  const added = upsertEntry(entries, entry('c', '2026-09-02T00:00:00Z'));
  assert(
    added.length === 3 && added.map((e) => e.docId).join() === 'b,c,a',
    'a new document is listed in recency order',
  );
  assert(removeEntry(added, 'c').length === 2, 'removing takes exactly one entry');
  assert(
    removeEntry(added, 'missing').length === 3,
    'and removing something absent changes nothing',
  );
}
