...
// === Part 1: conformance ===
{
  ...
  assert(
    localResult.id1 === yjsResult.id1,
    `local and Yjs stores generate the identical first item id ("${localResult.id1}" vs "${yjsResult.id1}") from the same starting state`,
  );
  assert(
    localResult.id1 === adapterResult.id1,
    `the adapter store generates the identical first item id too ("${adapterResult.id1}")`,
  );
  assert(
    localResult.relError === null && yjsResult.relError === null,
    'the relationship was added without error in both stores (added before the target was deleted)',
  );
  assert(
    localResult.refusedDelete === false &&
      yjsResult.refusedDelete === false &&
      adapterResult.refusedDelete === false,
    'all three stores refuse to delete a type that still has an item, and report the refusal identically',
  );
  assert(
    localResult.allowedDelete === true &&
      yjsResult.allowedDelete === true &&
      adapterResult.allowedDelete === true,
...
// 2a. Two peers concurrently edit DIFFERENT FIELDS of the SAME item - the core guarantee, same as team's.
{
  ...
  assert(
    itemA.title === 'Changed by A' && itemA.body === 'Changed by B',
    "peer A's merged view has BOTH concurrent field edits on the same item - A's title change and B's body change both survived",
  );
  assert(
    itemB.title === 'Changed by A' && itemB.body === 'Changed by B',
...
// 2b. Concurrent add of DIFFERENT items (different types even) converges cleanly.
{
  ...
  const idsA = peerA.store
    .getSnapshot()
    .items.map((i) => i.id)
    .sort();
  const idsB = peerB.store
    .getSnapshot()
    .items.map((i) => i.id)
    .sort();
  assert(
    idsA.includes(idFromA) && idsA.includes(idFromB),
    'peer A sees both concurrently-created items after sync',
  );
  assert(
    JSON.stringify(idsA) === JSON.stringify(idsB),
    'both peers converge to the identical item list',
  );
}
...
// 2c. Delete-vs-edit race on an item, same shape as team's member race.
{
  ...
  assert(
    peerA.store.getSnapshot().items.find((i) => i.id === itemId) === undefined,
    "the delete wins on peer A - not resurrected by B's concurrent edit",
  );
  assert(
    peerB.store.getSnapshot().items.find((i) => i.id === itemId) === undefined,
...
// 2d. Concurrent relationship additions between different item pairs merge cleanly.
{
  ...
  assert(
    peerA.store.getSnapshot().relationships.length === 2,
    'both concurrently-added relationships survive the merge on peer A',
  );
  assert(
    peerB.store.getSnapshot().relationships.length === 2,
    'both peers converge to the same two relationships',
  );
}
...
// === Part 3: the id-collision FIX - both items survive with distinct ids, zero data loss ===
{
  ...
  assert(
    idFromA === idFromB,
    `both disconnected peers still independently compute the same candidate display id ("${idFromA}") - this part of the scenario is unavoidable and unchanged; what matters is what happens next`,
  );
  ...
  assert(
    itemsA.length === 2,
    "BOTH items survive after sync - not one silently discarded, not a duplicate row showing one item's content twice",
  );
  assert(
    JSON.stringify(titlesA) === JSON.stringify(['Written by peer A', 'Written by peer B']),
    "BOTH peers' actual data (their distinct titles) survived intact - the fix separates the collision-prone display id from the collision-proof internal storage key each item actually lives under",
  );
  assert(
    new Set(itemsA.map((i) => i.id)).size === 2,
    'the two surviving items now have DIFFERENT display ids - the collision was automatically, deterministically repaired rather than left in place',
  );
  assert(
    JSON.stringify(
      itemsA.map((i) => ({ id: i.id, title: i.title })).sort((a, b) => a.id.localeCompare(b.id)),
    ) ===
      JSON.stringify(
        itemsB.map((i) => ({ id: i.id, title: i.title })).sort((a, b) => a.id.localeCompare(b.id)),
      ),
...
// === Part 4: THREE-way collision - verifies the repair generalizes beyond just two colliding peers ===
{
  ...
  assert(
    idFromA === idFromB && idFromB === idFromC,
    'all three disconnected peers independently compute the same candidate display id',
  );
  ...
  assert(
    itemsA.length === 3,
    'all THREE concurrently-created items survive a three-way collision, not just two',
  );
  assert(
    new Set(itemsA.map((i) => i.id)).size === 3,
    'all three end up with distinct display ids after repair',
  );
  assert(
    JSON.stringify(itemsA.map((i) => i.title).sort()) === JSON.stringify(['A', 'B', 'C']),
    "all three peers' distinct titles survived - no data loss even with three-way contention",
  );
  assert(
    JSON.stringify(
      itemsA.map((i) => ({ id: i.id, title: i.title })).sort((a, b) => a.id.localeCompare(b.id)),
      ...
      JSON.stringify(
        itemsC.map((i) => ({ id: i.id, title: i.title })).sort((a, b) => a.id.localeCompare(b.id)),
      ),
...
// between the modal rendering and the click landing.
{
  ...
  assert(
    blocked.itemTypes.some((t) => t.id === 'custom-1'),
    'the refused type is still present - the refusal is a genuine no-op, not a partial delete',
  );
  assert(
    blocked.items.some((i) => i.id === idA),
    'the item using the type is untouched - this is the data loss the old cascade caused',
  );
  assert(
    blocked.relationships.length === 1,
    'the relationship touching that item is untouched too',
  );
  ...
  assert(
    after.itemTypes.every((t) => t.id !== 'custom-1'),
    'the now-unused type is actually removed',
  );
  assert(
    after.items.some((i) => i.id === idB),
    'the unrelated item of a different type is left alone',
  );
}
...
// as a dangling reference rather than as uncategorized.
{
  ...
  assert(
    snap.categories.every((c) => c.id !== authId),
    'the deleted category is gone from the document',
  );
  assert(
    snap.items.find((i) => i.id === idA)!.categoryId === undefined,
    'the first item that used it is now uncategorized, not left pointing at a category that no longer exists',
  );
  assert(
    snap.items.find((i) => i.id === idB)!.categoryId === undefined,
    'every item that used it is cleared, not just the first one found',
  );
  assert(
    snap.items.find((i) => i.id === idC)!.categoryId !== undefined,
    'an item in a DIFFERENT category keeps its own assignment',
  );
  assert(
    snap.items.length === 3,
    'no items are removed - deleting a category only ever drops the label',
  );
}
...
// === Part 6: seedYjsRequirementsDoc - starting a session must preserve existing work exactly, not lose or mangle it ===
{
  ...
  assert(
    seededItemA.categoryId === existingSnapshot.items.find((i) => i.id === itemA)!.categoryId,
    "the seeded item's categoryId still correctly references the same (preserved) category id",
  );
  assert(
    seededSnapshot.relationships.some((r) => r.fromItemId === itemA && r.toItemId === itemB),
    'the seeded relationship still correctly references both original item ids',
  );
  assert(
    seededSnapshot.items.some((i) => i.id === itemC && i.typeId === 'custom-1'),
    "the item created under the custom type still correctly references the custom type's preserved id",
  );
  ...
  assert(
    builtInCount === originalBuiltInCount,
    "built-in types appear exactly once each after seeding - not duplicated by createYjsRequirementsStore's own empty-doc default-seeding logic running on top of already-seeded data",
  );
}
...
// all) - then syncs them, exactly as WebRTC would.
{
  ...
  assert(
    guestSnapshotBeforeSync.itemTypes.length === 0,
    "before any sync happens, the guest's doc is genuinely empty - no locally-seeded built-ins sitting there waiting to conflict with the host's",
  );
  ...
  const builtInTypeIds = guestSnapshotAfterSync.itemTypes
  ...
  assert(
    canonicalJSON({
      itemTypes: hostSnapshotAfterSync.itemTypes,
      items: hostSnapshotAfterSync.items,
    }) ===
      canonicalJSON({
        itemTypes: guestSnapshotAfterSync.itemTypes,
        items: guestSnapshotAfterSync.items,
      }),
...
// stable moment to actually paint anything.
{
  ...
  assert(
    repairedItem !== undefined &&
      !['REQ-1', 'REQ-2', 'REQ-3', 'REQ-4', 'REQ-5'].includes(repairedItem.id),
...
// post-change path instead.
{
  ...
  const danglingA = snapA.items.filter(
    (i) => i.categoryId && !snapA.categories.some((c) => c.id === i.categoryId),
  );

  assert(
    danglingA.length === 0,
    'after a concurrent assign-vs-delete merge, no item is left pointing at a category that no longer exists',
  );
  assert(
    snapA.items.length === 2,
    'and both items survive - only the label was dropped, never the item',
  );
  assert(
    JSON.stringify(snapA.items.map((i) => i.categoryId)) ===
      JSON.stringify(snapB.items.map((i) => i.categoryId)),
...
// appears in none of them.
{
  ...
  assert(
    deleted === true,
    "the deleting peer's in-use guard passes, because the item using the type hasn't reached it yet",
  );
  ...
  assert(
    orphans.length === 0,
    "after the merge no item is left with a type that doesn't exist - which would have made it invisible in the default group-by-type view",
  );
  assert(
    snapA.items.some((i) => i.id === itemId),
    `the concurrently-created item still exists, keeping its original display id (${itemId})`,
  );

  const restored = snapA.itemTypes.find((t) => t.id === 'custom-1');
  assert(
    restored !== undefined,
    'the type is restored rather than the item being reassigned to some fallback',
  );
  assert(
    restored?.label === 'Widget',
    'restored from the tombstone deleteCustomType recorded, so the real label survives rather than being reconstructed from the display-id prefix',
  );
  ...
  assert(
    restored?.isWorkable === true,
    'and isWorkable - the field that decides whether these items can go into a sprint at all, and the one a prefix-derived guess would get wrong',
  );
  assert(
    JSON.stringify(snapA.itemTypes) === JSON.stringify(snapB.itemTypes) &&
      JSON.stringify(snapA.items) === JSON.stringify(snapB.items),
...
// it only fires when something still references them.
{
  ...
  const snap = store.getSnapshot();
  assert(
    snap.itemTypes.every((t) => t.id !== 'custom-1'),
    'and stays deleted - the repair does not resurrect a type nothing references',
  );
}
...
// reused. A stale tombstone under that id describes an unrelated type.
{
  ...
  assert(
    reused?.label === 'Gadget',
    "the reused id carries the NEW type's definition, not the tombstoned one",
  );

  const itemId = store.addItem('custom-1');
  const snap = store.getSnapshot();
  assert(
    snap.items.find((i) => i.id === itemId) !== undefined,
    'and items created under it behave normally',
  );
  assert(
    snap.itemTypes.filter((t) => t.id === 'custom-1').length === 1,
    'with no duplicate type left behind',
  );
}
...
// === Part 26: item type conversion, retroactive references, gap-filling sequence ===
{
  function verifyConversion(store: RequirementsStore, label: string) {
    ...
    assert(
      Boolean(newId1?.startsWith('EPIC-')),
      `[${label}] single item conversion updates ID to prefix of target type`,
    );
    assert(
      convertedItem1?.typeId === 'epic',
      `[${label}] single item conversion updates typeId to epic`,
    );
    assert(
      convertedItem1?.title === 'Custom Item 1',
      `[${label}] single item conversion preserves item title and data`,
    );

    // Deleting custom-1 is still refused because item2 is using it
    assert(
      store.deleteCustomType('custom-1') === false,
      `[${label}] delete custom type refused when items remain`,
    );
    ...
    assert(
      Boolean(convertedItem2?.id.startsWith('DEP-')),
      `[${label}] batch converted item gets target type prefix DEP-`,
    );
    assert(convertedItem2?.typeId === 'dependency', `[${label}] item2 converted to dependency`);

    // Now delete custom-1 succeeds
    assert(
      store.deleteCustomType('custom-1') === true,
      `[${label}] delete custom type succeeds after all items transferred`,
    );
    ...
    assert(
      epicTicket?.typeId === 'epic' && epicTicket?.status === undefined,
      `[${label}] converting workable item to non-workable clears status/points`,
    );
    ...
    assert(
      restoredTicket?.typeId === 'ticket' && restoredTicket?.status === 'todo',
      `[${label}] converting non-workable item to workable sets default status to todo`,
    );
  }
...
// === Part 27: Retroactive reference updating and gap-filling IDs ===
{
  function verifyReferencesAndGapFilling(store: RequirementsStore, label: string) {
    ...
    assert(
      updatedRefItem?.body ===
        'This depends on #DEP-1 and has a link [#DEP-1](#ref:DEP-1) inside text.',
    ...
    assert(
      rel?.toItemId === 'DEP-1',
      `[${label}] relationship target retroactively updated from ${req2} to DEP-1`,
    );
