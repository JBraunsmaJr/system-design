...
console.log("=== Undo never touches a peer's work (WS3-R2) ===");
{
  ...
  assert(
    !labelsOf(alice).includes("From Alice"),
    "Alice's undo removes Alice's node",
  );
  assert(
  ...
  assert(
    !bobUndo.canRedo(),
    "Bob has nothing to redo - Alice's undo did not enter his stack",
  );

  bobUndo.undo();
  assert(
    !labelsOf(alice).includes("From Bob"),
    "Bob undoing his own edit removes it everywhere",
  );
...
console.log("=== Remote edits do not become undoable locally ===");
{
  ...
  assert(
    labelsOf(alice).includes("Bob only"),
    "Alice receives the edit",
  );
  assert(
    !aliceUndo.canUndo(),
    "but it does not appear in Alice's undo stack - she cannot undo work she " +
      "did not do",
  );
  aliceUndo.destroy();
  bobUndo.destroy();
...
console.log("=== Bursts collapse into one entry (WS3-R3) ===");
{
  ...
  assert(
    label === "Gateway",
    `a 25-step burst reverses in one press (got "${label}")`,
  );
...
console.log("=== History clears at a document boundary (WS3-R4) ===");
{
  ...
  assert(
    labelsOf(doc).includes("Before"),
    "clearing history does not revert the document",
  );
  undo.destroy();
}
...
console.log("=== Availability changes are published ===");
{
  ...
  undo.redo();
  assert(
    notifications === afterUnsubscribe,
