...
export function useWorkspace(options: WorkspaceOptions = {}): WorkspaceApi {
  ...
  const describe = (failure: unknown): string => {
    if (failure instanceof StoreClientError) {
      ...
      if (failure.reason === 'rolled-back')
        return 'The workspace returned an older version than this browser has already seen. Nothing was applied.';
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const who =
        health.authentication === 'none'
          ? ({ issuer: 'none', subject: 'anonymous' } as SessionInfo)
      ...
      setStatus(
        failure instanceof StoreClientError && failure.reason === 'offline'
          ? 'offline'
          : 'signed-out',
      );
    ...
    async (docId: string, file: DiagramFile) => {
      ...
      if (!workspaceKey || !key)
        throw new StoreClientError(
          'This device cannot write to the workspace yet.',
          'unauthenticated',
        );
      setBusy(true);
      try {
        ...
        const documentKey = existing
          ? await documentKeyFor(existing, workspaceKey)
          : fresh!.documentKey;
        ...
        const next = await client.updateIndex(WORKSPACE_ID, key, (current) =>
  ...
  const open = useCallback(
    async (entry: IndexEntry): Promise<DiagramFile> => {
      if (!client || !workspaceKey)
        throw new StoreClientError('This device cannot read the workspace yet.', 'unauthenticated');
