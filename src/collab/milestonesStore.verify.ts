...
// === Part 1: Local Milestones Store CRUD ===
{
  ...
  assert(
    list[0].name === 'Release 1.0' && list[0].type === 'release',
  ...
  list = store.getSnapshot();
  assert(
    list[0].relatedWorkableItemIds?.length === 1 &&
      list[0].relatedWorkableItemIds[0] === 'TICKET-2',
...
{
  const store = createLocalMilestonesStore();
  const id1 = store.addMilestone({
    type: 'release',
    name: 'Release 2.4',
    scheduledAt: '2026-09-30',
  });
  const id2 = store.addMilestone({
    type: 'release',
    name: 'Release 2.4.1',
    scheduledAt: '2026-09-30',
  });
  const id3 = store.addMilestone({
    type: 'release',
    name: 'Mobile Release',
    scheduledAt: '2026-09-30',
  });
  ...
  assert(
    list.length === 3 && id1 !== id2 && id2 !== id3,
    'multiple releases on same date coexist with distinct ids',
  );
  assert(
...
// === Part 3: Yjs Collaborative Milestones Store ===
{
  ...
  assert(
    storeA.getSnapshot()[0].relatedWorkableItemIds?.[0] === 'TICKET-10',
    'seeded workable items preserved',
  );
  ...
  assert(
    Boolean(
      finalA.relatedWorkableItemIds?.includes('TICKET-20') &&
      finalB.relatedWorkableItemIds?.includes('TICKET-20'),
    ),
