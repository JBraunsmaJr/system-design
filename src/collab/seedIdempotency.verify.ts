  ...
  members: [
    {
      ...
      ptoSpans: [
        { id: "pto1", startDate: "2026-01-08", endDate: "2026-01-08" },
      ],
    },
  ],
  settings: {
...
console.log("=== Seeding twice produces the same document ===");
{
  ...
  assert(
    twice.milestones.length === milestones.length,
    "milestones are not duplicated",
  );
  assert(
    twice.increments.length === increments.length,
    "program increments are not duplicated",
  );
  assert(
  ...
  assert(
    twice.team.members[0].ptoSpans.length === 1,
    "PTO spans are not duplicated",
  );
  assert(
    twice.team.settings.extraDaysOff.length === 1,
    "extra days off are not duplicated",
  );
}
...
console.log("=== Ordering arrays hold no duplicate ids ===");
{
  ...
  for (const name of ["nodeOrder", "edgeOrder"]) {
    ...
    assert(
      ...
      `${name} holds each id exactly once`,
    );
  }
}
...
console.log("=== pushIfAbsent ===");
{
  ...
  assert(
    !pushIfAbsent(order, reloaded, "a"),
    "a set rebuilt from the array sees the existing id",
  );
}
