...
// 2a. Two peers concurrently add DIFFERENT members, with no sync in between.
{
  ...
  assert(
    JSON.stringify(idsA) === JSON.stringify(['alice', 'bob']),
    'peer A sees both members after sync',
  );
  assert(
    JSON.stringify(idsB) === JSON.stringify(['alice', 'bob']),
    'peer B sees both members after sync',
  );
  assert(
    JSON.stringify(idsA) === JSON.stringify(idsB),
    'both peers converge to the identical member order after merge',
  );
}
...
// 2b. THE critical guarantee: two peers concurrently edit DIFFERENT FIELDS of the SAME member.
{
  ...
  assert(
    memberA.name === 'Changed By A' && memberA.role === 'Changed By B',
    "peer A's merged view has BOTH concurrent field edits - A's name change AND B's role change both survived, neither overwrote the other",
  );
  assert(
    memberB.name === 'Changed By A' && memberB.role === 'Changed By B',
    "peer B's merged view matches peer A's exactly - this is the actual reason to use a CRDT instead of a plain 'replace the whole object' update",
  );
}
...
// 2c. Concurrent PTO span additions to the same member from different peers.
{
  ...
  const spanIdsA = storeA
    .getSnapshot()
    .members[0].ptoSpans.map((p) => p.id)
    .sort();
  const spanIdsB = storeB
    .getSnapshot()
    .members[0].ptoSpans.map((p) => p.id)
    .sort();
  assert(
    JSON.stringify(spanIdsA) === JSON.stringify(['pA', 'pB']),
    'both concurrently-added PTO spans survive the merge on peer A',
  );
  assert(
    JSON.stringify(spanIdsA) === JSON.stringify(spanIdsB),
    'both peers converge to the identical set of PTO spans',
  );
}
...
// 2d. The real edge case: one peer deletes a member while another concurrently edits a field on that same member.
{
  ...
  const snapB = storeB.getSnapshot();
  assert(
    snapA.members.find((m) => m.id === 'doomed') === undefined,
    "the delete wins on peer A - the concurrently-edited member is gone, not resurrected by B's edit",
  );
  assert(
    snapB.members.find((m) => m.id === 'doomed') === undefined,
    "the delete wins on peer B too - both peers converge to the same (deleted) outcome, no crash or orphaned entry from B's edit to now-deleted data",
  );
  assert(
    snapA.members.length === 0 && snapB.members.length === 0,
    "no stale/orphaned member id lingers in either peer's member list after this race",
  );
}
...
// 2e. A member's full field set round-trips correctly through the Yjs read path (sanity check on the mapping itself).
{
  ...
  assert(
    m.id === 'full' &&
      m.name === 'Full Fields' &&
      m.role === 'Role' &&
      m.avatarColor === '#ff0000' &&
      m.defaultPointsPerDay === 2,
...
// === Part 4: confirming team needs NO dedicated seed function - its own public methods already preserve existing ids exactly (unlike requirements/programIncrements, which needed a bypass function since their add* operations always generate fresh ids) ===
{
  ...
  existingStore.addPtoSpan('existing-member-1', {
    id: 'existing-pto-1',
    startDate: '2026-09-01',
    endDate: '2026-09-03',
    startHalfDay: 'full',
