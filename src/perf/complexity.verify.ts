 ...
 */
function measureMedianDuration(
  fn: () => void,
  iterations: number = 7,
  warmups: number = 2,
): number {
...
// 1. flattenSubDiagramTree scaling (O(N))
{
  ...
  assert(
    ratio < 25,
    `flattenSubDiagramTree scales sub-quadratically (8x size -> ${ratio.toFixed(2)}x time)`,
  );
}
...
// 2. unflattenToSubDiagram scaling (O(N))
{
  ...
  const smallFlat = flattenSubDiagramTree(
    generateSubDiagram({ nodeCount: 50, edgeCount: 60, depth: 3, seed: 2 }),
  );
  const largeFlat = flattenSubDiagramTree(
    generateSubDiagram({ nodeCount: 400, edgeCount: 480, depth: 3, seed: 2 }),
  ); // 8x scale
  ...
  assert(
    ratio < 25,
    `unflattenToSubDiagram scales sub-quadratically (8x size -> ${ratio.toFixed(2)}x time)`,
  );
}
...
// 3. getNodesAtPath & getEdgesAtPath scaling (O(N))
{
  ...
  const smallFlat = flattenSubDiagramTree(
    generateSubDiagram({ nodeCount: 50, edgeCount: 60, depth: 1, seed: 3 }),
  );
  const largeFlat = flattenSubDiagramTree(
    generateSubDiagram({ nodeCount: 400, edgeCount: 480, depth: 1, seed: 3 }),
  );
...
// 4. computeEffectiveZIndices scaling (O(N))
{
  ...
  assert(
    ratio < 25,
    `computeEffectiveZIndices scales linearly (8x size -> ${ratio.toFixed(2)}x time)`,
  );
}
...
// 5. computeAlignment scaling (O(N))
{
  ...
  assert(
    ratio < 25,
    `computeAlignment scales linearly with candidates (8x size -> ${ratio.toFixed(2)}x time)`,
  );
}
...
// 6. buildOrthogonalRoute and getSegmentInsertions scaling (O(W))
{
  ...
  assert(
    routeRatio < 25,
    `buildOrthogonalRoute scales linearly in waypoints (8x size -> ${routeRatio.toFixed(2)}x time)`,
  );
  ...
  assert(
    insertRatio < 25,
    `getSegmentInsertions scales linearly in waypoints (8x size -> ${insertRatio.toFixed(2)}x time)`,
  );
}
...
// 7. getContainmentRelation scaling (O(D))
{
  ...
  assert(
    ratio < 25,
    `getContainmentRelation scales linearly with tree depth (8x depth -> ${ratio.toFixed(2)}x time)`,
  );
}
...
// 8. PERF-L1-5: moveEdgeWaypoint single snapshot rebuild & linear scaling
{
  ...
  rebuildCount = 0;
  store.moveEdgeWaypoint(bentEdge.id, firstWaypoint.id, {
    x: firstWaypoint.x + 10,
    y: firstWaypoint.y + 10,
  });

  assert(
    rebuildCount === 1,
    `moveEdgeWaypoint triggers exactly 1 snapshot rebuild (actual: ${rebuildCount})`,
  );
  ...
  seedYjsDiagramDoc(
    smallDoc,
    generateSubDiagram({ nodeCount: 50, edgeCount: 50, depth: 1, seed: 91 }),
  );
  ...
  seedYjsDiagramDoc(
    largeDoc,
    generateSubDiagram({ nodeCount: 400, edgeCount: 400, depth: 1, seed: 91 }),
  ); // 8x scale
  ...
  assert(
    ratio < 30,
    `Yjs snapshot rebuild scales sub-quadratically in node count (8x size -> ${ratio.toFixed(2)}x time)`,
  );
}
