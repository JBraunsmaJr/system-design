...
// 1. Seeded determinism
{
  ...
  const diffSeed = JSON.stringify(
    generateSubDiagram({ nodeCount: 100, edgeCount: 150, seed: 54321 }),
  );
...
// 2. Standard named fixtures sizes
{
  ...
  assert(
    medium.nodes.length === 150,
    `Medium fixture has 150 nodes (actual: ${medium.nodes.length})`,
  );
  assert(
    medium.edges.length === 220,
    `Medium fixture has 220 edges (actual: ${medium.edges.length})`,
  );
  ...
  assert(
    flattenedNested.nodes.length === 300,
    `Nested fixture has 300 total nodes (actual: ${flattenedNested.nodes.length})`,
  );
  assert(
    flattenedNested.edges.length === 400,
    `Nested fixture has 400 total edges (actual: ${flattenedNested.edges.length})`,
  );
  ...
  assert(
    groupNodes.length === 20,
    `Grouped fixture has 20 boundary groups (actual: ${groupNodes.length})`,
  );
  assert(
    standardNodes.length === 200,
    `Grouped fixture has 200 standard nodes (actual: ${standardNodes.length})`,
  );
  assert(
    grouped.edges.length === 250,
    `Grouped fixture has 250 edges (actual: ${grouped.edges.length})`,
  );
}
...
// 4. Scenario requirements support
{
  ...
  assert(
    hubEdges.length >= 25,
    `Large fixture contains hub node with >=25 attached edges (actual: ${hubEdges.length})`,
  );

  const grouped = getStandardFixture('grouped');
  const group0Children = grouped.nodes.filter((n) => n.parentId === 'group-0');
  assert(
    group0Children.length >= 10,
    `Grouped fixture contains boundary with >=10 children (actual: ${group0Children.length})`,
  );
}
