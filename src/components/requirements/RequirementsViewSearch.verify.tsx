...
const testDoc: RequirementsDocument = {
  ...
  items: [
    ...
    },
    {
      id: 'GOAL-1',
      typeId: 'goal',
      title: 'High reliability and auth security',
      body: 'Ensure 99.99% uptime',
      categoryId: 'cat-sec',
    },
  ],
  relationships: [],
...
// === Test 1: Empty search shows total count badge and standard search input ===
{
  ...
  assert(
    !html.includes('requirements-view__search-actions'),
    'search nav actions not rendered when search is empty',
  );
}
...
// === Test 2: Active search shows match counter (e.g. 1 of 2), prev/next nav buttons, and clear button ===
{
  ...
  assert(
    html.includes('requirements-view__search-actions'),
    'search actions container is rendered',
  );
