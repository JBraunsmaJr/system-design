...
console.log("=== HighlightedText ===");
{
  ...
  assert(html.includes('<mark class="search-highlight">REQ-42</mark>'), "case-insensitive search wrapped in mark");
}
...
// Test 1: Full title fits without truncation
{
  ...
  assert(html.includes('<mark class="search-highlight">title</mark>'), "matches inside short title are highlighted");
...
// Test 2: Truncated title with match in visible portion only
{
  ...
  assert(html.includes('<mark class="search-highlight">User</mark>'), "visible portion match is highlighted");
  assert(html.includes('<span class="search-ellipsis">...</span>'), "ellipsis is plain when match is only in visible part");
...
console.log("\n=== highlightInReactNode ===");
{
  ...
  assert(html.includes('<mark class="search-highlight">world</mark>'), "recursively highlights inside React element children");
}
