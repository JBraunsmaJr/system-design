...
const render = (text: string) =>
  renderToStaticMarkup(
    React.createElement(RequirementBody, {
      text,
      doc: EMPTY_REQUIREMENTS_DOCUMENT,
      onNavigateToItem: () => {},
    }),
  );
...
// because CommonMark renders a lone newline as a space.
{
  ...
  assert(
    breaks(html) === 2,
    `three typed lines produce two real line breaks, so they stay on separate lines - got ${breaks(html)}`,
  );
  assert(
    (html.match(/<p>/g) ?? []).length === 1,
    'and it stays ONE paragraph, so the spacing is tight rather than doubled',
  );
}
...
// === Part 2: blank lines still start new paragraphs ===
{
  ...
  assert(
    ...
    'a blank line still starts a new paragraph, as it always did',
  );
}
...
// visibly corrupt it.
{
  ...
  assert(
    breaks(html) === 0,
    'and its internal newlines are NOT turned into breaks - only soft breaks inside paragraphs are rewritten',
  );
}
...
// === Part 5: GFM tables survive ===
{
  ...
  assert(
    html.includes('<table>'),
    'a GFM table still renders as a table rather than collapsing into broken lines',
  );
}
...
// === Part 6: explicit hard breaks still work ===
{
  assert(
    breaks(render('one  \ntwo')) === 1,
    'the two-trailing-spaces hard break still produces exactly one break',
  );
}
...
// break landing mid-emphasis would break the emphasis itself.
{
  ...
  assert(
    ...
    'emphasis ending at a line end still parses as bold',
  );
...
// before rendering; breaks must not interfere with that rewrite.
{
  const doc = {
    ...
    items: [{ id: 'REQ-1', typeId: 'requirement', title: 'First', body: '' }],
  } as never;
...
// === Part 9: search match highlighting in markdown body ===
{
  ...
  assert(
    ...
    'matches are highlighted with search-highlight mark',
  );
  assert(
    html.includes('<mark class="search-highlight">Auth</mark>') ||
      html.includes('<mark class="search-highlight">auth</mark>'),
    'case-insensitive matches in text and code',
  );
}
