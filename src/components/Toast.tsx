...
export function Toast({
  ...
  const content = (
    <div className={`app-toast app-toast--${type}`} role="status" aria-live="polite">
      ...
      <div className="app-toast__body">
        ...
        {description && <div className="app-toast__description">{description}</div>}
      </div>
      {action && (
        <button type="button" className="app-toast__action-btn" onClick={action.onClick}>
          {action.label}
        </button>
      )}
      {onClose && (
  ...
  return createPortal(<div className="app-toast-viewport">{content}</div>, document.body);
}
