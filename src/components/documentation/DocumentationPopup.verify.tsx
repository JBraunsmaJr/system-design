...
// 2. Edge with properties displays popup content
{
  ...
  const html = renderToStaticMarkup(
    <DocumentationRenderer documentation={doc} title="Order Sync" subtitle="Sync Request" />,
  );
  assert(html.includes('Order Sync'), 'DocumentationRenderer renders edge title');
  assert(
    html.includes('Properties'),
    'DocumentationRenderer renders Properties section when present',
  );
...
// 3. Node with tags renders tags as badges
{
  ...
  const html = renderToStaticMarkup(<DocumentationRenderer documentation={doc} />);
...
// 4. Element with no documentation does not display popup (returns null / empty markup)
{
  ...
  const html = renderToStaticMarkup(
    <DocumentationPopup documentation={doc} open={true} anchor={{ x: 100, y: 100 }} />,
  );
...
// 5. DocumentationPopup with open=false does not display
{
  ...
  const html = renderToStaticMarkup(
    <DocumentationPopup documentation={doc} open={false} anchor={{ x: 100, y: 100 }} />,
  );
...
// 6. Complex property formatting and safety
{
  ...
  const html = renderToStaticMarkup(<DocumentationRenderer documentation={doc} title="Service" />);
  assert(
    html.includes('/api/v1/users, /api/v1/auth'),
    'DocumentationRenderer formats array values',
  );
...
// 7. Positive-path test for DocumentationPopup with open=true and meaningful documentation
{
  ...
  assert(
    html.includes('Routes incoming requests to backend services.'),
    'DocumentationPopup renders description text',
  );
