...
const mockDoc: RequirementsDocument = {
  ...
  relationships: [{ id: 'rel-1', typeId: 'blocks', fromItemId: 'TICKET-1', toItemId: 'TICKET-2' }],
  nextSequence: {},
};
...
// --- Test 1: findBlockingItemIds detects workable blockers when an item is dragged ---
{
  ...
  assert(
    blockersWhenDraggingTicket2.has('TICKET-1'),
    'TICKET-1 is identified as a blocker when TICKET-2 is dragged',
  );
...
// --- Test 2: In default state (no active drag), no blocker highlighting or blocker badge exists ---
{
  ...
  assert(
    !html.includes('is-blocker-highlight'),
    'No cards have is-blocker-highlight when no item is dragged',
  );
  assert(
    !html.includes('pi-board-item__blocker-badge'),
    'No cards have pi-board-item__blocker-badge when no item is dragged',
  );
...
// --- Test 3: Clearing drag state removes all blocker highlighting and badges ---
{
  ...
  assert(
    blockersWhenNoneDragged.size === 0,
    'blockingItemIds set is empty when no item is dragged',
  );
}
