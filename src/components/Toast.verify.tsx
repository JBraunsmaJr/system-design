...
// 2. Toast renders description when provided
{
  ...
  assert(
    html.includes('Anyone with this link can join and edit'),
    'Toast renders description text',
  );
}

// 3. Toast renders close button when onClose is provided
{
  const html = renderToStaticMarkup(<Toast message="Copied" onClose={() => {}} />);
  assert(
    html.includes('app-toast__close-btn'),
    'Toast renders close button when onClose is passed',
  );
  assert(
    html.includes('aria-label="Close notification"'),
