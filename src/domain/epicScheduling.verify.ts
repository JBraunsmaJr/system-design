...
// --- Test 1: Full Schedule Inference (100% Scheduled Children - AC-003, DR-003) ---
{
  ...
  assert(
    ...
    '100% scheduled epic infers start date from Sprint 1',
  );
...
// --- Test 2: Partial Schedule Inference with Backlog Items (AC-004, FR-004) ---
{
  ...
  assert(
    ...
    'partially scheduled epic has isFullyScheduled: false',
  );
...
// --- Test 4: Dual-Milestone Epic Anchoring (FR-006, AC-009) ---
{
  const milestones: Milestone[] = [
    {
      ...
      relatedItemIds: ['EPIC-2'],
    },
    {
      id: 'm-target',
      type: 'release',
      name: 'Target Release',
      scheduledAt: '2026-11-30',
      relatedItemIds: ['EPIC-2'],
    },
  ];
  ...
  assert(
    schedule.startDate === '2026-09-15',
    'dual-milestone anchor infers start date from Kickoff milestone',
  );
  assert(
    schedule.endDate === '2026-11-30',
    'dual-milestone anchor infers end date from Target Release milestone',
  );
  assert(
    schedule.isFullyScheduled === true,
    'dual-milestone anchored epic is flagged as fully scheduled',
  );
}
...
// --- Test 5: Child Workable Item Scheduled Across PIs ---
{
  ...
  assert(
    schedule.scheduledSprintIds.includes('sprint-1') &&
      schedule.scheduledSprintIds.includes('sprint-3'),
    'tracks all sprint IDs across PIs',
  );
}
