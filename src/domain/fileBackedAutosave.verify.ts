...
console.log("=== Attaching and writing ===");
{
  ...
  assert(
    again.ok,
    "a second write succeeds - our own change is not mistaken for an external one",
  );
...
console.log("=== A refusal is not a temporary state ===");
{
  ...
  assert(
    !(await autosave.write("{}")).ok,
    "and writes stay refused rather than retrying silently",
  );
}
