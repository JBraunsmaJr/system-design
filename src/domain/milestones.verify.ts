...
// --- Test 3: Required Date Validation (FR-001, FR-002, DR-002) ---
{
  ...
  assert(
    errors.some((e) => e.field === 'scheduledAt'),
    'missing scheduled date is rejected',
  );
  ...
  assert(
    formatErrors.some((e) => e.field === 'scheduledAt'),
    'non YYYY-MM-DD date format is rejected',
  );
  ...
  assert(
    calErrors.some((e) => e.field === 'scheduledAt'),
    'calendar-invalid date Feb 31 is rejected',
  );
}
...
// --- Test 4: Related Items Validation (FR-005, AC-005, Section 12) ---
{
  ...
  assert(
    errors.length === 0,
    'referencing existing workable and non-workable items passes validation',
  );
  ...
  assert(
    nonExistErrors.some(
      (e) => e.field === 'relatedItemIds' || e.field === 'relatedWorkableItemIds',
    ),
...
// --- Test 7: Date Range Filtering & Search (Section 13) ---
{
  ...
  assert(
    inJan.length === 2 && inJan[0]?.id === 'm-1' && inJan[1]?.id === 'm-2',
    'filters milestones correctly by date range',
  );
}
...
// --- Test 8: Find Milestones by Workable Item (Section 13) ---
{
  const testList: Milestone[] = [
    ...
    },
    {
      ...
      type: 'release',
      name: 'R2',
      scheduledAt: '2026-01-20',
      relatedWorkableItemIds: ['TICKET-2'],
    },
    {
      id: 'm-3',
      type: 'release',
  ...
  assert(
    forT1.length === 2 && forT1[0]?.id === 'm-1' && forT1[1]?.id === 'm-3',
    'finds milestones associated with TICKET-1',
  );
}
...
// --- Test 9: Color and Type Labels ---
{
  ...
  assert(
    getMilestoneTypeLabel('release') === 'Release',
    'returns human readable label for release',
  );
  assert(
    getMilestoneTypeLabel('code-freeze') === 'Code Freeze',
    'returns human readable label for code-freeze',
  );
}
...
// --- Test 10: Built-in Item Types & Relationship Types (FR-001, FR-002) ---
{
  ...
  assert(
    !!epicType &&
      epicType.prefix === 'EPIC' &&
      epicType.isWorkable === false &&
      epicType.color === '#8b5cf6',
  ...
  assert(
    !!depType &&
      depType.prefix === 'DEP' &&
      depType.isWorkable === false &&
      depType.color === '#f59e0b',
  ...
  assert(
    !!parentRel &&
      parentRel.label === 'Parent of' &&
      parentRel.inverseLabel === 'Child of' &&
      parentRel.isBlocking === false,
  ...
  assert(
    !!dependsRel &&
      dependsRel.label === 'Depends on' &&
      dependsRel.inverseLabel === 'Depended on by' &&
      dependsRel.isBlocking === true,
...
// --- Test 11: Milestone Related Item Helper Functions (FR-005) ---
{
  ...
  assert(
    epics.length === 1 && epics[0]?.id === 'EPIC-1',
    'filterMilestoneItemsByType filters by epic',
  );

  const forDep = findMilestonesForItem([milestone], 'DEP-1');
  assert(
    forDep.length === 1 && forDep[0]?.id === 'm-test',
    'findMilestonesForItem finds milestone linked to DEP-1',
  );
}
