...
function item(id: string, typeId: string, sprintId?: string): RequirementItem {
  return {
    ...
    title: id,
    body: '',
    status: typeId === 'ticket' ? 'todo' : undefined,
    sprintId,
  } as RequirementItem;
}
...
// that could ever clear it.
{
  ...
  assert(
    conflict === null,
    'a ticket blocked by a Requirement can be assigned to a sprint - the non-workable blocker is not a scheduling conflict',
  );
}
...
// The guard must not have thrown away the real rule along with the bug.
{
  ...
  assert(
    ...
    "a ticket blocked by another unscheduled TICKET is still blocked - the fix narrows the rule, it doesn't remove it",
  );
...
{
  const items = [
    item('TICKET-1', 'ticket'),
    item('TICKET-2', 'ticket'),
  ...
  assert(
    conflict?.severity === 'blocked' && conflict.blocker.id === 'TICKET-2',
    'with both a Requirement and an unscheduled Ticket blocking it, the real blocker is still reported and the Requirement is ignored',
  );
}
...
// === Part 4: same-sprint workable blocker is still only a "risk" ===
{
  ...
  assert(
    ...
    'a blocker in the SAME sprint is still a risk rather than a hard block - unchanged by this fix',
  );
}
...
// then flagged as fine once placed (or the reverse).
{
  ...
  assert(
    conflicts.length === 0,
    'an already-scheduled ticket is not flagged as conflicted just because a Requirement blocks it',
  );
}
...
// === Part 6: a real conflict is still found by findScheduleConflicts ===
{
  ...
  assert(
    conflicts.length === 1 && conflicts[0].severity === 'blocked',
    'a blocker scheduled AFTER the item it blocks is still a genuine conflict',
  );
}
...
// can never be scheduled can never come first.
{
  ...
  const blockers = findBlockingItemIds(
    'TICKET-1',
    relationships,
    relationshipTypes,
    itemTypes,
  ...
  assert(
    !blockers.has('REQ-1'),
    'a Requirement is not highlighted as something that must be scheduled first',
  );
...
// pull in documentation lineage as if it were schedule ordering.
{
  ...
  assert(
    blockers.size === 0,
    'traversal stops at the non-workable item rather than continuing to whatever blocks IT',
  );
}
...
// === Part 9: transitive workable chains still traverse fully ===
{
  ...
  assert(
    blockers.has('TICKET-2') && blockers.has('TICKET-3'),
    'an all-workable chain is still followed transitively',
  );
}
...
// === Part 10: non-blocking relationship types are still irrelevant ===
{
  ...
  assert(
    conflict === null,
    'a non-blocking relationship type never produces a conflict, workable or not',
  );
}
