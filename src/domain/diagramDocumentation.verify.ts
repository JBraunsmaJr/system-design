...
assert(
  formatPropertyValue(['a', 'b', 'c']) === 'a, b, c',
  'formatPropertyValue formats array correctly',
);
...
assert(
  typeof formattedCircular === 'string' && formattedCircular.includes('[Circular]'),
  'formatPropertyValue handles circular references safely without throwing',
);
...
assert(
  typeof formattedArr === 'string' && formattedArr.includes('[Circular]'),
  'formatPropertyValue handles cyclic array references safely without throwing',
);

assert(
  hasDocumentation({ properties: { cyclic: circularObj } }),
  'hasDocumentation handles cyclic structures without stack overflow',
);
...
assert(
  extractedNode.subtitle?.includes('Microservice') === true,
  'extractNodeDocumentation extracts subtitle',
);
assert(
  extractedNode.documentation.description === 'Routes incoming requests',
  'extractNodeDocumentation extracts description',
);
assert(
  hasDocumentation(extractedNode.documentation),
  'extractedNodeDocumentation hasDocumentation is true',
);
...
assert(
  hasDocumentation(extractedEdge.documentation),
  'extractedEdgeDocumentation hasDocumentation is true',
);
...
assert(
  centerPos.left === 110 && centerPos.top === 110,
  'computeDocumentationPopupPosition places offset by gap in open space',
);

// Right edge overflow
const rightEdgePos = computeDocumentationPopupPosition({ x: 950, y: 100 }, popupSize, viewport, 10);
assert(
  rightEdgePos.left < 950,
  'computeDocumentationPopupPosition adjusts left position near right edge',
);

// Bottom edge overflow
const bottomEdgePos = computeDocumentationPopupPosition(
  { x: 100, y: 750 },
  popupSize,
  viewport,
  10,
