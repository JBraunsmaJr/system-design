...
// --- Test 1: Milestone Linking to Diverse Types (AC-005, FR-005) ---
{
  for (const m of milestones) {
    ...
    assert(
      errors.length === 0,
      `Milestone '${m.name}' links validly across Epics, Dependencies, and Tickets`,
    );
  }
  ...
  assert(
    depMilestones.length === 1 && depMilestones[0]?.id === 'm-dep-deadline',
    'DEP-1 is linked to Vendor API Ready milestone',
  );
}
...
// --- Test 2: Epic Inferred Schedule with 100% Scheduled Children (AC-003, DR-003) ---
{
  ...
  assert(
    schedule.startDate === '2026-09-28',
    'Epic start date accounts for Start Milestone (2026-09-28)',
  );
  assert(schedule.endDate === '2026-10-28', 'Epic end date infers from Sprint 2 end (2026-10-28)');
  assert(
    schedule.isFullyScheduled === true,
    'isFullyScheduled is true when all 2 child tickets are scheduled',
  );
...
// --- Test 3: Epic Inferred Schedule with Unscheduled Child Item (AC-004, FR-004) ---
{
  ...
  assert(
    ...
    'Epic end date is STRICTLY undefined when 1 child ticket is in backlog',
  );
  assert(
    schedule.isFullyScheduled === false,
    'isFullyScheduled is false for partially scheduled epic',
  );
...
// --- Test 4: Sprint Milestone & Release Summaries (AC-006, FR-007) ---
{
  ...
  assert(
    summary1.directMilestones.length === 1,
    'Sprint 1 has 1 direct milestone (Vendor API Ready on 2026-10-05)',
  );
  assert(summary1.impactedReleases.length === 1, 'Sprint 1 impacts Release 3.0');
  assert(
    summary1.impactedReleases[0]?.completionPercentage === 33,
    'Release 3.0 is 33% completed (1/3 items done: TICKET-101)',
  );
  assert(
    summary1.externalDependencies.length === 1,
    'Sprint 1 identifies external dependency DEP-1',
  );
}
...
// --- Test 5: Strict Zero Capacity Workload Invariant (DR-001, FR-008, AC-007) ---
{
  ...
  const sprint1WorkableItems = doc.items.filter(
    (i) => i.sprintId === 'sprint-1' && isItemWorkable(doc, i),
  );
  const capacity1 = computeSprintCapacity(pi.sprints[0], sprint1Range, team, sprint1WorkableItems);

  assert(
    capacity1.totalAssignedPoints === 5,
    'Capacity assigned points is 5 (TICKET-101 only, no milestone/dependency workload)',
  );
}
