...
function hasEmphasis(markdown: string, kind: 'strong' | 'emphasis', expected: string): boolean {
  ...
  const collectText = (node: { children?: unknown[]; value?: string }): string => {
    ...
    return ((node.children ?? []) as { children?: unknown[]; value?: string }[])
      .map(collectText)
      .join('');
  };
  walk(tree as never);
...
// === Part 1: a clean selection wraps and parses as bold ===
{
  ...
  assert(
    r.newText === 'make this **bold** please',
    'a whitespace-free selection is wrapped exactly as before',
  );
  assert(
    hasEmphasis(r.newText, 'strong', 'bold'),
    'and the result genuinely parses as strong emphasis',
  );
  assert(
    r.newText.slice(r.newSelStart, r.newSelEnd) === 'bold',
    'the new selection covers the wrapped word, not the delimiters',
  );
}
...
// so this is the common case, not an edge case.
{
  ...
  assert(
    r.newText === 'make this **bold** please',
    'a trailing space is hoisted OUT of the delimiters rather than trapped inside them',
  );
  assert(
    hasEmphasis(r.newText, 'strong', 'bold'),
    'so the result parses as bold - the old behavior produced `**bold **`, which CommonMark renders as literal asterisks because a closing run may not be preceded by whitespace',
  );
  assert(
    r.newText !== 'make this **bold **please',
    'specifically, the old output `**bold **please` is no longer produced',
  );
}
...
// === Part 3: a LEADING space in the selection ===
{
  ...
  assert(
    r.newText === 'make this **bold** please',
    'a leading space is hoisted out too - an opening run may not be FOLLOWED by whitespace',
  );
...
// === Part 4: whitespace on both sides, and multiple spaces ===
{
  ...
  assert(
    r.newText === 'a  **bold**  b',
    'leading and trailing runs of whitespace are both preserved in place, outside the delimiters',
  );
  assert(hasEmphasis(r.newText, 'strong', 'bold'), 'and the result parses as bold');
}
...
// === Part 6: an empty selection still inserts the placeholder ===
{
  ...
  assert(
    r.newText === 'before **bold text**after',
    'an empty selection inserts the wrapped placeholder at the caret',
  );
  assert(
    r.newText.slice(r.newSelStart, r.newSelEnd) === 'bold text',
    'with the placeholder selected so it can be typed over',
  );
  assert(
    hasEmphasis(r.newText, 'strong', 'bold text'),
    'and the inserted placeholder parses as bold',
  );
}
...
// emphasis and leaves the user with stray asterisks to clean up.
{
  ...
  assert(
    r.newText === 'a   **bold text**b',
    'an all-whitespace selection keeps its whitespace and wraps the placeholder after it',
  );
...
// whitespace is part of the emphasized text and must stay inside.
{
  ...
  assert(
    r.newText === 'wrap **two words** here',
    'internal whitespace stays inside the delimiters',
  );
  assert(
    hasEmphasis(r.newText, 'strong', 'two words'),
    'and the whole phrase parses as one bold run',
  );
}
...
// the paragraph structure as well as the emphasis.
{
  ...
  assert(
    r.newText === 'line **one**\nline two',
    'a trailing newline is hoisted out, keeping the line structure intact',
  );
...
// === Part 10: the other helpers are unaffected ===
{
  ...
  assert(
    prefixed.newText === 'first\n- second',
    "insertLinePrefix still prefixes the caret's own line",
  );

  const table = insertTableSkeleton('text', 4);
  assert(
    table.newText.startsWith('text\n| Column 1 |'),
    'insertTableSkeleton still inserts a leading newline when not at a line start',
  );
  assert(
    table.newText.slice(table.newCaretPos).startsWith('Column 1'),
    'and still lands the caret in the first header cell',
  );
}
