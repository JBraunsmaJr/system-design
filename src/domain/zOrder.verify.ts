...
// because the rectangle painted over them.
{
  ...
  assert(
    z.get('rect')! < z.get('node-a')!,
    'a large rectangle sits behind a small node it covers - the nodes stay clickable',
  );
...
// The nested case: the bigger one is the container, so it goes behind.
{
  ...
  assert(
    z.get('outer')! < z.get('inner')!,
    "of two nested rectangles the BIGGER one is furthest back - it's the grouping boundary, so what's inside it draws on top",
  );
}
...
// === Part 3: three levels of nesting ===
{
  const boxes = [
    ...
    box('huge', 0, 0, 900, 900),
    box('medium', 10, 10, 300, 300),
  ];
  const z = computeEffectiveZIndices(boxes);

  assert(
    z.get('huge')! < z.get('medium')! && z.get('medium')! < z.get('small')!,
    'nesting works to any depth without a special case - it falls out of ordering by area',
  );
}
...
// would bury a callout drawn deliberately over something large.
{
  ...
  assert(
    z.get('callout')! > z.get('big-node')!,
    "a small shape drawn over a large node stays in front - size signals intent, not the node's type",
  );
}
...
// changes, or they'd flicker.
{
  ...
  assert(
    first.get('a') === second.get('a') && first.get('b') === second.get('b'),
    "equal-area boxes get the same z regardless of input order - the value comes from the box's own area, so array order can't perturb it",
  );
}
...
// canvas and would otherwise be comparing incomparable numbers.
{
  ...
  assert(
    z.get('rect') === 950,
    'an explicit zIndex is used exactly as given, not recomputed from geometry',
  );
  assert(
    z.get('rect')! > z.get('node')!,
    'so a rectangle explicitly raised above the automatic range sits in front of a small node, overriding the area rule',
  );
}
...
{
  const z = computeEffectiveZIndices([
    box('tiny', 0, 0, 1, 1),
    box('unmeasured', 0, 0, 0, 0),
    box('huge', 0, 0, 5000, 5000),
  ]);
  assert(
    [...z.values()].every((v) => v >= 0 && v < 1000),
    `every automatic z-index stays within 0..999 - got ${[...z.values()].join(', ')}`,
  );
}
...
// real document it moved 67 of 137 nodes.
{
  ...
  const after = computeEffectiveZIndices(
    many.map((b) => (b.id === 'n70' ? { ...b, width: 400 } : b)),
  );

  const moved = many.filter((b) => before.get(b.id) !== after.get(b.id)).map((b) => b.id);
  assert(
    moved.length === 1 && moved[0] === 'n70',
    `resizing one node changes ONLY that node's z-index - got ${moved.length} changed (${moved.slice(0, 5).join(', ')})`,
  );
}
...
// === Part 7: bring to front ===
{
  ...
  assert(
    patches.length === 1 && patches[0].id === 'rect',
    'bringing to front patches only the selected node',
  );
  const after = computeEffectiveZIndices(
    boxes.map((b) => (b.id === 'rect' ? { ...b, zIndex: patches[0].zIndex } : b)),
  );
  assert(
    after.get('rect')! > after.get('a')! && after.get('rect')! > after.get('b')!,
    'and it lands above everything else',
  );
}
...
// === Part 8: send to back ===
{
  ...
  assert(
    patches.length === 1,
    "sending to back produces a patch when the node isn't already at the back",
  );
  const after = computeEffectiveZIndices(
    boxes.map((b) => (b.id === 'rect' ? { ...b, zIndex: patches[0].zIndex } : b)),
  );
  ...
  assert(
    after.get('rect')! < after.get('node')!,
    'sending to back drops it below everything - the fix for a rectangle covering things',
  );
}
...
// show for it, and each one is a store write synced to every peer.
{
  ...
  assert(
    applyZOrderCommand(boxes, ['a'], 'front').length === 0,
    'bringing an already-frontmost node to the front changes nothing',
  );
  assert(
    applyZOrderCommand(boxes, ['b'], 'back').length === 0,
    'and the same for an already-backmost node',
  );
}
...
// nothing happened, and makes escaping a cover take many clicks.
{
  ...
  assert(
    patches[0].zIndex < 99,
    "and NOT past a distant node it doesn't overlap - that step would have been invisible",
  );
}
...
// === Part 11: forward with nothing overlapping is a no-op ===
{
  ...
  assert(
    applyZOrderCommand(boxes, ['alone'], 'forward').length === 0,
    'a node overlapping nothing has no reordering to do, so the command does nothing rather than silently changing a number',
  );
}
...
// === Part 12: a multi-node selection moves as a block ===
{
  ...
  assert(
    ...
    'and keep their own relative order rather than collapsing onto one value',
  );
}
...
{
  assert(
    boxesOverlap(box('a', 0, 0, 100, 100), box('b', 50, 50, 100, 100)),
    'genuinely overlapping boxes are detected',
  );
  assert(
    !boxesOverlap(box('a', 0, 0, 100, 100), box('b', 100, 0, 100, 100)),
    "boxes sharing only an edge don't count - neither is obscuring the other",
  );
  assert(
    !boxesOverlap(box('a', 0, 0, 100, 100), box('b', 200, 200, 50, 50)),
...
// Nodes report no dimensions until React Flow has measured them.
{
  ...
  assert(
    z.get('rect')! < z.get('unmeasured')!,
    'a not-yet-measured node is treated as zero-area and lands in front rather than throwing off the sort',
  );
}
