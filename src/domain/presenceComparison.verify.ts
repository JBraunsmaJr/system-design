...
{
  const before: PeerBadge[] = [
    { ...alice(), cursor: { x: 10, y: 10 } } as PeerBadge & { cursor: unknown },
  ];
  const after: PeerBadge[] = [
    { ...alice(), cursor: { x: 900, y: 400 } } as PeerBadge & { cursor: unknown },
  ];
...
{
  assert(
    !peerBadgesAreEqual([alice()], [{ ...alice(), name: 'Alicia' }]),
    'a renamed peer is a change - the badge renders the first initial and the hover title',
  );
  assert(
    !peerBadgesAreEqual([alice()], [{ ...alice(), color: '#000000' }]),
    'a recolored peer is a change - the badge dot uses it',
  );
  assert(
    !peerBadgesAreEqual([alice()], [{ ...alice(), clientId: 99 }]),
    "a different clientId is a change - it's the badge's React key",
  );
}
...
{
  assert(
    !peerBadgesAreEqual([], [alice()]),
    'a peer arriving on this item is a change - this is the staleness the memo was hiding',
  );
  assert(
    !peerBadgesAreEqual([alice()], []),
    'a peer leaving is a change too, so the badge actually disappears',
  );
  assert(
    !peerBadgesAreEqual([alice()], [alice(), bob()]),
    'a second peer joining the same item is a change',
  );
}
...
// genuinely sees undefined from any caller that omits it.
{
  ...
  assert(
    peerBadgesAreEqual(undefined, []),
    'omitted and empty are equal - the default is applied inside the component, so the comparator sees both forms',
  );
...
{
  assert(
    !peerBadgesAreEqual([alice(), bob()], [bob(), alice()]),
    'reordered peers compare unequal - the safe direction to be wrong in, and cheaper than sorting',
  );
}
