...
console.log('=== 1. Registry covers the whole version chain ===');
{
  ...
  assert(new Set(froms).size === froms.length, 'no two migrations share a `from` version');
  assert(
...
console.log('=== 3. Migrating forward ===');
{
  ...
  assert(file.schemaVersion === SCHEMA_VERSION, 'the result declares the current version');
  assert(applied.length === VERSION_ORDER.length - 1, 'every step in the chain ran');
...
console.log('=== 4. A missing version is treated as the oldest ===');
{
  ...
  assert(
    migrateToCurrent(blank, SCHEMA_VERSION).applied.length === VERSION_ORDER.length - 1,
...
console.log('=== 5. Newer and unrecognised versions are refused ===');
{
  ...
  assert(
    caught?.message.includes('99.0') === true && caught?.message.includes(SCHEMA_VERSION) === true,
...
console.log('=== 7. Migrations do not mutate their input ===');
{
  ...
  assert(input.schemaVersion === '0.1', "the caller's object is left at its original version");
}
