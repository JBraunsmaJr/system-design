...
// === generateSessionKey ===
{
  ...
  assert(
    typeof k1 === 'string' && k1.length >= 16,
    'generateSessionKey generates non-empty string key',
  );
...
// === createSessionLink ===
{
  ...
  assert(
    link1 === 'https://example.com/app#session=session-12345',
    'link without key or custom relay contains only session',
  );
  ...
  assert(
    link2 === 'https://example.com/app#session=session-12345&key=secret-key-123',
    'link with key includes key parameter',
  );
  ...
  assert(
    link2b === 'https://example.com/app#session=session-12345&key=compat-password',
    'link with password option sets key parameter for consistency',
  );
  ...
  assert(
    link3 ===
      'https://example.com/app#session=session-12345&key=k_abc&relay=wss%3A%2F%2Fcustom-relay.example.com',
  ...
  assert(link5.includes('key='), 'key with special chars is encoded');
...
// === parseSessionLink ===
{
  ...
  assert(
    p1.roomName === 'session-abc123' && p1.password === undefined && p1.relay === undefined,
    'bare session code parsed',
  );
  ...
  const p3 = parseSessionLink(
    'https://host.com/system-design/#session=session-xyz&key=mysecret&relay=wss%3A%2F%2Frelay.org',
  );
  assert(
  ...
  const p3b = parseSessionLink(
    'https://host.com/system-design/#session=session-xyz&password=mysecret&relay=wss%3A%2F%2Frelay.org',
  );
  assert(
  ...
  assert(
    p5.roomName === 'session-xyz' && p5.key === 'pass123' && p5.password === 'pass123',
    'hash fragment alone parsed',
  );

  // 5. Query fragment alone
  const p6 = parseSessionLink('?session=session-xyz&key=pass123');
  assert(
    p6.roomName === 'session-xyz' && p6.key === 'pass123' && p6.password === 'pass123',
    'query fragment alone parsed',
  );

  // 6. Key-value string
  const p7 = parseSessionLink('session=session-xyz&key=pass123');
  assert(
    p7.roomName === 'session-xyz' && p7.key === 'pass123' && p7.password === 'pass123',
    'raw key-value string parsed',
  );

  // 7. Aliases (room, id, pwd, signaling)
  const p8 = parseSessionLink(
    '#room=session-alias&pwd=alias-pass&signaling=ws%3A%2F%2Flocal%3A4444',
  );
  assert(
  ...
  const pMd = parseSessionLink(
    '[Join Session](https://host.com/system-design/#session=session-md&key=md-pass&relay=wss%3A%2F%2Fmd-relay.org)',
  );
  assert(
    pMd.roomName === 'session-md' &&
      pMd.key === 'md-pass' &&
      pMd.password === 'md-pass' &&
      pMd.relay === 'wss://md-relay.org',
    'markdown link parsed correctly',
  );

  // 9. Angle bracket URL format
  const pAngle = parseSessionLink(
    '<https://host.com/system-design/#session=session-angle&key=angle-pass&relay=wss%3A%2F%2Fangle-relay.org>',
  );
  assert(
    pAngle.roomName === 'session-angle' &&
      pAngle.key === 'angle-pass' &&
      pAngle.password === 'angle-pass' &&
      pAngle.relay === 'wss://angle-relay.org',
    'angle bracket wrapped URL parsed correctly',
  );

  // 10. Quoted URL format
  const pQuoted = parseSessionLink(
    '"https://host.com/system-design/#session=session-quoted&key=quoted-pass&relay=wss%3A%2F%2Fquoted-relay.org"',
  );
  assert(
    pQuoted.roomName === 'session-quoted' &&
      pQuoted.key === 'quoted-pass' &&
      pQuoted.password === 'quoted-pass' &&
      pQuoted.relay === 'wss://quoted-relay.org',
    'quoted URL parsed correctly',
  );

  // 11. JSON object payload
  const pJson = parseSessionLink(
    JSON.stringify({
  ...
  assert(
    pJson.roomName === 'session-json123' &&
      pJson.key === 'secret-json-key' &&
      pJson.password === 'secret-json-key' &&
      pJson.relay === 'wss://json-relay.org',
  ...
  `);
  assert(
    pDmText.roomName === 'session-dm-multiline' &&
      pDmText.key === 'dm-secret-code' &&
      pDmText.password === 'dm-secret-code' &&
      pDmText.relay === 'wss://dm-relay.net',
    'formatted multi-line DM text parsed correctly',
  );

  // 13. Inline DM text
  const pDmInline = parseSessionLink(
    'Session ID: session-dm-inline, Key: inline-secret, Relay URL: wss://inline-relay.org',
  );
  assert(
    pDmInline.roomName === 'session-dm-inline' &&
      pDmInline.key === 'inline-secret' &&
      pDmInline.password === 'inline-secret' &&
      pDmInline.relay === 'wss://inline-relay.org',
  ...
  assert(
    pPipe.roomName === 'session-pipe123' &&
      pPipe.key === 'pipe-key' &&
      pPipe.password === 'pipe-key' &&
      pPipe.relay === 'wss://pipe-relay.org',
...
// === cleanSessionFromUrl & sanitizeCurrentUrl ===
{
  ...
  const c1 = cleanSessionFromUrl(
    'https://example.com/system-design/#session=session-xyz&key=secret123',
  );
  assert(
    c1 === 'https://example.com/system-design/',
    'cleans hash fragment containing session and key',
  );

  // 2. Hash fragment with relay override
  const c2 = cleanSessionFromUrl(
    'https://example.com/system-design/#session=session-xyz&key=secret123&relay=wss%3A%2F%2Frelay.org',
  );
  assert(
    c2 === 'https://example.com/system-design/',
    'cleans hash fragment containing session, key, and relay',
  );

  // 3. Query params with session and key
  const c3 = cleanSessionFromUrl(
    'https://example.com/system-design/?session=session-xyz&key=secret123',
  );
  assert(
    c3 === 'https://example.com/system-design/',
    'cleans query params containing session and key',
  );

  // 4. Preserves non-session query parameters
  const c4 = cleanSessionFromUrl(
    'https://example.com/system-design/?theme=dark&session=session-xyz&key=secret123',
  );
  assert(
    c4 === 'https://example.com/system-design/?theme=dark',
  ...
  (
    globalThis as unknown as {
      window?: {
        ...
        history: { state: null; replaceState: (_s: unknown, _t: string, url: string) => void };
      };
  ...
  sanitizeCurrentUrl();
  assert(
    replacedUrl === 'https://example.com/system-design/',
    'sanitizeCurrentUrl calls window.history.replaceState with clean URL',
  );
