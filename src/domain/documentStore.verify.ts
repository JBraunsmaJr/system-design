...
async function runSuite(label: string, store: DocumentStore) {
  ...
  assert(afterDelete.ok && afterDelete.value.length === 1, 'deleted document leaves the index');
  ...
  console.log(`=== ${label}: legacy autosave import (WS5-R8) ===`);
  {
    ...
    assert(second.ok && second.value === null, 'a second run finds nothing and does nothing');
  }
...
console.log('=== Atomicity: a failed write leaves no dangling index entry ===');
{
  ...
  for (const [label, backend] of [
    ['atomic modify', withModify],
    ['writeAll fallback', fallback],
  ] as const) {
    ...
    assert(
      ...
      `${label}: a quota failure is reported as quota`,
    );
    assert(
...
console.log('=== Concurrent saves of different documents keep both entries (WS2-R3) ===');
{
  ...
  for (const [label, backend] of [
    ...
    await Promise.all(
      ids.map((id, i) => (i % 2 ? tabA : tabB).writeDocument(id, makeFile(`Doc ${i}`))),
    );
    const listed = await tabA.listDocuments();
    const found = listed.ok
      ? ids.filter((id) => listed.value.some((e) => e.docId === id)).length
      : 0;
    assert(
      found === ids.length,
      `${label}: all ${ids.length} concurrently saved documents are indexed (found ${found})`,
    );
  }
}
...
console.log('=== Error classification ===');
{
  ...
  assert(
    classifyStorageError(new SchemaVersionError('newer', '9.9', SCHEMA_VERSION)) === 'version',
