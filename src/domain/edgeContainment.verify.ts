...
// if only direct children are recognised.
{
  ...
  assert(
    isInside('handler', 'region', nested),
    'containment is transitive - a deeply nested node is inside every boundary above it',
  );
...
// something wrong.
{
  ...
  assert(
    isInside('a', 'nowhere', cyclic) === false,
    "and walking a cycle looking for a node that isn't there terminates instead of looping forever",
  );
}
...
{
  assert(
    getContainmentRelation('boundary', 'api', simple) === 'target-inside',
    'boundary -> contained node is classified as target-inside',
  );
  assert(
    getContainmentRelation('api', 'boundary', simple) === 'source-inside',
    'contained node -> boundary is classified as source-inside',
  );
  assert(
    getContainmentRelation('api', 'db', simple) === 'none',
    'two siblings inside the same boundary are unrelated - neither contains the other',
  );
  assert(
    getContainmentRelation('api', 'outsider', simple) === 'none',
    'an edge to something outside the boundary is an ordinary edge',
  );
  assert(
    getContainmentRelation('boundary', 'boundary', simple) === 'none',
    'a self-edge is not containment',
  );
}
...
// Flipping the boundary end sends that segment inward instead.
{
  ...
  assert(
    routed.sourcePosition === Position.Right,
    'the boundary end is flipped so the path heads INTO the boundary rather than out of it',
  );
  assert(
    routed.targetPosition === Position.Left,
    'the contained node keeps its own handle side - it really is approached from that side',
  );
}
...
// === Part 7: the reverse direction, egress from inside ===
{
  ...
  assert(
    routed.targetPosition === Position.Left,
    'when the boundary is the TARGET, its end is the one flipped',
  );
...
// an edge between two unrelated nodes.
{
  ...
  assert(
    routed.sourcePosition === Position.Bottom && routed.targetPosition === Position.Top,
    'an edge between unrelated nodes routes exactly as before',
  );
}
...
// boundary wall, trading one wrong-looking path for another.
{
  for (const relation of ['target-inside', 'source-inside'] as const) {
    ...
    const flipped = [
      routed.sourcePosition !== Position.Top,
      routed.targetPosition !== Position.Top,
    ].filter(Boolean);
...
// drawn curve, and therefore a conservative test.
{
  ...
  const escapes = (b: ReturnType<typeof pathBounds>) =>
    b.minX < boundary.x ||
    b.maxX > boundary.x + boundary.width ||
    b.minY < boundary.y ||
    b.maxY > boundary.y + boundary.height;
  ...
  for (const [side, sx, sy] of sides) {
    ...
    assert(
      !escapes(pathBounds(routed.sourcePosition, sx, sy)),
  ...
  assert(
    escapes(pathBounds(Position.Left, boundary.x, 200)),
...
// connector can't be grabbed to start a new edge.
{
  ...
  assert(
    inset.sourceY === 100 + HANDLE_CLEARANCE,
    'the boundary end of the band retreats INWARD along the edge, not sideways off it',
  );
  assert(inset.sourceX === points.sourceX, "and doesn't drift across the other axis");
  assert(
    inset.targetX === points.targetX && inset.targetY === points.targetY,
    "the contained node's end is untouched - its handle isn't the one being covered",
  );
  ...
  assert(
    HANDLE_CLEARANCE > 6,
    `the clearance (${HANDLE_CLEARANCE}px) exceeds the handle's own ~6px radius, so the band's end cap lands clear of it`,
  );
}
...
// === Part 12: each side retreats along its own axis ===
{
  ...
  for (const [side, axis, expected] of cases) {
    ...
    assert(
      inset[axis] === expected,
      `a connector on the ${side} side retreats into the boundary along the right axis`,
    );
  }
}
