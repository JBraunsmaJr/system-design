import { validateAssetLibrary, globalAssetLibraryManager, type AssetLibrary } from './assetLibrary';
...
console.log('=== 1. Testing Library Schema Validation ===');
{
  ...
  assert(
    resBad.errors.some((e) => e.includes('wrong-format') || e.includes('system-design-library')),
...
console.log('\n=== 2. Testing Library Manager Integration & Syncing ===');
{
  ...
  assert(
    globalIconRegistry.getIcon('mesh.envoy') !== undefined,
    'Library icon registered in globalIconRegistry',
  );
  assert(
    globalShapeRegistry.getShape('mesh.sidecar') !== undefined,
    'Library shape registered in globalShapeRegistry',
  );
  ...
  assert(
    globalIconRegistry.getIcon('mesh.envoy') === undefined,
    'Disabling library removes icon from active registry',
  );
  assert(
    globalShapeRegistry.getShape('mesh.sidecar') === undefined,
    'Disabling library removes shape from active registry',
  );
  ...
  assert(
    globalIconRegistry.getIcon('mesh.envoy') !== undefined,
    'Re-enabling library restores icon in active registry',
  );
  assert(
    globalShapeRegistry.getShape('mesh.sidecar') !== undefined,
    'Re-enabling library restores shape in active registry',
  );
  ...
  assert(
    globalIconRegistry.getIcon('mesh.envoy') === undefined,
    'Deleting library unregisters icon',
  );
  assert(
    globalShapeRegistry.getShape('mesh.sidecar') === undefined,
    'Deleting library unregisters shape',
  );
}
