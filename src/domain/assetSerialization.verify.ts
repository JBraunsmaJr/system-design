...
console.log('=== 1. Testing Diagram Serialization with Asset Fallbacks ===');
{
  ...
  assert(
    file.shapeFallbacks?.['acme.payment-service'] !== undefined,
    "shapeFallbacks contains 'acme.payment-service'",
  );
  assert(file.iconFallbacks !== undefined, 'toDiagramFile generates iconFallbacks');
  assert(
    file.iconFallbacks?.['acme.payment-hub'] !== undefined,
    "iconFallbacks contains 'acme.payment-hub'",
  );
  ...
  assert(
    globalShapeRegistry.getShape('acme.payment-service') === undefined,
    'Shape absent prior to loading',
  );
  assert(
    globalIconRegistry.getIcon('acme.payment-hub') === undefined,
    'Icon absent prior to loading',
  );
  ...
  assert(
    globalShapeRegistry.getShape('acme.payment-service') !== undefined,
    'Shape fallback automatically restored in registry',
  );
  assert(
    globalIconRegistry.getIcon('acme.payment-hub') !== undefined,
    'Icon fallback automatically restored in registry',
  );
  assert(
    globalShapeRegistry.getShape('acme.payment-service')?.name === 'Payment Service',
    'Restored shape retains name',
  );
}
...
console.log('\n=== 2. Testing Definition of Done Workflow (Requirements Section 21) ===');
{
  ...
  assert(
    foundIcons.length > 0 && foundIcons[0].id === 'fintech.ledger',
    'Found custom icon via search',
  );
