...
console.log('=== 1. Testing Built-in Shapes Registration & Retrieval ===');
{
  ...
  assert(
    server?.category === 'System Design',
    "system.server is categorized under 'System Design'",
  );
  assert(server?.iconId === 'Server', "system.server has iconId 'Server'");

  const db = globalShapeRegistry.getShape('system.database');
  assert(
    db !== undefined && db.geometry.type === 'cylinder',
    'system.database has cylinder geometry',
  );
...
console.log('\n=== 3. Testing Custom Shape Registration & Dynamic Search ===');
{
  ...
  assert(
    searchHits.some((s) => s.id === 'test.custom-gateway'),
    "Custom shape found by tag search 'ingress'",
  );

  globalShapeRegistry.unregisterLibraryShapes('test-lib');
  assert(
    globalShapeRegistry.getShape('test.custom-gateway') === undefined,
    'Unregistering library shapes cleans up registry',
  );
}
...
console.log('\n=== 4. Testing Backward Compatibility Layer ===');
{
  ...
  assert(
    SHAPE_TYPES.some((s) => s.id === 'circle'),
    'SHAPE_TYPES includes circle',
  );
  assert(
    SHAPE_TYPES.some((s) => s.id === 'square'),
    'SHAPE_TYPES includes square',
  );
  assert(
    SHAPE_TYPES.some((s) => s.id === 'rectangle'),
    'SHAPE_TYPES includes rectangle',
  );
}
