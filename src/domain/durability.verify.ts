 ...
 */
import { deriveDurability, isSoleReplicaHolder, type DurabilitySignals } from './durability.ts';
...
console.log('=== A failure outranks every reassuring signal ===');
{
  ...
  assert(withQuota.persistent, 'and it cannot be dismissed - the user has to act on it');
  assert(
...
{
  assert(deriveDurability(base).level === 'local', 'browser storage alone reports local');
  assert(
...
console.log("=== Offline queue is described in the user's terms ===");
{
  ...
  assert(!/\d/.test(none.detail), 'with nothing queued, no count is invented');
}

console.log('=== Replica count (WS13-R9) ===');
{
  const alone = deriveDurability({ ...base, replicaCount: 1 });
  assert(/only person/i.test(alone.detail), 'being the sole holder is stated plainly');
...
console.log('=== WS13-R8/R13: capability x permission x attachment matrix ===');
{
    ...
    for (const fileAttachment of attachments) {
      for (const fileBacked of [false, true]) {
        ...
        if (fileAccess !== 'available') {
          ...
          assert(
            !/file you chose|Saved to file|plan\.json/i.test(text),
            `${label}: never implies continuous file saving`,
          );
        }
        if (fileBacked) {
        ...
        if (fileAccess === 'available' && fileAttachment?.status === 'needs-permission') {
          assert(
            state.action === 'resume-file' && /resumes once you allow/.test(state.detail),
            `${label}: one click to resume (WS13-R2)`,
          );
        }
        if (fileAccess === 'available' && fileAttachment?.status === 'denied') {
          assert(
            /declined/.test(state.detail) && /not being updated/.test(state.detail),
            `${label}: says the file is not being updated`,
          );
        }
        if (fileAccess === 'available' && fileAttachment?.status === 'conflict') {
          assert(
            state.tone === 'alert' && state.persistent && state.action === 'resolve-conflict',
            `${label}: a conflict demands a decision (WS13-R4)`,
          );
        }
      }
    }
  ...
  assert(
    deriveDurability({
      ...
      fileAttachment: { fileName: 'x', status: 'conflict' },
    }).level === 'at-risk',
