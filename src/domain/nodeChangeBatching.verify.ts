...
// === Part 1: a single mid-drag position change is queued, not committed ===
{
  ...
  const { isActiveGesture } = classifyNodeChanges(
    changes,
    pending,
    new Map<string, CurrentNodeGeometry>(),
  );

  assert(
    isActiveGesture,
    'a position change with dragging: true is correctly reported as an active, in-progress gesture',
  );
  assert(
    pending.size === 1 && pending.get('n1')?.type === 'position',
    'the position change is queued into the pending map',
  );
}
...
// === Part 2: the final event of a drag (dragging: false) is NOT an active gesture ===
{
  const pending = new Map<string, PendingNodeUpdate>();
  const changes: NodeChange[] = [
    { id: 'n1', type: 'position', position: { x: 99, y: 99 }, dragging: false },
  ];
  const { isActiveGesture } = classifyNodeChanges(
    changes,
    pending,
  ...
  assert(
    !isActiveGesture,
    'the drag-ending event (dragging: false) is correctly NOT treated as an active gesture - the caller should flush immediately, not wait for another frame',
  );
  const update = pending.get('n1');
  assert(
    update?.type === 'position' && update.position.x === 99 && update.position.y === 99,
    'the final position is still correctly queued for the (immediate) flush',
  );
}
...
// === Part 3: a standalone change with no dragging flag at all (arrow-key nudge, alignment-snap correction) is NOT an active gesture ===
{
  ...
  assert(
    !isActiveGesture,
    "a position change with no dragging flag at all (e.g. an arrow-key nudge) is treated the same as a completed gesture - commit right away, don't wait a frame for something that was never a high-frequency gesture to begin with",
  );
}
...
// === Part 4: multiple position changes for the SAME node within one batch - only the latest survives ===
{
  ...
  const update = pending.get('n1');
  assert(
    pending.size === 1 && update?.type === 'position' && update.position.x === 3,
    'multiple position changes for the same node id in one batch correctly deduplicate to only the LAST one, not three separate pending entries',
  );
}
...
// === Part 5: a multi-node drag - each node gets its own independent pending entry ===
{
  ...
  assert(
    pending.size === 3,
    'each of the three simultaneously-dragged nodes gets its own independent pending entry, not merged or overwritten',
  );
  assert(
    (pending.get('b') as { type: 'position'; position: { x: number } }).position.x === 20,
    "each node's own position is preserved correctly, not mixed up with another node's",
  );
}
...
// === Part 6: a mixed batch - if ANY change in the batch is still actively dragging, the whole batch is treated as active ===
{
  ...
  assert(
    isActiveGesture,
    "if even one change in the batch is still actively dragging, the whole batch waits for the next frame rather than committing immediately - correct, since node b's gesture is still in progress",
  );
}
...
// === Part 7: dimensions/resizing follow the identical pattern to position/dragging ===
{
  ...
  assert(
    activeDuringResize,
    'a dimensions change with resizing: true is correctly treated as an active gesture',
  );
  assert(
    pendingActive.get('n1')?.type === 'dimensions',
    'the dimensions change is correctly queued',
  );
  ...
  assert(
    !activeAfterResize,
    'the resize-ending event (resizing: false) is correctly NOT treated as an active gesture',
  );
}
...
// === Part 8: irrelevant change types (select, remove, add) are ignored entirely - no pending entry, no effect on isActiveGesture ===
{
  ...
  assert(
    !isActiveGesture,
    'a batch containing only selection/removal changes is not treated as an active drag/resize gesture',
  );
  assert(
    pending.size === 0,
    "selection and removal changes create no pending position/dimension entries at all - there's nothing for this function to do with them",
  );
}
...
// === Part 9: accumulation across multiple calls - simulating several onNodesChange invocations before a single flush, exactly as happens during a real drag ===
{
  ...
  classifyNodeChanges(
    [{ id: 'n2', type: 'position', position: { x: 100, y: 100 }, dragging: true }],
    pending,
    new Map<string, CurrentNodeGeometry>(),
  );
  ...
  assert(
    pending.size === 2,
    'three separate onNodesChange calls across two distinct nodes correctly accumulate into exactly two pending entries, not three',
  );
  assert(
    (pending.get('n1') as { type: 'position'; position: { x: number } }).position.x === 2,
    "n1's pending entry reflects its most recent call, not its first",
  );
}
...
// === Part 10: applySelectionChanges - a single select ===
{
  ...
  assert(
    result.length === 1 && result[0] === 'a',
    'a single select change correctly adds that id to an empty selection',
  );
}
...
// === Part 11: applySelectionChanges - THE EXACT REPORTED BUG: selecting node A, then selecting node B, must result in B selected, not A ===
{
  ...
  assert(
    afterClickA.length === 1 && afterClickA[0] === 'A',
    'after the first click (selecting A), A is correctly selected',
  );
...
// === Part 12: applySelectionChanges - shift-click style multi-select (add without deselecting) ===
{
  ...
  assert(
    afterShiftB.length === 2 && afterShiftB.includes('A') && afterShiftB.includes('B'),
    'a shift-click-style change (select B without deselecting A) correctly results in both being selected',
  );
}
...
{
  const afterA = applySelectionChanges([{ id: 'A', type: 'select', selected: true }], []);
  const afterDeselect = applySelectionChanges(
    [{ id: 'A', type: 'select', selected: false }],
    afterA,
  );
  assert(
    afterDeselect.length === 0,
    'deselecting the only selected node correctly results in an empty selection',
  );
}
...
// === Part 14: applySelectionChanges - idempotency (selecting an already-selected node doesn't duplicate it) ===
{
  ...
  assert(
    afterAAgain.length === 1 && afterAAgain[0] === 'A',
    "selecting an already-selected node again doesn't add a duplicate entry",
  );
}
...
{
  const current = ['A'];
  const result = applySelectionChanges(
    [{ id: 'n1', type: 'position', position: { x: 1, y: 1 }, dragging: true }],
    current,
  );
  assert(
    result === current,
    "a batch with no 'select' changes at all returns the EXACT SAME array reference, not a new (even if equal-content) one - this is what lets React's own setState bail out of an unnecessary re-render during an ordinary drag",
  );
}
...
// a self-sustaining loop needing nothing to actually change to keep running.
{
  ...
  const current = new Map<string, CurrentNodeGeometry>([
    ['n1', { position: { x: 0, y: 0 }, width: 200, height: 100, isAutoSized: false }],
  ]);
  const changes: NodeChange[] = [
    { id: 'n1', type: 'dimensions', dimensions: { width: 200, height: 100 } },
  ];
  const { isActiveGesture } = classifyNodeChanges(changes, pending, current);

  assert(
    pending.size === 0,
    "a dimensions change identical to the node's current width/height is not queued at all - this is the actual fix for the reported infinite loop, where React Flow kept re-emitting unchanged dimensions and each one was previously written to the store unconditionally",
  );
  assert(
    !isActiveGesture,
    "a no-op dimensions change (with no resizing flag) correctly doesn't count as an active gesture either",
  );
}
...
// === Part 17: a dimensions change that's genuinely DIFFERENT from current geometry is still queued normally ===
{
  ...
  const changes: NodeChange[] = [
    { id: 'n1', type: 'dimensions', dimensions: { width: 250, height: 100 }, resizing: true },
  ]; // width genuinely changed
  classifyNodeChanges(changes, pending, current);

  assert(
    pending.size === 1 && pending.get('n1')?.type === 'dimensions',
    "a dimensions change that's genuinely different from the current value (even if only one of width/height changed) is still correctly queued - the no-op guard doesn't accidentally swallow real changes",
  );
}
...
// === Part 18: the same no-op guard applies to position changes ===
{
  const pending = new Map<string, PendingNodeUpdate>();
  const current = new Map<string, CurrentNodeGeometry>([
    ['n1', { position: { x: 50, y: 75 }, isAutoSized: false }],
  ]);
  ...
  assert(
    pending.size === 0,
    "a position change identical to the node's current position is not queued at all",
  );
  ...
  assert(
    pending.size === 1,
    "a position change that's genuinely different (even by one pixel) is still correctly queued",
  );
}
...
// === Part 19: a change for a node with NO entry in currentNodes at all (e.g. brand new node) is queued - there's nothing to compare against, so it can never be treated as a no-op ===
{
  ...
  const changes: NodeChange[] = [
    { id: 'n1', type: 'dimensions', dimensions: { width: 100, height: 50 }, resizing: true },
  ];
  classifyNodeChanges(changes, pending, current);

  assert(
    pending.size === 1,
    "a change for a node with no current-geometry entry at all is still queued normally - there's no known 'current' value to compare against, so it can never be mistaken for a no-op",
  );
}
...
// === Part 21: a mixed batch - some no-op, some genuinely changed - only the real changes are queued ===
{
  ...
  assert(
    pending.size === 1 && pending.has('resized') && !pending.has('unchanged'),
    'in a mixed batch, only the node that genuinely changed size is queued - the unchanged one is correctly filtered out without affecting the other',
  );
}
...
// still positioned against. Each client measures this for itself.
{
  ...
  const changes: NodeChange[] = [
    { id: 'auto', type: 'dimensions', dimensions: { width: 260, height: 126 } },
  ];
  const { isActiveGesture } = classifyNodeChanges(changes, pending, current);

  assert(
    pending.size === 0,
    "a content-sized node's measured dimensions are never queued for the store, even though they differ from its (absent) explicit width/height - this is the fix for connectors and peer selection outlines rendering at the wrong size on other clients",
  );
  assert(
    !isActiveGesture,
    "a dropped content-sized dimensions change doesn't count as an active gesture either",
  );
}
...
// collaborator does need to see.
{
  ...
  const changes: NodeChange[] = [
    { id: 'grp', type: 'dimensions', dimensions: { width: 520, height: 300 }, resizing: true },
  ];
  const { isActiveGesture } = classifyNodeChanges(changes, pending, current);

  assert(
    pending.get('grp')?.type === 'dimensions',
    "a resizable node's genuine resize is still queued normally",
  );
  assert(
    isActiveGesture,
    'an in-progress resize of a resizable node is still reported as an active gesture',
  );
}
...
// failed until they were given the flag a user resize actually carries.
{
  const pending = new Map<string, PendingNodeUpdate>();
  const current = new Map<string, CurrentNodeGeometry>([
    ['n1', { position: { x: 0, y: 0 }, width: 100, height: 50, isAutoSized: false }],
  ]);
  const changes: NodeChange[] = [
    { id: 'n1', type: 'dimensions', dimensions: { width: 400, height: 300 } },
  ];
  classifyNodeChanges(changes, pending, current);
  assert(
    pending.size === 0,
    'a passive measurement is not queued even when it differs from the stored size',
  );
}
