import { globalIconRegistry, sanitizeSvg, isValidSvg, type IconDefinition } from './iconRegistry';
...
console.log('=== 1. Testing Built-in Icons Registration & Retrieval ===');
{
  ...
  assert(
    server?.category === 'Compute & Hardware',
    'Server icon is categorized under Compute & Hardware',
  );
...
console.log('\n=== 2. Testing SVG Sanitization & Security ===');
{
  ...
  assert(
    cleaned.includes('<circle cx="50" cy="50" r="40" />'),
    'Sanitization preserves safe vector geometry',
  );

  assert(isValidSvg(cleaned), 'Sanitized SVG is valid SVG');
  assert(!isValidSvg('not an svg'), 'Non-SVG string rejected by isValidSvg');
  assert(
    !isValidSvg('<svg><script>bad</script></svg>'),
    'Raw SVG with script tag rejected by isValidSvg',
  );
  ...
  assert(
    !sanitizedEvasive.toLowerCase().includes('<script'),
    'Strips script tags with whitespace (</script >)',
  );
  ...
  assert(
    !sanitizedEvasive.toLowerCase().includes('onerror'),
    'Strips unquoted event handlers (onerror=)',
  );
  ...
  assert(
    sanitizedEvasive.includes('<circle cx="20" cy="20" r="10"'),
    'Preserves circle in evasive test',
  );
}
...
console.log('\n=== 3. Testing Custom Icon Registration & Search ===');
{
  ...
  assert(
    searchResults.some((i) => i.id === 'aws.lambda'),
    "Custom icon found when searching by tag 'serverless'",
  );

  const catResults = globalIconRegistry.getIconsByCategory('Cloud / AWS');
  assert(
    catResults.some((i) => i.id === 'aws.lambda'),
    "Custom icon found under category 'Cloud / AWS'",
  );

  globalIconRegistry.unregisterLibraryIcons('aws-icons');
  assert(
    globalIconRegistry.getIcon('aws.lambda') === undefined,
    'Unregistering library icons removes them from registry',
  );
}
