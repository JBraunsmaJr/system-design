...
const milestones: Milestone[] = [
  ...
  // Release milestone linked to TICKET-1, TICKET-2, TICKET-3
  {
    ...
    relatedItemIds: ['TICKET-1', 'TICKET-2', 'TICKET-3'],
  },
];
...
// --- Test 1: Direct Milestone Aggregation (FR-007, Scenario 4) ---
{
  ...
  assert(
    summary1.directMilestones[0]?.id === 'm-freeze',
    'Sprint 1 direct milestone is Alpha Freeze',
  );
}
...
// --- Test 2: Impacted Release Aggregation (FR-007) ---
{
  ...
  assert(
    relInfo?.relatedItemsInSprint.length === 2,
    'Sprint 1 contains 2 items linked to Release 2.4',
  );
  assert(relInfo?.totalRelatedItemsCount === 3, 'Release 2.4 has 3 total related items');
  assert(
    relInfo?.isFullyContainedInSprint === false,
    'Release 2.4 is not fully contained in Sprint 1',
  );
...
// --- Test 4: Capacity Workload Invariance (DR-001, FR-008, AC-007) ---
{
  ...
  assert(
    capacitySummary.totalAssignedPoints === 8,
    'total assigned capacity points strictly equals ticket points (5 + 3 = 8)',
  );
}
