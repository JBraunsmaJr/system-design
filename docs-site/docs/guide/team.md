...
2. **Name & Role**: Enter the member's full name and optional engineering role (e.g., _Frontend Engineer_, _Staff Architect_, _QA Lead_).
...
4. **Notes**: Add optional context (e.g., _Vacation_, _Training_, _Conference_).
5. Click **Record PTO**.
...
- **US Federal Holidays**:
  ...
  - Click **View preview** to inspect the full list of holidays calculated for the active year (e.g., _New Year's Day_, _MLK Day_, _Memorial Day_, _Juneteenth_, _Independence Day_, _Labor Day_, _Thanksgiving_, _Christmas_).
...
2. Enter the holiday name (e.g., _Company Summit_, _Winter Shutdown Day 1_), the calendar date, and optional notes.
...
- **Utilization Status**: Visual progress bar indicating load percentage (e.g., _Optimal_, _Under-allocated_, or _Over-allocated_).
