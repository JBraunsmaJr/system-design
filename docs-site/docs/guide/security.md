...
<svg class="kd" viewBox="0 0 680 560" role="img" aria-label="Key hierarchy: the device key unwraps the user key, which unwraps the workspace key, which unwraps the document key, from which the storage key is derived to open sealed document content.">
  ...
  <text class="t" x="510" y="504" text-anchor="middle" dominant-baseline="central">Organisation's recovery key</text>
...
development, or where an organisation has decided the server should read
...
<svg class="kd" viewBox="0 0 680 270" role="img" aria-label="Three recovery routes: another approved browser, your recovery code, or the organisation's offline key — each leading back to readable documents.">
  ...
  <text class="ts" x="555" y="64" text-anchor="middle" dominant-baseline="central">Held by the organisation</text>
...
| Offline recovery key | The organisation, not you | Someone has left; a records request |
...
nobody — including the organisation that owns it — could ever recover. The
...
- **A lost recovery code with no devices left.** That is an administrator
  ...
  unless the organisation's offline key recovers them.
