...
async function run(
  name: string,
  backend: { store: StoreBackend; index: ReturnType<typeof createMemoryWorkspaceIndex> },
) {
  console.log(`\n########## ${name} ##########`);
  ...
  try {
    ...
    await fetch(`${origin}/v1/docs`, {
      ...
      body: JSON.stringify({
        docId,
        keys: {
          wrappedForWorkspace: 'd3JhcA==',
          wrappedForRecovery: await escrowDocumentKey(documentKey, recoveryPem),
        },
      }),
    });
    ...
    const aliceSync = createDocumentSync({
      client: alice,
      docId,
      documentKey,
      doc: aliceDoc,
    ...
    const bobSync = createDocumentSync({
      client: bob,
      docId,
      documentKey,
      doc: bobDoc,
    ...
    check(
      JSON.stringify([...nodesOf(aliceDoc).keys()].sort()) ===
        JSON.stringify([...nodesOf(bobDoc).keys()].sort()),
    ...
    const carolSync = createDocumentSync({
      client: carol,
      docId,
      documentKey,
      doc: carolDoc,
    ...
    check(
      JSON.stringify([...nodesOf(carolDoc).keys()].sort()) ===
        JSON.stringify([...nodesOf(aliceDoc).keys()].sort()),
    ...
    const bobAgain = createDocumentSync({
      client: bob,
      docId,
      documentKey,
      doc: bobDoc,
    ...
    console.log('  -- leaving straight after an edit');
    {
      // The last change before a page goes away would otherwise wait out
      // the batching delay and never be sent.
      const leaver = createStoreClient({ baseUrl: origin });
      const leaverDoc = new Y.Doc();
      const leaverSync = createDocumentSync({
        client: leaver,
        docId,
        documentKey,
        doc: leaverDoc,
      ...
      await leaverSync.start();
      nodesOf(leaverDoc).set('last-second', { label: 'Typed as the tab closed' });
      leaverSync.flushOnExit();
      leaverSync.stop();
      await sleep(600);
      const reader = new Y.Doc();
      const readerSync = createDocumentSync({
        client: createStoreClient({ baseUrl: origin }),
        docId,
        documentKey,
        doc: reader,
        pollMs: 60_000,
      });
      await readerSync.start();
      check(
        nodesOf(reader).has('last-second'),
        'a change made just before leaving reaches the workspace anyway',
      );
      readerSync.stop();
    }
    {
      // Refused because someone else wrote first: kept, and sent next time.
      const racer = createStoreClient({ baseUrl: origin });
      const racerDoc = new Y.Doc();
      const racerSync = createDocumentSync({
        client: racer,
        docId,
        documentKey,
        doc: racerDoc,
      ...
      await racerSync.start();
      // Someone else writes, so this browser's idea of the version is stale.
      nodesOf(aliceDoc).set('someone-else', { label: 'Written meanwhile' });
      await aliceSync.flush();
      nodesOf(racerDoc).set('refused-at-exit', { label: 'Refused on the way out' });
      racerSync.flushOnExit();
      racerSync.stop();
      await sleep(600);
      // The same browser comes back, with its document restored from its
      // own storage as it would be.
      const again = createDocumentSync({
        client: racer,
        docId,
        documentKey,
        doc: racerDoc,
      ...
      await again.start();
      await again.flush();
      const checker = new Y.Doc();
      const checkerSync = createDocumentSync({
        client: createStoreClient({ baseUrl: origin }),
        docId,
        documentKey,
        doc: checker,
      ...
      await checkerSync.start();
      check(
        nodesOf(checker).has('refused-at-exit'),
        'a change refused on the way out is sent on the next visit, not lost',
      );
      check(
        nodesOf(checker).has('someone-else'),
        'and nothing the other person wrote is lost either',
      );
      again.stop();
      checkerSync.stop();
    }
    ...
    check(
      held.length <= 3,
      `the store holds a short log rather than every edit ever (${held.length} blobs)`,
    );
    const afterCompaction = new Y.Doc();
    const reader = createDocumentSync({
      client: createStoreClient({ baseUrl: origin }),
      docId,
      documentKey,
      doc: afterCompaction,
    ...
    await reader.start();
    check(
      nodesOf(afterCompaction).has('alice') &&
        nodesOf(afterCompaction).has('bob-offline') &&
        nodesOf(afterCompaction).has('extra-4'),
      'and a browser opening it afterwards still sees everything',
    );
    compacting.stop();
    reader.stop();
    ...
    check(
      !served.includes('Ledger') && !served.includes('Fraud checks'),
      'none of it in the clear',
    );
...
if (DATABASE_URL) {
  ...
  await run('postgres', {
    ...
    index: createPostgresWorkspaceIndex(pool) as never,
  });
  for (const record of await store.list({ includeDeleted: true })) await store.purge(record.docId);
  await pool.end();
  await store.close();
