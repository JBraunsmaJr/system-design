...
async function run() {
  ...
  try {
    ...
    await page1.waitForFunction("typeof window.__PERF__?.loadFixture === 'function'", null, {
      timeout: 15000,
    });
    ...
    const toastVisible = await page1
      .waitForSelector('.app-toast', { timeout: 2000 })
      .then(() => true)
      .catch(() => false);
    if (!toastVisible) {
    ...
    if (
      !copiedLink.includes('session=') ||
      !copiedLink.includes('key=') ||
      !copiedLink.includes('relay=')
    ) {
    ...
    if (nodesCount1 !== nodesCount2 || nodesCount2 === 0) {
      throw new Error(
        `Sync failed between Page 1 and Page 2: Page1=${nodesCount1}, Page2=${nodesCount2}`,
      );
    }
    ...
    if (copiedLinkPage2 !== copiedLink) {
      throw new Error(
        `Link copied by participant (Page 2) differs from host (Page 1): ${copiedLinkPage2} vs ${copiedLink}`,
      );
    }
    ...
    if (
      page3Url.includes('session=') ||
      page3Url.includes('key=') ||
      page3Url.includes('relay=') ||
      page3Url.includes('#')
    ) {
    ...
    console.log(
      `Node counts after Page 3 joins via URL: Page1=${nodesCount1}, Page3=${nodesCount3}`,
    );
