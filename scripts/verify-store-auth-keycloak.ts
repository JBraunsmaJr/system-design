...
const standIn = external
  ? null
  : await startTestOidcProvider({ subject: 'tester', loginForm: true });
const issuer = external ?? standIn!.issuer;
if (standIn) {
  console.log(
    'KEYCLOAK_ISSUER is not set: running against a stand-in provider that serves a login form.',
  );
}
...
try {
  ...
  check(
    typeof (discovery as { authorization_endpoint?: string }).authorization_endpoint === 'string',
    'Keycloak publishes a discovery document',
  );
  check(
    /\/protocol\/openid-connect\/auth$/.test(
      (discovery as { authorization_endpoint: string }).authorization_endpoint,
    ),
  ...
  for (let attempt = 0; attempt < 3 && !backAtStore(); attempt++) {
    await page.waitForURL((url) => url.href.startsWith(origin), { timeout: 15000 }).catch(() => {});
  ...
  if (!backAtStore()) {
    ...
    check(
      false,
      `the sign-in came back to the store (stuck at ${page.url()} - "${heading ?? 'no heading'}"${error ? `: ${error}` : ''})`,
    );
  }
  ...
  check(
    session?.issuer === issuer.replace(/\/+$/, ''),
    'the session records Keycloak as the issuer',
  );
  ...
  if (external) {
    ...
    check(
      ...
      `and with Keycloak that is its own identifier, not the username (${session?.subject})`,
    );
  }
