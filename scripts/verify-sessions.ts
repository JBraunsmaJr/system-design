...
async function contract(
  ...
  check(
    created.issuer === IDENTITY.issuer && created.subject === 'alice',
    'it records (issuer, subject), not an email',
  );
  ...
  check(
    (await sessions.get(created.id))?.userId === 'user-1',
    'the directory user is remembered with it',
  );
  ...
  check(
    (await sessions.take('state-1')) === null,
    'and only once: a state that returns twice is a replay',
  );
...
if (DATABASE_URL) {
  ...
  console.log('\n########## what a database is for ##########');
  {
    ...
    check(
      (await afterRestart.get(session.id))?.subject === 'alice',
      'a session survives the process that created it',
    );
    const secondInstance = createPostgresSessionStore(pool);
    await secondInstance.destroy(session.id);
    check(
      (await first.get(session.id)) === null,
      'and signing out on one instance ends it on the other',
    );

    await first.remember(pending('shared-state'));
    check(
      (await secondInstance.take('shared-state')) !== null,
      'a sign-in started on one instance can complete on another',
    );
