...
{
  const full = parsePresenceState(7, {
    name: 'Alice',
    color: '#5b7cfa',
    cursor: { x: 10, y: 20 },
    selectedNodeIds: ['n1'],
  ...
  assert(
    parsePresenceState(1, { color: '#5b7cfa' }) === null,
    "a state missing name entirely is rejected outright - there's no genuine peer identity to show without it",
  );
  assert(
    parsePresenceState(1, { name: 'Alice' }) === null,
    'a state missing color entirely is rejected outright, for the same reason',
  );
  assert(
    parsePresenceState(1, null) === null,
    'a null state (no awareness entry at all) is rejected outright',
  );
  assert(
    parsePresenceState(1, { name: 123, color: '#5b7cfa' }) === null,
    "a name that isn't actually a string is rejected, not coerced",
  );

  const noCursorField = parsePresenceState(1, { name: 'Bob', color: '#000' });
  assert(
    noCursorField !== null && noCursorField.cursor === null,
    "a valid name/color with no cursor field at all still parses successfully, defaulting cursor to null rather than being rejected - a peer's state can be legitimately incomplete right after joining",
  );

  const malformedCursor = parsePresenceState(1, {
    name: 'Bob',
    color: '#000',
    cursor: { x: 'not a number', y: 5 },
  });
  assert(
    malformedCursor !== null && malformedCursor.cursor === null,
    "a malformed cursor (x isn't actually a number) doesn't reject the whole peer - it's defaulted to null while name/color still come through correctly",
  );

  const noSelections = parsePresenceState(1, { name: 'Bob', color: '#000' });
  assert(
    noSelections !== null &&
      Array.isArray(noSelections.selectedNodeIds) &&
      noSelections.selectedNodeIds.length === 0 &&
      Array.isArray(noSelections.selectedEdgeIds) &&
      noSelections.selectedEdgeIds.length === 0,
    'missing selectedNodeIds/selectedEdgeIds default to empty arrays rather than being undefined or rejecting the peer',
  );

  const malformedSelections = parsePresenceState(1, {
    name: 'Bob',
    color: '#000',
    selectedNodeIds: 'not-an-array',
  });
  assert(
    malformedSelections !== null &&
      Array.isArray(malformedSelections.selectedNodeIds) &&
      malformedSelections.selectedNodeIds.length === 0,
    "a selectedNodeIds that isn't actually an array is defaulted to empty rather than passed through as-is or rejecting the peer",
  );

  const withViewAndFocus = parsePresenceState(1, {
    name: 'Bob',
    color: '#000',
  ...
  assert(
    withViewAndFocus !== null &&
      withViewAndFocus.viewMode === 'requirements' &&
      withViewAndFocus.focusedItemId === 'REQ-5',
    'viewMode and focusedItemId are correctly carried over when present and valid',
  );

  const noViewOrFocus = parsePresenceState(1, { name: 'Bob', color: '#000' });
  assert(
    noViewOrFocus !== null &&
      noViewOrFocus.viewMode === null &&
      noViewOrFocus.focusedItemId === null,
    "missing viewMode/focusedItemId default to null rather than being undefined or rejecting the peer - matches a peer on a view that doesn't track this (diagram/team/skill-tree), or the brief window before their first full broadcast",
  );

  const malformedView = parsePresenceState(1, {
    name: 'Bob',
    color: '#000',
    viewMode: 42,
    focusedItemId: { not: 'a string' },
  });
  assert(
    malformedView !== null &&
      malformedView.viewMode === null &&
      malformedView.focusedItemId === null,
    "a viewMode/focusedItemId that aren't actually strings are defaulted to null rather than passed through as-is or rejecting the peer",
  );

  const withPath = parsePresenceState(1, {
    name: 'Bob',
    color: '#000',
    diagramPath: 'node-a/node-b',
  });
  assert(
    withPath !== null && withPath.diagramPath === 'node-a/node-b',
    'diagramPath is correctly carried over when present and valid',
  );

  const noPath = parsePresenceState(1, { name: 'Bob', color: '#000' });
  assert(
    noPath !== null && noPath.diagramPath === '',
    'a missing diagramPath defaults to an empty string (root level) rather than being undefined or rejecting the peer',
  );

  const malformedPath = parsePresenceState(1, { name: 'Bob', color: '#000', diagramPath: 42 });
  assert(
    malformedPath !== null && malformedPath.diagramPath === '',
    "a diagramPath that isn't actually a string is defaulted to empty (root) rather than passed through as-is or rejecting the peer",
  );
}
...
try {
  ...
  const sessionA = startCollabSession(new Y.Doc(), 'presence-verify-room-a', {
    signalingUrls: [`ws://localhost:${PORT}`],
  });
  const sessionB = startCollabSession(new Y.Doc(), 'presence-verify-room-b', {
    signalingUrls: [`ws://localhost:${PORT}`],
  });
  ...
  assert(
    seenByA1.length === 1 && seenByA1[0].length === 0,
    'subscribeToPresence fires immediately with an empty list before anyone (including this peer) has set any presence',
  );

  sessionA.setLocalPresence({
    name: 'Alice',
    color: '#5b7cfa',
    cursor: null,
    selectedNodeIds: [],
  ...
  assert(
    seenByA1[seenByA1.length - 1].length === 0,
    "setting THIS peer's own presence does not appear in ITS OWN subscribeToPresence feed - seeing yourself in a 'who else is here' list would be redundant",
  );
  unsubA();
  ...
  const unsubCheckGarbage = sessionB.subscribeToPresence((peers) => {
    if (
      peers.some(
        (p) => (p as unknown as { someUnrelatedField?: number }).someUnrelatedField !== undefined,
      )
    ) {
      sawGarbagePeer = true;
    }
  ...
  const updateFromA = encodeAwarenessUpdate(sessionA.provider.awareness, [
    sessionA.provider.awareness.clientID,
  ]);
  applyAwarenessUpdate(sessionB.provider.awareness, updateFromA, 'test-sync');

  assert(
    !sawGarbagePeer,
    'an awareness entry without a valid name/color is never surfaced through subscribeToPresence, even once real sync has happened',
  );
  unsubCheckGarbage();
  ...
  const updatedFromA = encodeAwarenessUpdate(sessionA.provider.awareness, [
    sessionA.provider.awareness.clientID,
  ]);
  ...
  assert(
    true,
    'an existing subscription picks up a presence UPDATE (not just the initial appearance) without needing to resubscribe',
  );
