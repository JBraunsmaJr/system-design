...
const serverWithUrl = createHttpService({
  store: newStore(),
  audit,
  sessions,
  providers,
...
await new Promise<void>((resolve) =>
  serverWithUrl.listen(Number(new URL(origin).port), '127.0.0.1', resolve),
);
...
async function signIn(
  provider: string,
): Promise<{ cookie: string | null; setCookie: string | null; finalLocation: string | null }> {
...
try {
  console.log('=== A store with no sign-in configured ===');
  {
    ...
    check(
      !!createHttpService({ store: newStore(), allowUnauthenticated: true }),
      'starts unauthenticated only when the caller says so',
    );
  }
  ...
  console.log('\n=== Before signing in ===');
  {
    ...
    check(
      (await (await call('/v1/auth/session')).json()).error?.reason === 'unauthenticated',
      'and there is no session to report',
    );
    const listed = await (await call('/v1/auth/providers')).json();
    check(
      JSON.stringify(listed.providers) === JSON.stringify(['oidc', 'github']),
      'the configured providers are advertised',
    );
  ...
  console.log('\n=== Signing in with OIDC ===');
  {
    ...
    check(
      ...
      'and a state and nonce',
    );
    check(
      !authorizeUrl.searchParams.get('client_secret'),
      'and no client secret, which stays on the server',
    );
    ...
    check(
      ...
      'HttpOnly and SameSite=Lax, so page scripts cannot read it and another site cannot use it',
    );
    check(
      !/Secure/i.test(setCookie ?? ''),
      'not Secure here, because this deployment is plain http for development',
    );
    check(
      finalLocation === '/app',
      'and the browser is sent on to the application, not left on the store',
    );
    check(
      !(cookie ?? '').includes('alice'),
      'the cookie is opaque: it carries no identity of its own',
    );

    const session = await (await call('/v1/auth/session', cookie)).json();
    check(
      session.session?.subject === 'alice' && session.session?.issuer === idp.issuer,
      'the session reports (issuer, subject), not an email address',
    );
    ...
    check(
      (await call('/v1/docs', cookie)).status === 401,
      'and the cookie stops working immediately',
    );
  }
  ...
  {
    check(
      (await call('/v1/auth/callback?state=made-up&code=x')).status === 400,
      'a callback for a sign-in that never started is refused',
    );
    const start = await call('/v1/auth/oidc/start');
    const authorizeUrl = start.headers.get('location') ?? '';
    const atProvider = await fetch(authorizeUrl, { redirect: 'manual' });
    const callbackUrl = atProvider.headers.get('location') ?? '';
    check(
      (await fetch(callbackUrl, { redirect: 'manual' })).status === 302,
      'a genuine callback succeeds',
    );
    check(
      (await fetch(callbackUrl, { redirect: 'manual' })).status === 400,
  ...
  console.log('\n=== Signing in with GitHub ===');
  {
    ...
    const session = await (await call('/v1/auth/session', cookie)).json();
    check(
      session.session?.subject === '99001',
      "the subject is GitHub's numeric account id, not the login name that can be reused",
    );
    ...
    check(
      otherSession.session?.subject === '12345',
      'a different account with the same login is a different subject',
    );
  }
  ...
  console.log('\n=== Token verification, directly ===');
  {
    ...
    check(
      reasonOf(() => verifyIdToken('not.a.token', { issuer: 'i', audience: 'a', keys: [] })) ===
        'malformed',
      'a malformed token is named as such',
    );
    check(
      reasonOf(() =>
        verifyIdToken(`${Buffer.from('{"alg":"HS256"}').toString('base64url')}.e30.x`, {
          issuer: 'i',
          audience: 'a',
          keys: [],
        }),
  ...
  console.log('\n=== A provider the browser and the store reach differently ===');
  {
    ...
    const behindNetwork = await startTestOidcProvider({
      subject: 'split',
      publicIssuer: 'http://provider.public.invalid',
    });
    ...
    try {
      ...
      const asBrowser = authorizeUrl.replace(
        'http://provider.public.invalid',
        behindNetwork.issuer,
      );
      ...
      check(
        !!cookie,
        'and the callback completes: the token exchange went to the address the store can reach',
      );
      const session = await (
        await fetch(`${splitOrigin}/v1/auth/session`, { headers: { cookie } })
      ).json();
      check(
        session.session?.issuer === 'http://provider.public.invalid',
        'the session records the public issuer, which is what tokens claim',
      );
    } finally {
  ...
  console.log('\n=== A provider the store cannot reach at all ===');
  {
    ...
    const unreachable = createHttpService({
      ...
      providers: [
        createProvider({
          id: 'oidc',
          kind: 'oidc',
          issuer: 'http://127.0.0.1:9/realms/x',
          clientId: 'c',
          clientSecret: 's',
        }),
      ],
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${port}/v1/auth/oidc/start`, {
        redirect: 'manual',
      });
      ...
      check(
        (body.error?.message ?? '').includes('127.0.0.1:9') &&
          /localhost is the container itself/.test(body.error?.message ?? ''),
  ...
  console.log('\n=== The audit trail (WS10-R3) ===');
  {
    ...
    check(
      entries.some(
        (e) => e.operation === 'login' && e.outcome === 'ok' && e.subject?.includes('alice'),
      ),
      'a successful sign-in is recorded with its subject',
    );
    check(
      entries.some((e) => e.operation === 'login' && e.outcome === 'denied'),
      'and so is a refused one',
    );
    check(
      entries.some((e) => e.operation === 'logout'),
    ...
    check(
      entries.some(
        (e) =>
          e.outcome === 'denied' &&
          (e.detail as { reason?: string } | undefined)?.reason === 'unauthenticated',
      ),
      'and so is a request made without a session, with the reason',
    );
    check(
      !entries.some((e) => JSON.stringify(e).includes('gho_test')),
      'no token or secret appears in the trail',
    );
  }
} finally {
