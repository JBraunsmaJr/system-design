...
async function rejects(work: () => Promise<unknown>, reason: string, message: string) {
  try {
    ...
    const actual =
      error instanceof StoreError ? error.reason : `other: ${String(error).slice(0, 60)}`;
    if (actual !== reason) {
      failures++;
...
async function runContract() {
  console.log('=== Documents and opaque blobs (WS8-R1) ===');
  {
    ...
    check(
      created.version === 1 && created.blobs.length === 0,
      'a new document starts at version 1 with no blobs',
    );
    await rejects(
      () => service.create({ docId: 'doc-a', keys: KEYS }),
      'conflict',
      'creating it twice is a conflict',
    );
    await rejects(
      () => service.head('doc-missing'),
      'not-found',
    ...
    check(
      read.blobs.length === 1 &&
        read.blobs[0].bytes.every((b, i) => b === sealed[i]) &&
        read.blobs[0].bytes.length === sealed.length,
      'a blob comes back byte for byte, including bytes that are not valid text',
    );
    check(
      read.record.keys.wrappedForRecovery === KEYS.wrappedForRecovery,
      'the wrapped keys are stored and returned unchanged',
    );
  }
  ...
  console.log('\n=== Versions only ever move forward (WS8-R15) ===');
  {
    ...
    check(
      ...
      `each write moves the version forward (${first.version} -> ${second.version})`,
    );
    ...
    check(
      deleted.version > second.version,
      'deleting moves the version too, so a rolled-back deletion is detectable',
    );
  ...
  console.log('\n=== A failed write leaves nothing behind (WS8-R16) ===');
  {
    ...
    for (const [when, injected] of injections) {
      ...
      check(
        after.blobs.length === before.blobs.length,
        'and no metadata points at a blob that was never written',
      );
    }
  ...
  console.log('\n=== Compaction, delete, restore, purge ===');
  {
    ...
    for (let i = 0; i < 20; i++)
      await service.append({ docId: 'doc-c', kind: 'update', bytes: bytes(`update ${i}`) });
    const before = await service.head('doc-c');
    const compacted = await service.compact('doc-c', {
      kind: 'snapshot',
      bytes: bytes('the whole document'),
    });
    check(
      compacted.blobs.length === 1 && compacted.blobs[0].kind === 'snapshot',
      'compaction leaves one snapshot',
    );
    check(
      compacted.docId === before.docId && compacted.version > before.version,
      'the document keeps its identity, and its version moves on (WS8-R5)',
    );
    check(
      (await blobs.totalBytes('doc-c')) === bytes('the whole document').length,
      'and the superseded blobs are gone',
    );
    ...
    check(
      (await service.list()).length === 0 &&
        (await service.list({ includeDeleted: true })).length === 1,
      'it is absent from listings but present for an administrator (WS9-R6)',
    );
    check(
      (await blobs.totalBytes('doc-c')) > 0,
      'its content is retained, because deletion is soft (WS10-R4)',
    );
    await service.restore('doc-c');
    check(
      (await service.read('doc-c')).blobs.length === 1,
      'restoring returns it with its content intact',
    );
    ...
    check(
      (await blobs.totalBytes('doc-c')) === 0 &&
        (await service.list({ includeDeleted: true })).length === 0,
      'purging removes everything, irreversibly',
    );
    await rejects(
      () => service.head('doc-c', { includeDeleted: true }),
      'not-found',
  ...
  console.log('\n=== Size and latency (WS8-R17) ===');
  {
    ...
    for (const blobCount of [10, 100, 1000]) {
      ...
      for (let i = 0; i < blobCount; i++) {
        const [, ms] = await timed(
          () => service.append({ docId: 'doc-n', kind: 'update', bytes: sealedUpdate }),
          1,
        );
        appendTotal += ms;
      }
      ...
      const [, compactMs] = await timed(
        () => service.compact('doc-n', { kind: 'snapshot', bytes: snapshot }),
        1,
      );
    ...
    for (const documentCount of [10, 500]) {
      ...
      rows.push(
        `  ${String(documentCount).padStart(5)} docs  | list ${listMs.toFixed(1).padStart(7)} ms | ${listed.length} entries`,
      );
    }
    ...
    check(
      openWorst < BUDGET_MS,
      `opening a 1,000-blob document stays within ${BUDGET_MS} ms (${openWorst.toFixed(1)} ms)`,
    );
  }
}
