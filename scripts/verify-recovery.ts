...
try {
  ...
  check(
    !asStored.includes('Payments platform') && !asStored.includes('Gateway'),
    'the package holds no readable content',
  );
  check(
    !!pkg.keys.wrappedForWorkspace && !!pkg.keys.wrappedForRecovery,
    "it carries both wraps: the workspace key's and the escrow (WS7-R4)",
  );

  const everyday = await openDocumentPackage(pkg, workspaceKey);
  check(
    new TextDecoder().decode(everyday[0].data) === JSON.stringify(DOCUMENT),
    'a member with the workspace key opens it',
  );

  console.log('\n=== The workspace key is gone ===');
  const strangerWorkspaceKey = await generateWorkspaceKey();
  await rejects(
    () => openDocumentPackage(pkg, strangerWorkspaceKey),
    'another workspace key does not open it',
  );

  console.log('\n=== Recovery, in this process ===');
  let recoveredHere: { kind: string; data: Uint8Array }[] | null = null;
  try {
    recoveredHere = await recoverDocumentPackage(
      pkg,
      await importRecoveryPrivateKey(fromPem(recoveryPem)),
    );
  } catch (error) {
  ...
  check(
    recoveredHere !== null &&
      new TextDecoder().decode(recoveredHere[0].data) === JSON.stringify(DOCUMENT),
    'the recovery key alone recovers the content',
  );
  const strangerPair = await generateWrappingKeyPair('recovery');
  const strangerKey = await importRecoveryPrivateKey(
    await exportPrivateKey(strangerPair.privateKey),
  );
  await rejects(
    () => recoverDocumentPackage(pkg, strangerKey),
    "another organization's recovery key does not",
  );
  ...
  const run = (args: string[]) =>
    spawnSync(
      process.execPath,
      [join('node_modules', 'tsx', 'dist', 'cli.mjs'), 'scripts/recover-document.ts', ...args],
      {
        encoding: 'utf8',
      },
    );
  ...
  check(
    ok.status === 0,
    `the tool exits successfully (${ok.status}${ok.status === 0 ? '' : `: ${ok.stderr.slice(0, 200)}`})`,
  );
  check(ok.stdout.trim() === outPath, 'and prints where it wrote the document');
  const recovered = JSON.parse(readFileSync(outPath, 'utf8'));
  check(
    JSON.stringify(recovered) === JSON.stringify(DOCUMENT),
    'the recovered file is the original document, byte for byte',
  );
  check(
    recovered.title === 'Payments platform' && recovered.nodes.length === 2,
    'with its title and contents intact',
  );
  ...
  const wrongKey = run([
    ...
    packagePath,
    '--out',
    join(workDir, 'no.json'),
    '--quiet',
  ]);
  check(
    wrongKey.status === 1 && /does not match/.test(wrongKey.stderr),
    'the wrong recovery key fails with an explanation, not a stack trace',
  );
  ...
  check(
    nonsense.status === 1 && /not valid JSON/.test(nonsense.stderr),
    'a file that is not a package says so plainly',
  );

  const help = run(['--help']);
  check(
    help.status === 0 && /recovery private key/.test(help.stderr + help.stdout),
    '--help explains what the key is',
  );
  ...
  check(
    noEscrow.keys.wrappedForRecovery === undefined,
    'a package without escrow simply has no recovery wrap',
  );
  const recoveryKey = await importRecoveryPrivateKey(fromPem(recoveryPem));
  await rejects(
    () => recoverDocumentPackage(noEscrow, recoveryKey),
    'and recovery says so rather than failing obscurely',
  );
} finally {
