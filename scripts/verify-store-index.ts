...
async function run(name: string, workspaceIndex: WorkspaceIndexStore) {
  ...
  const seal = async (entries: IndexEntry[]) =>
    Buffer.from(
      await crypto.seal(CONTEXT, new TextEncoder().encode(JSON.stringify(entries)), indexKey),
    ).toString('base64');
  const open = async (sealed: string): Promise<IndexEntry[]> =>
    JSON.parse(
      new TextDecoder().decode(
        await crypto.open(CONTEXT, Uint8Array.from(Buffer.from(sealed, 'base64')), indexKey),
      ),
    );

  const get = async () =>
    (await (await fetch(`${origin}/v1/workspaces/${WORKSPACE}/index`)).json()) as {
      index: string | null;
      version: number | null;
    };
  ...
  try {
    ...
    check(
      empty.index === null && empty.version === null,
      'has no index yet, and says so rather than failing',
    );
    ...
    const entries: IndexEntry[] = [
      {
        docId: 'doc-a',
        wrappedDocKey: 'd3JhcHBlZC1h',
        title: 'Payments platform',
        updatedAt: '2026-09-17T10:00:00Z',
      },
      {
        docId: 'doc-b',
        wrappedDocKey: 'd3JhcHBlZC1i',
        title: 'Fraud review',
    ...
    check(
      listed.length === 2 && listed[0].title === 'Payments platform',
      'listing the workspace is one fetch and one decrypt',
    );
    check(
      listed[0].wrappedDocKey === entries[0].wrappedDocKey,
      "and it carries each document's wrapped key, so no per-document unwrap is needed",
    );

    console.log('  -- the store cannot read it (WS9-R5)');
    check(
      !fetched.index!.includes(Buffer.from('Payments platform').toString('base64')),
      'the sealed blob does not contain a title',
    );
    const asText = Buffer.from(fetched.index!, 'base64').toString('latin1');
    check(
      !asText.includes('Payments platform') && !asText.includes('Fraud review'),
      'no title appears anywhere in the bytes the store holds',
    );
    check(
      !asText.includes(Buffer.from(await exportSymmetricKey(workspaceKey)).toString('latin1')),
      'and no key does either',
    );

    console.log('  -- conditional writes (WS9-R3)');
    const conflictOnCreate = await put(await seal(entries), null);
    check(
      conflictOnCreate.status === 409,
      'creating an index that already exists is a conflict, not an overwrite',
    );

    // Two clients read the same version and both add a document.
    const before = await get();
    const alice = [
      ...(await open(before.index!)),
      {
        docId: 'doc-c',
        wrappedDocKey: 'Yw==',
        title: "Alice's diagram",
    ...
    check(
      bobWrote.status === 409,
      'and the second is refused rather than silently overwriting the first',
    );
    ...
    check(
      finalEntries.some((entry) => entry.docId === 'doc-c') &&
        finalEntries.some((entry) => entry.docId === 'doc-d'),
      "and both documents are in the index: neither client's change was lost",
    );

    console.log('  -- bad requests');
    check(
      (await put(await seal(entries), 999)).status === 409,
      'a write against a version that never existed is refused',
    );
    ...
    check(
      (await fetch(`${origin}/v1/workspaces/${WORKSPACE}/index`, { method: 'DELETE' })).status ===
        405,
