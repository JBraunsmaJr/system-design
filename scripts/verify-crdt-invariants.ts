...
const files = [...sources('src'), ...sources('scripts')].filter(
  (f) => f.path !== 'scripts/verify-crdt-invariants.ts',
);
...
console.log('=== WS4-R5: gc stays on ===');
{
  ...
  check(
    offenders.length === 0,
    `no Y.Doc is built with gc: false${offenders.length ? ` (${offenders.join(', ')})` : ''}`,
  );
