...
import {
  ...
  verificationCodeFor,
  type UserDirectory,
} from '../store/src/userDirectory.ts';
...
async function run(name: string, directory: UserDirectory) {
  ...
  const json = async (response: Response) =>
    (await response.json()) as Record<string, never> & Record<string, unknown>;
  ...
  try {
    ...
    const firstRegistered = (
      await json(
        await call('/v1/users/me/devices', {
          method: 'POST',
          cookie,
          body: JSON.stringify({
            publicKey: b64(await exportPublicKey(first.publicKey)),
            label: 'Laptop',
          }),
        }),
      )
    ...
    check(
      firstRegistered.approvedAt !== null,
      "the user's first device is approved as it registers: it is the one that makes the user key",
    );
    const firstKeys = await json(
      await call('/v1/users/me/keys', { cookie, deviceId: firstRegistered.deviceId }),
    );
    check(
    ...
    await call('/v1/users/me/keys', {
      method: 'PUT',
      cookie,
      body: JSON.stringify({ generation: 1, wrappedKey: workspaceForUser }),
    });
    const held = await json(await call('/v1/users/me', { cookie }));
    const asStored = JSON.stringify(held);
    check(
      !asStored.includes(await raw(workspaceKey)),
      'nothing the store returns contains the workspace key itself',
    );
    ...
    const secondRegistered = (
      await json(
        await call('/v1/users/me/devices', {
          method: 'POST',
          cookie,
          body: JSON.stringify({ publicKey: secondPublic, label: 'Desktop' }),
        }),
      )
    ...
    const impostorCode = await verificationCodeFor(
      b64(await exportPublicKey((await generateWrappingKeyPair('device')).publicKey)),
    );
    check(
      impostorCode !== secondRegistered.verificationCode,
      'a substituted key would show a different code, so approval would be refused',
    );
    ...
    await rejects(
      () =>
        call(`/v1/users/me/devices/${secondRegistered.deviceId}/approve`, {
          method: 'POST',
          cookie,
          deviceId: firstRegistered.deviceId,
          body: JSON.stringify({ ...JSON.parse(body), verificationCode: impostorCode }),
        }),
      400,
      'approval with the wrong code is refused',
    );
    const approved = await call(`/v1/users/me/devices/${secondRegistered.deviceId}/approve`, {
      method: 'POST',
      cookie,
      deviceId: firstRegistered.deviceId,
      body,
    ...
    const keys = await json(
      await call('/v1/users/me/keys', { cookie, deviceId: secondRegistered.deviceId }),
    );
    check(
      keys.status === 'approved' && !!keys.wrappedUserKey,
      'it is handed the user key, wrapped to it',
    );
    const wrap = keys.wrappedUserKey as { keyWrap: string; body: string };
    const userKeyOnSecond = await unwrapPrivateKeyWithPrivateKey(
      {
        keyWrap: Uint8Array.from(Buffer.from(wrap.keyWrap, 'base64')),
        body: Uint8Array.from(Buffer.from(wrap.body, 'base64')),
      },
      second.privateKey,
      KEY_CONTEXT,
    );
    const workspaceOnSecond = await unwrapKeyWithPrivateKey(
      Uint8Array.from(
        Buffer.from((keys.workspaceKeys as { wrappedKey: string }[])[0].wrappedKey, 'base64'),
      ),
      userKeyOnSecond,
      'AES-KW',
    );
    check(
      (await raw(workspaceOnSecond)) === (await raw(workspaceKey)),
      'and unwraps the workspace key with it: the new browser can now open documents',
    );
    ...
    check(
      !storeReached,
      "everything the store holds is useless without the device's own private key",
    );

    console.log('  -- revoking a device (WS7-R14)');
    const revoked = await json(
      await call(`/v1/users/me/devices/${secondRegistered.deviceId}`, { method: 'DELETE', cookie }),
    );
    check(
      (revoked.device as { revokedAt: string | null }).revokedAt !== null,
      'revocation is recorded',
    );
    check(
      (revoked.device as { wrappedUserKey: unknown }).wrappedUserKey === null,
      'and its wrap of the user key is taken away',
    );
    await rejects(
      () => call('/v1/users/me/keys', { cookie, deviceId: secondRegistered.deviceId }),
      403,
      'the revoked device is refused its keys',
    );
    ...
    const thirdRegistered = (
      await json(
        await call('/v1/users/me/devices', {
          method: 'POST',
          cookie,
          body: JSON.stringify({ publicKey: thirdPublic, label: 'Phone' }),
        }),
      )
    ).device as { deviceId: string; verificationCode: string };
      ...
      () =>
        call(`/v1/users/me/devices/${thirdRegistered.deviceId}/approve`, {
          ...
          body: JSON.stringify({
            ...
            wrappedUserKey: {
              keyWrap: b64(wrappedForThird.keyWrap),
              body: b64(wrappedForThird.body),
            },
          }),
        }),
      403,
    ...
    const stored = await call('/v1/users/me/recovery', {
      method: 'PUT',
      cookie,
      body: JSON.stringify({ salt: b64(salt), sealedUserKey: b64(sealedUserKey) }),
    });
    check(stored.status === 204, 'the sealed user key and its salt are stored');
    const fetched = (await json(await call('/v1/users/me/recovery', { cookie }))).recovery as {
      salt: string;
    ...
    const workspaceViaCode = await unwrapKeyWithPrivateKey(
      Uint8Array.from(Buffer.from(workspaceForUser, 'base64')),
      unlocked,
      'AES-KW',
    );
    check(
      (await raw(workspaceViaCode)) === (await raw(workspaceKey)),
      'someone with no device left recovers the user key from their code, and through it the workspace key',
    );
    ...
    try {
      await openPrivateKey(
        ...
        await deriveRecoveryKey(
          wrongCode.secret,
          Uint8Array.from(Buffer.from(fetched.salt, 'base64')),
        ),
        KEY_CONTEXT,
      );
      wrongWorked = true;
    ...
    console.log('  -- administrator re-grant (WS7-R13)');
    {
      ...
      const published = await call('/v1/users/me/public-key', {
        method: 'PUT',
        cookie,
        body: JSON.stringify({ publicKey: userPublic }),
      });
      check(published.status === 200, 'a member publishes their public user key');
      const listed = (await json(await call('/v1/admin/users', { cookie }))).users as {
        userId: string;
        publicKey?: string;
      }[];
      check(
        listed.some((user) => user.publicKey === userPublic),
        'an administrator can see it, to wrap the workspace key to them',
      );

      // Now everything is lost: every device gone, recovery code forgotten.
      const me = listed[0];
      const regranted = await call(`/v1/admin/users/${me.userId}/regrant`, {
        method: 'POST',
        cookie,
      });
      check(regranted.status === 200, 'the administrator re-grants access');
      const afterDevices = (await json(await call('/v1/users/me/devices', { cookie }))).devices as {
        revokedAt: string | null;
      }[];
      check(
        afterDevices.every((device) => device.revokedAt !== null),
        'every previous device is revoked',
      );
      check(
        (await json(await call('/v1/users/me/recovery', { cookie }))).recovery === null,
        'the old recovery wrap is gone',
      );
      check(
        (
          (await json(await call('/v1/admin/users', { cookie }))).users as { publicKey?: string }[]
        )[0].publicKey === undefined,
        'and so is the old user key, which nobody can reach',
      );
      ...
      const freshRegistered = (
        await json(
          await call('/v1/users/me/devices', {
            ...
            cookie,
            body: JSON.stringify({
              publicKey: b64(await exportPublicKey(freshDevice.publicKey)),
              label: 'Replacement',
            }),
          }),
        )
      ...
      check(
        freshRegistered.approvedAt !== null,
        'their next device enrolls as a first device again',
      );
      await call('/v1/users/me/public-key', {
        method: 'PUT',
        cookie,
        body: JSON.stringify({ publicKey: b64(await exportPublicKey(freshUserKey.publicKey)) }),
      });
      ...
      const granted = await call(`/v1/admin/users/${me.userId}/workspace-key`, {
        method: 'PUT',
        cookie,
        body: JSON.stringify({ generation: 2, wrappedKey: rewrapped }),
      });
      check(granted.status === 204, 'and wraps the workspace key to it');

      const recovered = await unwrapKeyWithPrivateKey(
        Uint8Array.from(Buffer.from(rewrapped, 'base64')),
        freshUserKey.privateKey,
        'AES-KW',
      );
      check(
        (await raw(recovered)) === (await raw(workspaceKey)),
        'the person is back in: the same workspace key, through a new user key',
      );
      check(
        (
          await json(
            await call('/v1/users/me/keys', { cookie, deviceId: freshRegistered.deviceId }),
          )
        ).status !== undefined,
        'and their new device is serviceable',
      );
    }
    ...
    check(
      audit.all().some((e) => e.operation === 'device-approve' && e.outcome === 'denied'),
      'a refused approval is audited too',
    );
  } finally {
