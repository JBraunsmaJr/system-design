...
async function run(backend: Backend) {
  ...
  console.log('  -- 30 days (the default)');
  {
    ...
    check(
      ...
      'hidden from ordinary listings, present in the deleted view (WS9-R6)',
    );
    check(
      (await service.read('doc-30d', { includeDeleted: true })).blobs.length === 1,
      'its content is retained',
    );
    ...
    check(
      restored.deletedAt === null && restored.purgeAfter === null,
      'restoring clears the deletion and its purge date',
    );
    ...
    check(
      (await service.purgeDue(clock)).includes('doc-30d'),
      'once the period has passed, the sweep purges it',
    );
  ...
  console.log('  -- immediate');
  {
    ...
    check(
      ...
      'and nothing is retained, not even for an administrator',
    );
  ...
  console.log('  -- indefinite');
  {
    ...
    check(
      (await service.purgeDue(clock)).length === 0,
      'and ten years later the sweep still leaves it alone',
    );
    await service.purge('doc-forever');
    check(
      (await service.list({ includeDeleted: true })).length === 0,
      'an administrator can still purge it explicitly',
    );
    clock = new Date('2026-09-17T12:00:00Z');
  }
  ...
  console.log('  -- legal hold outranks every mode (WS10-R8)');
  {
    ...
    const held = await service.setLegalHold('doc-held', {
      reason: 'FOIA request 2026-114',
      placedBy: 'admin@example.gov',
    });
    ...
    check(
      (await service.purgeDue(clock)).length === 0,
      'the sweep never takes it, however long it has been',
    );
    await rejects(
      () => service.purge('doc-held'),
      'conflict',
      'and an explicit purge is refused while the hold stands',
    );
    check(
      (await service.read('doc-held', { includeDeleted: true })).blobs.length === 1,
  ...
  console.log('  -- a hold survives immediate deletion');
  {
    ...
    await service.setLegalHold('doc-held-now', {
    ...
    check(
      (await service.read('doc-held-now', { includeDeleted: true })).blobs.length === 1,
