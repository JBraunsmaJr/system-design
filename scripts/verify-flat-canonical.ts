...
const fixtures = readdirSync(fixtureDir)
  ...
  .map((f: string) => ({
    name: f,
    file: parseDiagramFile(readFileSync(join(fixtureDir, f), 'utf8')),
  }));
...
}
{
  ...
  assert(
    JSON.stringify((inner?.data as { parentPath?: string[] }).parentPath) ===
    ...
    nodes: [
      {
        ...
        type: 'typed',
        position: { x: 1, y: 1 },
        data: { nodeType: 'service', label: 'Old' },
      },
    ],
    edges: [],
  } as never);
...
for (const { name, file } of fixtures) {
  ...
  paths.push(
    ['no-such-node'],
    [...(paths[1] ?? []), 'no-such-child'],
    ['no-such-node', ...(paths[1] ?? [])],
  );
...
console.log('\n=== WS5-R9: lossless at four levels deep ===');
{
  ...
  const level = (
    prefix: string,
    depth: number,
  ): { nodes: AnyNode[]; edges: Edge<ArchEdgeData>[] } => {
    ...
    const edges = [
      {
        ...
        target: `${prefix}1`,
        type: 'typed',
        data: { edgeType: 'sync' } as ArchEdgeData,
      },
      {
        id: `${prefix}f`,
        source: `${prefix}1`,
  ...
  const deepest = flat.nodes.filter(
    (n) => ((n.data as { parentPath?: string[] }).parentPath ?? []).length === 3,
  );
  assert(
    flat.nodes.length === 12 && flat.edges.length === 8 && deepest.length === 3,
    `four levels flatten to 12 nodes and 8 edges (${deepest.length} at depth 4)`,
  );
  const rebuilt = unflattenToSubDiagram(flat.nodes, flat.edges);
  const strip = (x: unknown): unknown => JSON.parse(JSON.stringify(x));
  assert(
    JSON.stringify(strip(rebuilt)) === JSON.stringify(strip(tree)),
    'flatten then rebuild yields the original tree exactly',
  );

  const deepFile = {
    ...fixtures[fixtures.length - 1].file,
    nodes: tree.nodes,
    edges: tree.edges,
  } as DiagramFile;
  const doc = new Y.Doc();
  ...
  const loaded = createYjsDiagramStore(doc).getSnapshot();
  assert(
    sameSet(loaded.nodes.map(nodeKey), flat.nodes.map(nodeKey)) &&
      sameSet(loaded.edges.map(edgeKey), flat.edges.map(edgeKey)),
    'and a file that deep loads into a document at every level',
  );
  const back = unflattenToSubDiagram(loaded.nodes, loaded.edges);
  assert(
    JSON.stringify(strip(back)) === JSON.stringify(strip(tree)),
    'and comes back out of the document unchanged',
  );
}
...
console.log('\n=== WS1-R3: flattening only at import boundaries ===');
{
  ...
  assert(
    offenders.length === 0,
    `no other production callers${offenders.length ? `: ${offenders.join(', ')}` : ''}`,
  );
}
