...
const undoDisabled = (p: Page) =>
  p.$eval('button[aria-label="Undo"]', (el) => (el as HTMLButtonElement).disabled);
const titleOf = (p: Page) => p.inputValue('[aria-label="Diagram title"]');
const storeWrites = (p: Page) => p.evaluate<number>(`window.__PERF__.getCounters().storeWrites`);
  ...
  try {
    ...
    for (const [name, p] of [
      ...
      p.on('pageerror', (e) => {
        failures++;
        console.error(
          `  FAIL: page ${name} threw: ${e.message}\n${(e.stack ?? '').split('\n').slice(0, 8).join('\n')}`,
        );
      });
    }
    ...
    await sleep(300);
    check(
      (await nodeCount(a)) === fixture.nodes.length,
      `root level shows the file's ${fixture.nodes.length} nodes`,
    );
    ...
    await a
      .waitForFunction(
        ...
        NESTED.data.subDiagram!.nodes.length,
        { timeout: 5000 },
      )
      .catch(() => {});
    check(
    ...
    check(
      (await storeWrites(a)) === 1,
      `a ten-frame drag of one node writes the document once (wrote ${await storeWrites(a)})`,
    );
    ...
    check(
      multiWrites >= 2 && multiWrites <= rootIds.length,
      `a 150-frame drag of the selection writes once per moved node (${multiWrites} writes)`,
    );
    ...
    console.log('\n=== Dragging a node out of its group: one write, no jump ===');
    {
      ...
      const c = await (
        await browser.newContext({ ...perms, viewport: { width: 1600, height: 1000 } })
      ).newPage();
      ...
      const viewport = () =>
        c.$eval('.react-flow__viewport', (e) => (e as HTMLElement).style.transform);
      ...
      const margin = (m: { dx: number; dy: number }) =>
        Math.min(
          pane.right - (box.x + box.width + m.dx),
          pane.bottom - (box.y + box.height + m.dy),
        );
      const move = margin(down) >= margin(right) ? down : right;
      if (margin(move) < 80) {
        throw new Error(
          `no drop point clear of the pane edge (best margin ${Math.round(margin(move))}px) - it would auto-pan`,
        );
      }
      const { dx, dy } = move;
      ...
      await sleep(300);
      check(
        (await viewport()) === viewportBefore,
        'the viewport did not auto-pan (so screen positions are comparable)',
      );
      ...
      check(
        shift < 6,
        `node-21 stays where it was dropped (moved ${shift.toFixed(1)}px on release)`,
      );
      check(
        !!after &&
          (after.y > groupBox.y + groupBox.height || after.x > groupBox.x + groupBox.width),
        'and is outside the group',
      );
      check(
        (await storeWrites(c)) === 1,
        `leaving the group is one write (wrote ${await storeWrites(c)})`,
      );
      ...
      check(
        !!back &&
          !!groupNow &&
          Math.abs(back.x - groupNow.x - (box.x - groupBox.x)) < 2 &&
          Math.abs(back.y - groupNow.y - (box.y - groupBox.y)) < 2,
    ...
    const edgeLabel = (id: string) =>
      a.getAttribute(`.react-flow__edge[data-id="${id}"]`, 'aria-label');
    ...
    const grab = await a.evaluate(() => {
      const el = document.querySelector(
        '.react-flow__edge[data-id="e1"] .react-flow__edgeupdater-target',
      );
      if (!el) return null;
      ...
      console.error(
        ...
        await a.evaluate(() => {
          ...
          if (!el)
            return (
              'not rendered: ' +
              Array.from(document.querySelectorAll('.react-flow__edge[data-id="e1"] *'))
                .map((e) => (e as SVGElement).className?.baseVal ?? e.tagName)
                .join(',')
            );
          const r = el.getBoundingClientRect();
    ...
    if (grab && drop) {
      ...
      for (let i = 1; i <= 10; i++)
        await a.mouse.move(
          grab.x + ((drop.x - grab.x) * i) / 10,
          grab.y + ((drop.y - grab.y) * i) / 10,
        );
      await a.mouse.up();
      await sleep(200);
      check(
        (await edgeLabel('e1')) === 'Edge from n-gateway to n-db',
        `e1 now ends at n-db (was: ${originalLabel})`,
      );
    ...
    console.log('\n=== Bends: one write per gesture, one undo each (WS4-R1/R2 for edges) ===');
    {
      const bendCount = () =>
        a
          .locator('.react-flow__edge[data-id="e2"] .typed-edge__waypoint, .typed-edge__waypoint')
          .count();
      ...
      check(
        (await storeWrites(a)) === 1,
        `creating it is one write (wrote ${await storeWrites(a)})`,
      );
      ...
      const edgePath = () =>
        a.getAttribute('.react-flow__edge[data-id="e2"] path.react-flow__edge-path', 'd');
      ...
      check(
        (await storeWrites(a)) === 1,
        `a 15-step bend drag is one write (wrote ${await storeWrites(a)})`,
      );
      await a.click('.react-flow__pane', { position: { x: 5, y: 5 } });
      await a.keyboard.press('Control+z');
      await sleep(250);
      ...
      await sleep(700);
      const label = a
        .locator('.react-flow__edge-labels .typed-edge__label, .typed-edge__label')
        .first();
      ...
      check(
        (await storeWrites(a)) === 1,
        `a 15-step label drag is one write (wrote ${await storeWrites(a)})`,
      );
      await a.click('.react-flow__pane', { position: { x: 5, y: 5 } });
      await a.keyboard.press('Control+z');
      await sleep(250);
      ...
      check(
        !!back && Math.hypot(back.x - lbox.x, back.y - lbox.y) < 2,
        'one undo puts the label back',
      );
    }
    ...
    {
      const pathOf = (p: Page) =>
        p.getAttribute('.react-flow__edge[data-id="e2"] path.react-flow__edge-path', 'd');
      await a.evaluate(`window.__PERF__.setSelectedEdges(["e2"])`);
      await sleep(250);
      const dot = await a.locator('.typed-edge__insert-dot').first().boundingBox();
      if (dot) {
        ...
        await sleep(1000);
        check(
          (await pathOf(b)) !== bPathBefore,
          "B sees A's new bend while A is still dragging it",
        );
        await a.mouse.up();
        await sleep(1000);
    ...
    check(
      (await titleOf(a)) === 'Renamed by B' && (await titleOf(b)) === 'Renamed by B',
      "B's rename survives A's undo",
    );
    ...
    const guarded = await a.waitForSelector('.leave-guard', { timeout: 3000 }).then(
      () => true,
      () => false,
    );
    ...
    await Promise.all([
      a.waitForNavigation(),
      a.click('.export-menu__dropdown button:has-text("New")'),
    ]);
    await a.waitForSelector('.collab-panel__trigger');
    await sleep(300);
    const docAfter = new URL(a.url()).searchParams.get('doc');
    check(
      !!docAfter && docAfter !== docBefore,
      `New opens a different document (${docBefore} -> ${docAfter})`,
    );
