...
console.log('=== It starts, serves, and stops ===');
{
  ...
  if (health) {
    ...
    check(
      ...
      'and that this one has no sign-in, which it was told to allow',
    );
  }
  const printed = running.output();
  check(
    /Deleted documents are kept.*7y/.test(printed),
    `startup prints the retention it will apply (${/(Deleted documents[^\n]*)/.exec(printed)?.[1] ?? 'nothing'})`,
  );
  check(/Sign-in: NONE/.test(printed), 'and says plainly that it has no sign-in');
  check(
    process.env.DATABASE_URL
      ? /Storage: PostgreSQL/.test(printed)
      : /Storage: in memory/.test(printed),
...
console.log('\n=== It refuses to start badly configured ===');
{
  const cases: [string, Record<string, string>, RegExp][] = [
    [
      ...
      { PORT: String(PORT + 1), ALLOW_UNAUTHENTICATED: 'true' },
      /PUBLIC_URL is required/,
    ],
    [
      'with no sign-in configured',
      { PORT: String(PORT + 1), PUBLIC_URL: `http://localhost:${PORT + 1}` },
