    ...
    // file back through the page.
    {
      ...
      const readTitle = () =>
        page.evaluate(async () => {
          ...
          ).getFileHandle('file-doc-v1.json');
      ...
      const ok =
        (await step('1a store an OPFS handle in IndexedDB', page, browser, () =>
          page.evaluate(async () => {
            const handle = await (
              await navigator.storage.getDirectory()
            ).getFileHandle('diag.json', { create: true });
          ...
          page.evaluate(async () => {
            const h = (
              window as unknown as {
                __h: FileSystemFileHandle & { queryPermission?: (d: unknown) => Promise<string> };
              }
            ).__h;
            return typeof h.queryPermission === 'function'
        ...
        (await step('1e getFile', page, browser, () =>
          page.evaluate(
            async () =>
              (await (window as unknown as { __h: FileSystemFileHandle }).__h.getFile()).size,
          ),
        ));
      await ctx.close().catch(() => {});
