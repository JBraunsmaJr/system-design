...
// Test 3: Fallback priority
{
  ...
  const config = evaluateConfig(res.targetContent!);
  assert(
    config.SIGNALING_URL === 'wss://fallback-relay.example.com',
    'SIGNALING_URL fallback works',
  );
...
// Test 5: JS string escaping (quotes and backslashes)
{
  ...
  const config = evaluateConfig(res.targetContent!);
  assert(
    config.SIGNALING_URL === `wss://relay.example.com/"test"\\path`,
    'Quotes and backslashes preserved',
  );
  assert(
    config.APP_URL === `https://app.example.com/test\\"quote"`,
    'Quotes and backslashes preserved in APP_URL',
  );
  assert(
    config.ICE_SERVERS === `[{"urls":"stun:stun.l.google.com:19302"}]`,
    'JSON string escaped and parsed cleanly',
  );
