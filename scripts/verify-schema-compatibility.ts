...
function findDroppedPaths(
  ...
  if (Array.isArray(before)) {
    ...
    before.forEach((item, i) => findDroppedPaths(item, after[i], `${path}[${i}]`, dropped));
    return dropped;
  }
...
{
  const base = JSON.parse(readFileSync(join(fixturesDir, `schema-${SCHEMA_VERSION}.json`), 'utf8'));
  let refused = false;
  let message = '';
  try {
...
{
  const base = JSON.parse(readFileSync(join(fixturesDir, 'schema-0.1.json'), 'utf8')) as Record<
    string,
    unknown
  >;
...
console.log('=== 5. Unknown fields from a newer minor version survive ===');
{
  ...
  try {
    ...
    const firstNodeData = parsed.nodes?.[0]?.data as Record<string, unknown> | undefined;
  ...
  try {
    const parsed = parseDiagramFile(JSON.stringify(withTopLevel)) as unknown as Record<
      string,
      unknown
    >;
    const roundTripped = JSON.parse(JSON.stringify(parsed)) as Record<string, unknown>;
    topLevelSurvives =
      (roundTripped.somethingFromANewerPatch as Record<string, unknown>)?.keep === true;
  } catch {
