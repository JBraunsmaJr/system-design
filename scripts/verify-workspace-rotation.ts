...
import { documentKeyFor, escrowDocumentKey, indexKeyFor, newDocumentKey, upsertEntry } from "../src/collab/workspaceDocuments.ts";
import { exportPublicKey, exportSymmetricKeyHex, generateWorkspaceKey, generateWrappingKeyPair } from "../src/crypto/keys.ts";
...
try {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    await client.updateIndex(WORKSPACE, oldIndexKey, (entries) =>
      upsertEntry(entries, { docId, wrappedDocKey, title: `Document ${i}`, updatedAt: new Date(2026, 8, 19, 0, i).toISOString() }),
    );
  }
  // One of them deleted, but still inside its retention.
  await client.deleteDocument("doc-0");
  check((await client.readIndex(WORKSPACE, oldIndexKey)).entries.length === COUNT, "all of them are listed, including the deleted one");

  const blobsBefore = await blobs.totalBytes();
  const contentBefore = new Map<string, string>();
  for (let i = 1; i < COUNT; i++) {
    const entry = (await client.readIndex(WORKSPACE, oldIndexKey)).entries.find((e) => e.docId === `doc-${i}`)!;
    contentBefore.set(`doc-${i}`, (await client.getDocument(`doc-${i}`, await documentKeyFor(entry, oldKey))).file.title);
  }

  console.log("\n=== Rotating ===");
  const started = performance.now();
  const result = await rotateWorkspaceKey({ client, workspaceId: WORKSPACE, currentKey: oldKey, currentGeneration: 1 });
  const elapsed = performance.now() - started;
  check(result.generation === 2 && result.documentsRewrapped === COUNT, `every document is re-wrapped (${result.documentsRewrapped}) at generation ${result.generation}`);
  check((await blobs.totalBytes()) === blobsBefore, "and not one content blob is rewritten (WS7-R7)");
  ...
  check(after.entries.length === COUNT && after.generation === 2, "the index opens with the new key, and says which generation it is");
  let opened = 0;
  ...
  await rejects(() => client.readIndex(WORKSPACE, oldIndexKey), "the old key no longer opens the index");
  const staleEntry = { wrappedDocKey: (await client.readIndex(WORKSPACE, newIndexKey)).entries[0].wrappedDocKey };
  ...
  await rejects(() => client.getDocument("doc-0", deletedKey), "it reads as deleted while it is deleted");
  await fetch(`${origin}/v1/docs/doc-0/restore`, { method: "POST" });
  const restored = await client.getDocument("doc-0", deletedKey);
  check(restored.file.title === "Document 0", "and once restored it opens with the NEW key: rotation re-wrapped it too");
  ...
  console.log("\n=== Members ===");
  {
    ...
    await directory.setUserPublicKey(alice.userId, Buffer.from(await exportPublicKey(userKey.publicKey)).toString("base64"));
    await directory.upsertUser({ issuer: "https://idp", subject: "bob" });

    const second = await rotateWorkspaceKey({ client, workspaceId: WORKSPACE, currentKey: newKey, currentGeneration: 2 });
    check(second.membersGranted === 1, "the new key is wrapped to each member who has published a key");
    check(second.membersSkipped.length === 1, "and anyone who has not is named rather than skipped silently");
    const wraps = await directory.getWorkspaceKeys(alice.userId);
    check(wraps.some((wrap) => wrap.generation === 3), "the member's wrap is recorded at the new generation");
    check(
