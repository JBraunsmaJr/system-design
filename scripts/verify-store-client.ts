...
async function rejects(work: () => Promise<unknown>, reason: string, message: string) {
  try {
    ...
    const actual =
      error instanceof StoreClientError ? error.reason : `other: ${String(error).slice(0, 60)}`;
    if (actual !== reason) {
      failures++;
...
function memoryBackend(): {
  store: StoreBackend;
  index: ReturnType<typeof createMemoryWorkspaceIndex>;
} {
...
async function run(
  ...
  const wrappedDocKey = Buffer.from(
    await wrapKey(await deriveStorageKey(documentKey), workspaceKey),
  ).toString('base64');
  ...
  try {
    ...
    check(
      published === recoveryPublicPem,
      'the store publishes its recovery public key, so clients can wrap to it',
    );
    await rejects(
      () =>
        client.putDocument('doc-unescrowed', documentKey, DOCUMENT, {
    ...
    check(
      JSON.stringify(fetched.file) === JSON.stringify(DOCUMENT),
    ...
    check(
      !stored.includes('Payments platform') && !stored.includes('Gateway'),
      'no title and no node label appears in what the store serves',
    );
    ...
    await rejects(
      () => client.getDocument(docId, generateSessionKey()),
      'unreadable',
      "another link's key is refused, and says so plainly",
    );

    console.log('  -- updates and versions (WS8-R15)');
    const edited = { ...DOCUMENT, title: 'Payments platform v2' };
    const second = await client.putDocument(docId, documentKey, edited, {
      wrappedForWorkspace: wrappedDocKey,
    });
    check(second > version, `a second upload moves the version on (${version} -> ${second})`);
    check(
      (await client.getDocument(docId, documentKey)).file.title === 'Payments platform v2',
      'and reading returns the newer document',
    );
    ...
    await client.updateIndex('workspace-1', indexKey, (entries) => [
      ...entries.filter((e) => e.docId !== docId),
      entry,
    ]);
    const listed = await client.readIndex('workspace-1', indexKey);
    check(
      listed.entries.length === 1 && listed.entries[0].title === 'Payments platform',
      'the index lists the document by title',
    );
    const sealedIndex = await (await fetch(`${origin}/v1/workspaces/workspace-1/index`)).json();
    check(
      !Buffer.from((sealedIndex as { index: string }).index, 'base64')
        .toString('latin1')
        .includes('Payments platform'),
    ...
    console.log('  -- the organization can recover it with nothing else (WS7-R4, R6)');
    {
      ...
      check(
        !!raw.document.keys.wrappedForRecovery,
        'the store kept the recovery wrap alongside the document',
      );
      ...
      const recovered = await recoverDocumentPackage(
        pkg,
        await importRecoveryPrivateKey(await exportPrivateKey(recoveryPair.privateKey)),
      );
      const file = JSON.parse(new TextDecoder().decode(recovered[0].data)) as { title: string };
      check(
        file.title === 'Payments platform v2',
        'and the offline recovery key alone opens what the workspace saved',
      );
    }
    ...
    await rejects(
      () => client.getDocument(docId, documentKey),
      'deleted',
      'a deleted document reads as deleted, distinct from missing',
    );
    await rejects(
      () => client.getDocument('doc-never', documentKey),
      'not-found',
      'and a document that never existed is not found',
    );
    ...
    await rejects(
      () => offline.session(),
      'offline',
      'an unreachable store is reported as offline, not as a crash',
    );
    ...
    const everything = JSON.stringify(
      await (await fetch(`${origin}/v1/docs?includeDeleted=true`)).json(),
    );
    check(
      !everything.includes(exported),
      'the workspace key appears nowhere in what the store serves',
    );
  } finally {
