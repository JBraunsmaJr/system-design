...
export async function startDevServers(options: { vitePort: number; signalingPort: number; quiet?: boolean }): Promise<DevServers> {
  ...
  for (const port of [options.vitePort, options.signalingPort]) {
    const busy = (await Promise.all([tryConnect("127.0.0.1", port), tryConnect("::1", port)])).some(Boolean);
    if (busy) throw new Error(`Port ${port} is already in use. Stop whatever is serving it and re-run.`);
  }
  ...
  const vite = spawn(process.execPath, ["node_modules/vite/bin/vite.js", "--port", String(options.vitePort), "--strictPort"], {
