...
export async function startTestOidcProvider(
  ...
  const sign = (payload: Record<string, unknown>, mode: Misbehaviour): string => {
    ...
    if (mode === 'bad-signature')
      return `${body}.${Buffer.from(randomBytes(256)).toString('base64url')}`;
  ...
  const server: Server = createServer(async (request, response) => {
    ...
    if (url.pathname === '/login-actions/profile' && request.method === 'POST') {
      ...
      if (!submitted.get('email')) {
        ...
        return response.end(
          "<html><body><h1 id='kc-page-title'>Update Account Information</h1><p id='input-error'>Email is required.</p></body></html>",
        );
      }
    ...
    if (url.pathname === '/login-actions/authenticate' && request.method === 'POST') {
      ...
      if (!submitted.get('username') || !submitted.get('password')) {
        ...
        return response.end(
          "<html><body><p id='input-error'>Invalid username or password.</p></body></html>",
        );
      }
...
export async function startTestGitHub(): Promise<TestGitHub> {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === '/user') {
      if (request.headers.authorization !== 'Bearer gho_test')
