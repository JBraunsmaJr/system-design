...
async function run() {
  ...
  try {
    ...
    console.log(
      '=== A document persists without a save, and localStorage is not used (WS2-R1) ===',
    );
    {
      ...
      await ready(p);
      check(
        new URL(p.url()).searchParams.get('doc') === 'local',
        'a bare URL opens the default document and names it in the URL',
      );
      ...
      check(
        n === 25 && t === 'Persisted title',
        `a reload restores content and title with no explicit save (${n} nodes, "${t}")`,
      );
      await p.goto(servers.appUrl + '/system-design/');
      await ready(p);
      check(
        new URL(p.url()).searchParams.get('doc') === 'local',
        'a bare URL reopens the last document',
      );
    }
    ...
    console.log('\n=== The retired draft is imported once, then cleared ===');
    {
      ...
      await ready(p);
      check(
        (await nodeCount(p)) === FIXTURE.nodes.length && (await titleOf(p)) === FIXTURE.title,
        'an empty document is seeded from the old draft',
      );
      await sleep(SAVE_WAIT);
      check(
        (await legacyDraft(p)) === null,
        'and the old draft is cleared once the content is stored',
      );
      await p.reload();
      await ready(p);
      check(
        (await nodeCount(p)) === FIXTURE.nodes.length && (await titleOf(p)) === FIXTURE.title,
        'the imported content survives a reload',
      );
    }
    ...
    console.log('\n=== Two tabs, two documents (WS2-R3) ===');
    {
      ...
      await ready(alpha);
      await ready(beta);
      await alpha.setInputFiles(
        'input[type="file"][accept="application/json"]',
        resolve(join('fixtures', 'schema-0.7.json')),
      );
      ...
      await ready(beta);
      check(
        (await nodeCount(alpha)) === FIXTURE.nodes.length &&
          (await titleOf(alpha)) === FIXTURE.title,
        'alpha keeps its own content',
      );
      check(
    ...
    console.log('\n=== The document manager (WS2-R3, WS2-R5, WS2-R6) ===');
    {
      ...
      await Promise.all([
        p.waitForNavigation(),
        p.click('.export-menu__dropdown button:has-text("New")'),
      ]);
      await ready(p);
      const secondId = new URL(p.url()).searchParams.get('doc') ?? '';
      check(
        secondId !== 'manager-one' && (await nodeCount(p)) === 0,
        'File > New opens a new, empty document',
      );
      ...
      await openManager();
      check(
        (await row('manager-one').count()) === 1 && (await row(secondId).count()) === 1,
        'both documents are listed',
      );
      check(
        (await row(secondId).locator('.document-manager__current').count()) === 1,
        'the open one is marked',
      );
      check(
        await row(secondId).locator('.document-manager__forget').isDisabled(),
        'the open document cannot be forgotten from its own tab',
      );
      const storageText = (await p.textContent('.document-manager__storage')) ?? '';
      check(
        /Using .+ of .+ available/.test(storageText),
        `storage usage is shown (WS2-R5): "${storageText.slice(0, 60)}..."`,
      );
      ...
      await p.waitForFunction(() =>
        [...document.querySelectorAll('.document-manager__title')].some(
          (e) => e.textContent === 'Doc one renamed',
        ),
      );
      ...
      await sleep(200);
      check(
        (await titleOf(p)) === 'Doc two renamed',
        'renaming the open document renames it in the editor too',
      );
      ...
      const copyId = await p
        .locator(
          '.document-manager__row:has(.document-manager__title:text-is("Copy of Doc one renamed"))',
        )
      ...
      await row(copyId ?? '')
        .locator('.document-manager__forget')
        .click();
      await p.waitForFunction(
        (id) => !document.querySelector(`.document-manager__row[data-doc-id="${id}"]`),
        copyId,
      );
      check(
        /permanently deletes/.test(confirmText) && /cannot be undone/.test(confirmText),
        'forgetting asks first, and says it is irreversible (WS2-R6)',
      );
      await sleep(300);
      const dbs = await databaseNames(p);
      check(
        !dbs.includes(`system-design:doc:${copyId}`),
        "the forgotten document's database is deleted",
      );
      ...
      await Promise.all([
        p.waitForNavigation(),
        row('manager-one').locator('.document-manager__open').click(),
      ]);
      await ready(p);
      check(
        new URL(p.url()).searchParams.get('doc') === 'manager-one' &&
          (await titleOf(p)) === 'Doc one renamed' &&
          (await nodeCount(p)) === 25,
    ...
    console.log('\n=== Sessions and local replicas ===');
    {
      ...
      check(
        !hostDbs.some((n) => n.startsWith('system-design:room:')),
      ...
      check(
        local?.title === 'Guest local work',
        `joining did not overwrite the guest's own document entry (title: ${local?.title})`,
      );
    }
    ...
    console.log('\n=== WS4-R6/R7/R8: compact and rebase from the document list ===');
    {
      ...
      for (let i = 0; i < 20; i++)
        await p.fill('[aria-label="Diagram title"]', i % 2 ? 'Rebase me' : 'Rebase me!');
      await p.fill('[aria-label="Diagram title"]', 'Rebase me');
      await sleep(SAVE_WAIT);
      ...
      check(
        /Compacted|already compact/.test((await p.textContent('.document-manager__notice')) ?? ''),
        'compaction runs without asking and reports the result (R7)',
      );
      ...
      await Promise.all([
        p.waitForNavigation(),
        row('rebase-local').locator('.document-manager__rebase').click(),
      ]);
      await ready(p);
      ...
      check(
        !!newId && newId !== 'rebase-local',
        `the rebased document opens under a new id (${newId})`,
      );
      check(
        (await nodeCount(p)) === 25 && (await titleOf(p)) === 'Rebase me',
        'with the same content and name (R6)',
      );
      await openManager();
      check(
      ...
      if (
        await p.waitForSelector('.leave-guard', { timeout: 2000 }).then(
          () => true,
          () => false,
        )
      ) {
      ...
      await sleep(500);
      check(
        /blocked/.test(blockedQuestion) && /30 days/.test(blockedQuestion),
        'a recently shared document is blocked, naming the window (R8)',
      );
      check(/cannot sync/.test(blockedQuestion), 'and forcing it names the consequence');
      check(
        new URL(p.url()).searchParams.get('doc') === 'rebase-shared',
    ...
    console.log('\n=== WS13-R12: rehosting a room everyone has left ===');
    {
      ...
      if (
        await host.waitForSelector('.leave-guard', { timeout: 2000 }).then(
          () => true,
          () => false,
        )
      ) {
      ...
      check(
        (await sessionRow.count()) === 1,
        `the session's copy is listed as a document (session:${room})`,
      );
      check(
        (await sessionRow.locator('.document-manager__session').count()) === 1,
        'marked as coming from a session',
      );
      await Promise.all([
        guest.waitForNavigation(),
        sessionRow.locator('.document-manager__open').click(),
      ]);
      await ready(guest);
      check(
        (await nodeCount(guest)) === 25 && (await titleOf(guest)) === 'Rehost me',
        "and it opens with the session's content",
      );

      // ...and hosts the same room again.
      await guest.click('.collab-panel__trigger');
      const resume = guest.locator('.collab-panel__resume');
      const offered = await resume.waitFor({ timeout: 3000 }).then(
        () => true,
        () => false,
      );
      check(
        offered && ((await resume.textContent()) ?? '').includes(room),
        'the collab panel offers to resume that room',
      );
